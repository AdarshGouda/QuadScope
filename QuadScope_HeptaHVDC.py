...
# -----------------------------
# Reference (Moog) operating curves for milling motor
# -----------------------------
...
MOOG_MILLING_CONT = [
    ...
    (4000.0,  0.054, 23.532, 5.34, 1.113),
    (4250.0,  0.058, 23.434, 5.32, 1.178),
    (4500.0,  0.061, 23.333, 5.30, 1.242),
    (4750.0,  0.064, 23.230, 5.28, 1.306),
    (5000.0,  0.068, 23.126, 5.26, 1.369),
    (5250.0,  0.071, 23.019, 5.24, 1.432),
    (5500.0,  0.075, 22.911, 5.22, 1.495),
    (5750.0,  0.078, 22.802, 5.20, 1.557),
    (6000.0,  0.082, 22.691, 5.18, 1.619),
    (6250.0,  0.085, 22.579, 5.16, 1.681),
    (6500.0,  0.088, 22.466, 5.14, 1.742),
    (6750.0,  0.092, 22.351, 5.12, 1.803),
    (7000.0,  0.095, 22.236, 5.10, 1.864),
    (7250.0,  0.099, 22.119, 5.07, 1.925),
    (7500.0,  0.102, 22.001, 5.05, 1.985),
    (7750.0,  0.106, 21.882, 5.03, 2.045),
    (8000.0,  0.109, 21.762, 5.01, 2.105),
    (8250.0,  0.112, 21.640, 4.99, 2.165),
    (8500.0,  0.116, 21.517, 4.97, 2.224),
    (8750.0,  0.119, 21.393, 4.95, 2.283),
    (9000.0,  0.123, 21.268, 4.93, 2.342),
    (9250.0,  0.126, 21.142, 4.91, 2.401),
    (9500.0,  0.130, 21.015, 4.89, 2.460),
    (9750.0,  0.133, 20.887, 4.86, 2.519),
    (10000.0, 0.137, 20.758, 4.84, 2.577),
    (10250.0, 0.140, 20.628, 4.82, 2.636),
    (10500.0, 0.144, 20.496, 4.80, 2.694),
    (10750.0, 0.147, 20.364, 4.78, 2.752),
    (11000.0, 0.151, 20.230, 4.76, 2.810),
    (11250.0, 0.154, 20.095, 4.74, 2.868),
    (11424.2, 0.155, 20.000, 4.72, 2.910),
]
...
class SineFilterParams:
    ...
    """
    enabled: bool = False
    enforce_inv_current_limit: bool = False  # Option B: limit inverter-side current including filter reactive draw
...
class CableParams:
    ...
    i_limit_derate_with_temp: bool = False

    # inductance reduction factor when using 2 conductors in parallel (geometry-dependent)
    l_parallel_factor: float = 0.85


    # In COAX/Mono architecture, this 'cable' represents only a short downhole harness.
    # When drop_negligible=True, R and L are forced ~0 (no voltage-drop modeling),
    # but the phase current limit is retained as the downhole inverter phase current rating.
    ...
    def effective_r_phase(self) -> float:
        ...
        if bool(getattr(self, "drop_negligible", False)):
            return 0.0
    ...
    def i_phase_limit(self) -> float:
        ...
        if bool(getattr(self, "i_limit_derate_with_temp", False)):
            ...
            if (tf is not None) and (tf > 0.0):
                I = I / math.sqrt(tf)
        # In COAX/Mono mode, interpret i_limit_arms as the inverter *phase* current rating.
        if bool(getattr(self, "drop_negligible", False)):
            return I
...
@dataclass
class UPMParams:
    """UPM (DC source) stack model.

    Each module provides a nominal DC output (v_module_v) with a current capability (i_module_a).
    Stacking rules:
      - Series increases voltage capability (Vmax ∝ n_series)
      - Parallel increases current capability (Imax ∝ n_parallel)
    """

    enabled: bool = True
    # If True, solver uses UPM stack voltage as the DC-link source (overrides VfParams.vdc_link_v)
    use_as_vdc_source: bool = True

    # per-module ratings (defaults per user: 1100 VDC @ 2.5 A)
    v_module_v: float = 1100.0
    i_module_a: float = 2.5

    # stacking
    n_series: int = 1
    n_parallel: int = 1

    # commanded / set voltage (per module). If enforce_stack_limits=True, it is clamped to v_module_v.
    v_set_v: float = 1100.0

    # optional internal droop model (per module, at temp_ref)
    r_internal_ohm: float = 0.0
    eta: float = 1.0
    enforce_stack_limits: bool = True

    def _ns_np(self, coax_mode: bool = False) -> tuple[int, int]:
        ns = max(1, int(self.n_series))
        np = max(1, int(self.n_parallel))
        if coax_mode:
            # per user instruction: in COAX mode enforce n_series=1
            ns = 1
        return ns, np

    def stack_vdc(self, coax_mode: bool = False) -> float:
        ns, _ = self._ns_np(coax_mode=coax_mode)
        vmod = max(0.0, float(self.v_module_v))
        vset = max(0.0, float(self.v_set_v))
        if self.enforce_stack_limits:
            vset = min(vset, vmod)
        return vset * ns

    def stack_imax(self) -> float:
        _, np = self._ns_np(coax_mode=False)
        return max(0.0, float(self.i_module_a)) * np

    def stack_r_internal(self, coax_mode: bool = False) -> float:
        # series adds R, parallel divides R
        ns, np = self._ns_np(coax_mode=coax_mode)
        return max(0.0, float(self.r_internal_ohm)) * ns / np



@dataclass
class CoaxParams:
    """HVDC delivery over Heptacable (7-46A-XS-CB) feeding downhole inverter.

    Assumptions (per your latest direction):
      - Reserve 2 helical conductors + armor for LTS comms (unchanged from current ops).
      - Use the remaining 5 conductors for HVDC delivery (1 center + 4 helicals).
      - No DC-solenoid pair requirement.

    Implementation notes:
      - We model a *DC* loop: HVDC(+) bundle and HVDC(-) bundle, each as parallel conductors.
      - Wiring is represented by a compact key that specifies whether the center conductor is on the
        + or - bundle, and how many helicals share that same polarity.
      - We keep the existing 5-segment temperature model knobs from v36.

    Keys (8 canonical splits, electrically unique under {R_center != R_helical}):
      - "C+0H|4H", "C+1H|3H", "C+2H|2H", "C+3H|1H"
      - "1H|C+3H", "2H|C+2H", "3H|C+1H", "4H|C+0H"
    """

    enabled: bool = True
    enforce_i_limit: bool = True

    # Legacy field (kept so existing UI/status formatting doesn't break)
    return_path: str = ""

    # Length + temperature model (same UI semantics as v36)
    length_m: float = 7160.0  # 23 kft ~ 7010 m; keep existing default unless UI overwrites
    temp_ref_C: float = 20.0
    temp_alpha_per_C: float = 0.00393
    temp_model_5seg: bool = True
    temp5_seg_len_m: list = field(default_factory=lambda: [1500.0, 1100.0, 900.0, 1800.0, 1860.0])
    temp5_seg_temp_C: list = field(default_factory=lambda: [100.0,   125.0, 160.0, 175.0, 175.0])
    temp_factor_r: float = 1.0  # only used when temp_model_5seg is False

    # Datasheet DC resistances @ 20C (ohm/m). Reuse the existing entry fields:
    #  - r_center_ohm_per_m : center conductor
    #  - r_serve_ohm_per_m  : helical conductor (repurposed)
    #  - r_armor_ohm_per_m  : unused for HVDC loop (kept for UI compatibility)
    r_center_ohm_per_m: float = 23.0 / 1000.0   # 7.0 Ω/kft ≈ 23 Ω/km
    r_serve_ohm_per_m: float = 24.0 / 1000.0    # 7.3 Ω/kft ≈ 24 Ω/km (helical)
    r_armor_ohm_per_m: float = 4.3 / 1000.0     # 1.3 Ω/kft ≈ 4.3 Ω/km (unused in HVDC loop)

    # Per-conductor current rating (A). In HVDC bundling we enforce:
    #   I_dc <= I_cond * min(N_plus, N_minus)
    i_limit_a: float = 1.61  # per-conductor rating

    # Optional loop inductance (kept for UI compatibility; not used in steady-state torque calc)
    l_loop_h_per_m: float = 0.0

    # Wiring selections
    wiring_sel: str = "C+1H|3H"   # used for the main solve/envelope
    wiring_a: str = "C+1H|3H"     # Feasibility compare A
    wiring_b: str = "C+2H|2H"     # Feasibility compare B

    def wiring_catalog(self):
        return [
            ("C+0H|4H",   "+: C        | -: 4H        (1/4)"),
            ("C+1H|3H",   "+: C+1H     | -: 3H        (2/3)"),
            ("C+2H|2H",   "+: C+2H     | -: 2H        (3/2)"),
            ("C+3H|1H",   "+: C+3H     | -: 1H        (4/1)"),
            ("1H|C+3H",   "+: 1H       | -: C+3H      (1/4, C on -)"),
            ("2H|C+2H",   "+: 2H       | -: C+2H      (2/3, C on -)"),
            ("3H|C+1H",   "+: 3H       | -: C+1H      (3/2, C on -)"),
            ("4H|C+0H",   "+: 4H       | -: C         (4/1, C on -)"),
            ("1H|3H",     "+: 1H       | -: 3H        (1/3, no C)"),
            ("1H|1H",     "+: 1H       | -: 1H        (1/1, no C)"),
            ("2H|2H",     "+: 2H       | -: 2H        (2/2, no C)"),
            ("3H|1H",     "+: 3H       | -: 1H        (3/1, no C)"),
        ]

    def _parse_side(self, s: str):
        s = str(s).strip().upper()
        nC = 1 if "C" in s else 0
        nH = 0
        if "H" in s:
            # accept formats: '3H', 'C+2H'
            t = s.replace("C+", "").replace("+C", "")
            t = t.replace("C", "")
            t = t.strip()
            if t == "H":
                nH = 1
                ...
                m = re.search(r"(\d+)H", t)
                nH = int(m.group(1)) if m else 0
        return int(nC), int(max(0, nH))
    ...
    def _parse_key(self, key: str):
        ...
        nC_p, nH_p = self._parse_side(left)
        nC_m, nH_m = self._parse_side(right)
        return nC_p, nH_p, nC_m, nH_m

    def _temp_scale(self, T: float) -> float:
        return max(0.0, 1.0 + float(self.temp_alpha_per_C) * (float(T) - float(self.temp_ref_C)))

    def _r_conductor_total(self, is_center: bool) -> float:
        r_per_m = float(self.r_center_ohm_per_m if is_center else self.r_serve_ohm_per_m)
        if self.temp_model_5seg and len(self.temp5_seg_len_m) == 5 and len(self.temp5_seg_temp_C) == 5:
            r = 0.0
            for L, T in zip(self.temp5_seg_len_m, self.temp5_seg_temp_C):
                r += r_per_m * float(L) * self._temp_scale(float(T))
            return max(0.0, r)
        return max(0.0, r_per_m * float(self.length_m) * float(self.temp_factor_r))

    def _req_parallel(self, nC: int, nH: int, R_C: float, R_H: float) -> float:
        if (nC + nH) <= 0:
            return float("inf")
        inv = 0.0
        if nC > 0 and R_C > 0:
            inv += (float(nC) / float(R_C))
        if nH > 0 and R_H > 0:
            inv += (float(nH) / float(R_H))
        if inv <= 0.0:
            return float("inf")
        return 1.0 / inv

    def loop_resistance_ohm(self, key: str = None) -> float:
        key = str(key or self.wiring_sel).strip()
        nC_p, nH_p, nC_m, nH_m = self._parse_key(key)
        R_C = self._r_conductor_total(is_center=True)
        R_H = self._r_conductor_total(is_center=False)
        R_plus = self._req_parallel(nC_p, nH_p, R_C, R_H)
        R_minus = self._req_parallel(nC_m, nH_m, R_C, R_H)
        if not math.isfinite(R_plus) or not math.isfinite(R_minus):
            return float("inf")
        return max(0.0, float(R_plus + R_minus))

    def dc_current_limit_a(self, key: str = None) -> float:
        """Bundle-aware DC current limit based on per-conductor rating."""
        if not bool(self.enforce_i_limit):
            return float("inf")
        key = str(key or self.wiring_sel).strip()
        nC_p, nH_p, nC_m, nH_m = self._parse_key(key)
        Np = int(nC_p + nH_p)
        ...
        if Np <= 0 or Nm <= 0:
            return 0.0
        return max(0.0, float(self.i_limit_a) * float(min(Np, Nm)))

    def loop_inductance_h(self) -> float:
        return max(0.0, float(self.l_loop_h_per_m) * float(self.length_m))
@dataclass
class MotorParams:
    """
...
class LimitsParams:
    ...
    downhole_vll_rms_limit: float = 500.0  # optional (line-to-line)
...
class SystemParams:
    ...
    upm: UPMParams = field(default_factory=UPMParams)
    coax: CoaxParams = field(default_factory=CoaxParams)
    ...
    regen_cable_limit_enabled: bool = False
...
class SystemModel:
    ...
    # ------------------------------------------------------------------
    # v30: UPM + COAX DC-link model (binding droop + DC current limit)
    # ------------------------------------------------------------------
    def _dc_link_model_params(self):
        """Return (enabled, vdc_source, r_tot, i_lim_dc, eta_src, coax_mode).

        This models the *DC* path: UPM (source) → COAX (monocable) → inverter DC link.

        Notes:
          - Vdc_source is the *setpoint* at the source (UPM setpoint if enabled+override, else UI Vdc knob).
          - r_tot is source internal R + coax loop R.
          - i_lim_dc is enforced as the minimum of (UPM stack Imax, coax rating if enabled).
          - In coax mode we force n_series=1 at the UPM stack (series stacking disabled).
        """
        p = self.p
        # Only meaningful when the inverter voltage basis is DC-link
        basis_is_vdc = str(getattr(p.vf, 'voltage_entry_basis', 'DC')).strip().upper().startswith('DC')
        if not basis_is_vdc:
            return (False, float(p.vf.vdc_link_v), 0.0, float('inf'), 1.0, False)

        upm = getattr(p, 'upm', None)
        coax = getattr(p, 'coax', None)
        upm_en = bool(getattr(upm, 'enabled', False)) if upm is not None else False
        coax_en = bool(getattr(coax, 'enabled', False)) if coax is not None else False
        coax_mode = bool(coax_en)

        if not (upm_en or coax_en):
            return (False, float(p.vf.vdc_link_v), 0.0, float('inf'), 1.0, coax_mode)

        # Defaults
        vdc_source = float(p.vf.vdc_link_v)
        eta_src = 1.0
        r_src = 0.0
        i_lim_upm = float('inf')

        if upm_en:
            eta_src = max(1e-6, float(getattr(upm, 'eta', 1.0)))
            # In coax mode, we restrict series stacking to 1.
            vdc_source_upm = float(upm.stack_vdc(coax_mode=coax_mode))
            if bool(getattr(upm, 'use_as_vdc_source', False)):
                vdc_source = vdc_source_upm
            r_src = float(upm.stack_r_internal(coax_mode=coax_mode))
            i_lim_upm = float(upm.stack_imax())

        r_coax = 0.0
        i_lim_coax = float('inf')
        if coax_en:
            wkey = str(getattr(coax, 'wiring_sel', 'C+1H|3H'))
            r_coax = float(coax.loop_resistance_ohm(wkey))
            if bool(getattr(coax, 'enforce_i_limit', True)):
                i_lim_coax = float(getattr(coax, 'dc_current_limit_a', lambda k=None: getattr(coax, 'i_limit_a', float('inf')))(wkey))

        r_tot = float(r_src + r_coax)
        i_lim_dc = float(min(i_lim_upm, i_lim_coax))

        enabled = True
        return (enabled, float(vdc_source), float(r_tot), float(i_lim_dc), float(eta_src), coax_mode)
    def _max_iq_given_limits_no_dc_droop(self, motor_rpm: float, cable: CableParams) -> Tuple[
    ...
    def max_iq_given_limits(self, motor_rpm: float, cable: CableParams):
        """Binding DC-link model (UPM + COAX) with inverter-side Vdc droop.

        In COAX mode, the inverter-side DC link voltage is not the UI 'Vdc set' directly.
        It is the source setpoint minus the DC path drop:

            Vdc@inv = Vsource - Idc * Rloop

        But Idc depends on the achievable operating point, which depends on Vdc@inv via
        the inverter voltage limit. Therefore we solve a small fixed-point problem.

        v32 issue (what caused 'dips'):
          - only 4 iterations
          - no damping
          - returned the *last* iterate (which can be a bad, non-self-consistent point
            when the map oscillates)

        This version:
          - uses under-relaxation (damping)
          - runs more iterations
          - returns the most self-consistent iterate (min |Vdc_new - Vdc_guess|)
        """
        enabled, vdc_src, r_tot, i_lim_dc, eta_src, _coax_mode = self._dc_link_model_params()
        if (not enabled) or (r_tot <= 1e-12 and (not (i_lim_dc < float('inf')))):
            res = self._max_iq_given_limits_no_dc_droop(motor_rpm, cable)
            # Cache DC-link interface estimates (used by DC Link sweep tab)
            idc_est = 0.0
            try:
                iq, idb, im, f_e, v_cmd, v_surf_lim, v_dh = res
                v_motor = self.motor_voltage_required_phase_rms(motor_rpm, idb, iq)
                v_drop = self.cable_drop_phase_rms(motor_rpm, im, cable)
                v_node = float(v_motor) + float(v_drop)
                i_inv = self.inverter_current_mag_rms(motor_rpm, v_node, float(im))
                v_inv = self.inverter_voltage_required_phase_rms(motor_rpm, v_node, float(im))
                s_va = max(0.0, 3.0 * float(v_inv) * float(i_inv))
                idc_est = (s_va / max(1e-6, float(eta_src))) / max(1e-6, float(vdc_src))
            except Exception:
                idc_est = 0.0
            self._dc_last_vdc_source = float(vdc_src)
            self._dc_last_vdc_at_inv = float(vdc_src)
            self._dc_last_idc = float(idc_est)
            self._dc_last_r_tot = float(r_tot)
            self._dc_last_i_lim = float(i_lim_dc)
            return res

        p = self.p
        vf = p.vf
        vdc_orig = float(vf.vdc_link_v)

        # Solver controls (kept internal to avoid UI churn)
        max_iter = 25
        relax = 0.35          # 0=no update, 1=full update
        tol_v = 0.5           # volts

        vdc_guess = max(1e-6, float(vdc_src))
        best = None
        best_err = float("inf")
        best_vdc_at_inv = float(vdc_guess)
        best_idc = 0.0

        try:
            for _ in range(max_iter):
                # Use the *estimated inverter-side* Vdc to derive the AC voltage limit
                vf.vdc_link_v = float(vdc_guess)

                iq, idb, im, f_e, v_cmd, v_surf_lim, v_dh = self._max_iq_given_limits_no_dc_droop(motor_rpm, cable)

                # Compute inverter-side voltage/current at this operating point (for DC current estimate)
                v_motor = self.motor_voltage_required_phase_rms(motor_rpm, idb, iq)
                v_drop = self.cable_drop_phase_rms(motor_rpm, im, cable)
                v_node = float(v_motor) + float(v_drop)
                i_inv = self.inverter_current_mag_rms(motor_rpm, v_node, float(im))
                ...
                # Conservative DC estimate using apparent power
                s_va = max(0.0, 3.0 * float(v_inv) * float(i_inv))
                idc = (s_va / max(1e-6, float(eta_src))) / max(1e-6, float(vdc_guess))

                # Enforce DC current limit by scaling Iq downward (first-order, conservative)
                if (i_lim_dc < float('inf')) and (idc > i_lim_dc + 1e-9) and (iq > 1e-12):
                    scale = max(0.0, min(1.0, float(i_lim_dc) / max(1e-12, float(idc))))
                    iq = float(iq) * scale
                    im = math.sqrt(float(idb) * float(idb) + float(iq) * float(iq))
                ...
                # Update Vdc at inverter after DC path drop
                vdc_new = max(0.0, float(vdc_src) - float(idc) * float(r_tot))
                err = abs(vdc_new - vdc_guess)

                # Keep the most self-consistent iterate
                if err < best_err:
                    best_err = err
                    best = (float(iq), float(idb), float(im), float(f_e), float(v_cmd), float(v_surf_lim), v_dh)
                    best_vdc_at_inv = float(vdc_guess)
                    best_idc = float(idc)

                if err < tol_v:
                    vdc_guess = max(1e-6, vdc_new)
                    break

                # Under-relaxed fixed-point update (stabilizes oscillation)
                vdc_guess = (1.0 - relax) * float(vdc_guess) + relax * float(vdc_new)
                vdc_guess = max(1e-6, min(float(vdc_src), float(vdc_guess)))

        finally:
            vf.vdc_link_v = float(vdc_orig)

        if best is None:
            return self._max_iq_given_limits_no_dc_droop(motor_rpm, cable)

        # If the droop drives Vdc to ~0, force zero capability
        if vdc_guess <= 1e-6:
            return 0.0, 0.0, 0.0, best[3], best[4], best[5], best[6]

        # Cache best DC-link fixed-point result for interface plots
        self._dc_last_vdc_source = float(vdc_src)
        self._dc_last_vdc_at_inv = float(best_vdc_at_inv)
        self._dc_last_idc = float(best_idc)
        self._dc_last_r_tot = float(r_tot)
        self._dc_last_i_lim = float(i_lim_dc)

        return best

    def _max_iq_given_limits_fast_no_dc_droop(self, motor_rpm: float, cable: CableParams) -> Tuple[
        float, float, float, float, float, float, Optional[float]]:
    ...
    def max_iq_given_limits_fast(self, motor_rpm: float, cable: CableParams):
        """Fast binding DC-link model (UPM + COAX) with inverter-side Vdc droop.

        Same idea as max_iq_given_limits(), but uses the fast no-droop solver inside the loop.
        """
        enabled, vdc_src, r_tot, i_lim_dc, eta_src, _coax_mode = self._dc_link_model_params()
        if (not enabled) or (r_tot <= 1e-12 and (not (i_lim_dc < float('inf')))):
            res = self._max_iq_given_limits_fast_no_dc_droop(motor_rpm, cable)
            # Cache DC-link interface estimates (used by DC Link sweep tab)
            idc_est = 0.0
            try:
                iq, idb, im, f_e, v_cmd, v_surf_lim, v_dh = res
        ...
        try:
            for _ in range(max_iter):
                ...
                iq, idb, im, f_e, v_cmd, v_surf_lim, v_dh = self._max_iq_given_limits_fast_no_dc_droop(motor_rpm, cable)
        ...
        if best is None:
            return self._max_iq_given_limits_fast_no_dc_droop(motor_rpm, cable)
        if vdc_guess <= 1e-6:
            return 0.0, 0.0, 0.0, best[3], best[4], best[5], best[6]
        # Cache best DC-link fixed-point result for interface plots
        self._dc_last_vdc_source = float(vdc_src)
    ...
    def _find_feasible_id_for_iq_no_dc_droop(self, motor_rpm: float, iq_req: float, cable: 'CableParams') -> Tuple[
        bool, float, float, float, float, float, Optional[float]]:
        """No-DC-droop feasibility check for a required Iq.

        Returns:
          feasible, id_used_rms, i_mag_used_rms, f_e, v_cmd, v_surface_limit, v_dh_phase

        Notes:
          - This is the classic v22 check: scan Id candidates (0 and optional negative Id for FW),
            and verify voltage/current/downhole constraints at that operating point.
          - It uses the *current* p.vf.vdc_link_v to derive inverter voltage limit via _v_cmd_phase().
        """
        p = self.p
        fw = p.fw

        # Keep consistent with max_iq_given_limits(): allow control strategy to override FW behavior
        mode = getattr(p, "control_strategy", "VF")
        fw_enabled_eff = bool(getattr(fw, "enabled", False))
        if mode == "MODE_A":
            fw_enabled_eff = False
        elif mode == "MODE_B":
            fw_enabled_eff = True
        ...
        # candidate Id range
        if (not fw_enabled_eff) or float(getattr(fw, "id_max_arms", 0.0)) <= 1e-9 or (
            bool(getattr(fw, "apply_only_above_base", False)) and f_e <= p.vf.base_freq_hz + 1e-9
        ):
            id_candidates = [0.0]
        else:
            id_max = min(float(abs(getattr(fw, "id_max_arms", 0.0))), float(i_lim))
            n_id = max(11, int(getattr(fw, "id_grid_points", 21)))
            id_candidates = np.linspace(0.0, -id_max, n_id)
        ...
        for id_rms in id_candidates:
            i_mag = math.sqrt(float(id_rms) * float(id_rms) + float(iq_req) * float(iq_req))
            if i_mag > float(i_lim) + 1e-12:
                continue

            v_motor = self.motor_voltage_required_phase_rms(motor_rpm, float(id_rms), float(iq_req))
            v_drop = self.cable_drop_phase_rms(motor_rpm, float(i_mag), cable)
            v_node = float(v_motor) + float(v_drop)

            # Option B: inverter-side current limit when sine filter is enabled
            if sf.enabled and getattr(sf, 'enforce_inv_current_limit', True):
                i_inv = self.inverter_current_mag_rms(motor_rpm, v_node, float(i_mag))
                if float(i_inv) > float(i_lim) + 1e-12:
                    continue
            ...
            if v_dh_phase is not None and float(v_motor) > float(v_dh_phase) + 1e-12:
                continue

            # feasible
            if best is None or float(i_mag) < best[1]:
                best = (float(id_rms), float(i_mag))

        if best is None:
            return False, 0.0, float(i_lim), float(f_e), float(v_cmd), float(v_surface_limit), v_dh_phase

        return True, float(best[0]), float(best[1]), float(f_e), float(v_cmd), float(v_surface_limit), v_dh_phase

    def find_feasible_id_for_iq(self, motor_rpm: float, iq_req: float, cable: 'CableParams') -> Tuple[
        bool, float, float, float, float, float, Optional[float]]:
        """Binding DC-link feasibility check for required Iq (UPM + COAX droop).

        When UPM/COAX DC-link modeling is enabled, the inverter-side DC bus is:

            Vdc@inv = Vsource - Idc * Rtot

        But Idc depends on the operating point (through inverter power), which depends on Vdc@inv
        (through the inverter voltage limit). Therefore we iterate a small fixed-point loop.
        ...
        # If no droop model is active, fall back to classic behavior
        if (not enabled) or (r_tot <= 1e-12 and not (i_lim_dc < float('inf'))):
            return self._find_feasible_id_for_iq_no_dc_droop(motor_rpm, iq_req, cable)
        ...
        try:
            for _ in range(max_iter):
                ...
                feasible, id_used, im_used, f_e, v_cmd, v_surf_lim, v_dh = self._find_feasible_id_for_iq_no_dc_droop(
                    motor_rpm, iq_req, cable
                )

                # If not feasible even at this (guessed) Vdc, and we're starting from the maximum Vdc (Vsource),
                # then it's not feasible under droop either.
                if not feasible:
                    # Still update the "best" with infeasible info for reporting consistency
                    return False, 0.0, float(cable.i_phase_limit()), float(f_e), float(v_cmd), float(v_surf_lim), v_dh

                # Compute inverter-side VA (conservative) and corresponding DC current estimate
                v_motor = self.motor_voltage_required_phase_rms(motor_rpm, float(id_used), float(iq_req))
                v_drop = self.cable_drop_phase_rms(motor_rpm, float(im_used), cable)
                v_node = float(v_motor) + float(v_drop)
                i_inv = self.inverter_current_mag_rms(motor_rpm, v_node, float(im_used))
                v_inv = self.inverter_voltage_required_phase_rms(motor_rpm, v_node, float(im_used))
                s_va = max(0.0, 3.0 * float(v_inv) * float(i_inv))

                idc = (s_va / max(1e-6, float(eta_src))) / max(1e-6, float(vdc_guess))

                # If DC current limit is exceeded, this required point is not feasible.
                if (i_lim_dc < float('inf')) and (idc > float(i_lim_dc) + 1e-9):
                    return False, 0.0, float(cable.i_phase_limit()), float(f_e), float(v_cmd), float(v_surf_lim), v_dh
                ...
                if err < best_err:
                    best_err = err
                    best = (True, float(id_used), float(im_used), float(f_e), float(v_cmd), float(v_surf_lim), v_dh)
...
def slider_for_spin(spin) -> QSlider:
    ...
    if isinstance(spin, QDoubleSpinBox):
        ...
        def _spin_to_slider(_=None):
            idx = int(round((float(spin.value()) - lo) / step))
...
class DecisionTablesPane(QWidget):
    """Decision/summary tables for quick tradeoffs.

    Shows:
      - Max torque vs RPM for COAX-Mono architecture (with bands)
      - Electrical utilization at UI target (COAX-Mono)
      - Voltage/current summaries at the UI target + envelope checkpoints
      - DC/UPM/COAX summary + contact-block creepage constraint
    """

    def __init__(self, parent: Optional[QWidget] = None):
        super().__init__(parent)
        # 3x2 grid so we can show: envelope torque table, utilization, diagnostics, gates/winner, DC summary, limits.
        self.fig = Figure(figsize=(13.0, 9.2))
        self.canvas = FigureCanvas(self.fig)
        self.canvas.setSizePolicy(QSizePolicy.Expanding, QSizePolicy.Expanding)
...
class MotorPane(QWidget):
    """Motor-focused diagnostics.

    Goal: show what the motor is doing across RPM (Iq/Id, voltage headroom, torque, power),
    plus how that maps through the drivetrain to output torque.
    """
    ...
    def __init__(self):
        ...
        self.fig = Figure(figsize=(15.2, 8.8), dpi=100)
        gs = self.fig.add_gridspec(nrows=1, ncols=2, width_ratios=[2.35, 1.0], wspace=0.18)

        gs_left = gs[0, 0].subgridspec(nrows=2, ncols=1, hspace=0.22)
        gs_right = gs[0, 1].subgridspec(nrows=3, ncols=1, hspace=0.32)

        # Left: big plots
        self.ax_motor = self.fig.add_subplot(gs_left[0, 0])
        self.ax_out = self.fig.add_subplot(gs_left[1, 0])

        # Right: currents, voltages, and a compact table
        self.ax_curr = self.fig.add_subplot(gs_right[0, 0])
        self.ax_volt = self.fig.add_subplot(gs_right[1, 0])
        self.ax_tbl = self.fig.add_subplot(gs_right[2, 0])

        self.fig.subplots_adjust(left=0.05, right=0.99, top=0.96, bottom=0.07)
...
class MoogCurvesPane(QWidget):
    """Reference Moog-style operating curves vs current τ_extra model.

    This is intended as a sanity/fit check: if τ_extra(ω) matches the Moog inferred loss torque,
    then reconstructing torque-out from I·Kt should overlay the Moog torque-out curve.
    """
    ...
    def __init__(self):
        ...
        self.fig = Figure(figsize=(9.5, 6.0), tight_layout=True)
        self.canvas = FigureCanvas(self.fig)
        ...
        self.ax_tq = axs[0, 0]
        self.ax_i = axs[0, 1]
        self.ax_loss = axs[1, 0]
        self.ax_visc = axs[1, 1]

    @staticmethod
    def _dataset_for_preset(preset_name: str):
        # For now: Spear + Annular placeholders use Milling dataset.
        return MOOG_MILLING_CONT, MOOG_MILLING_PEAK, MOOG_MILLING_KTTR_LBIN_PER_ARMS

    def render(self, model: 'SystemModel', preset_name: str = "Milling Moog Curve"):
        cont, peak, kttr = self._dataset_for_preset(preset_name)
        ...
        def to_arrays(rows):
            ...
            cur = a[:, 3]
            pkw = a[:, 4]
            return rpm, visc, tq_out, cur, pkw

        rpm_c, visc_c, tq_c, i_c, p_c = to_arrays(cont)
        rpm_p, visc_p, tq_p, i_p, p_p = to_arrays(peak)

        # Current τ_extra model predictions at these speeds
        def model_taus(rpm_arr):
            omega = rpm_arr * (2.0 * math.pi / 60.0)
            tau_extra_nm = np.zeros_like(omega)
            tau_core_nm = np.zeros_like(omega)
            tau_visc_nm = np.zeros_like(omega)
            for k, w in enumerate(omega):
                te, tc, tv = model.tau_extra_nm(float(abs(w)))
                tau_extra_nm[k] = te
                tau_core_nm[k] = tc
                tau_visc_nm[k] = tv
            # convert Nm -> lbf-in
            nm_to_lbf_in = 1.0 / LBIN_TO_NM
            return tau_extra_nm * nm_to_lbf_in, tau_core_nm * nm_to_lbf_in, tau_visc_nm * nm_to_lbf_in

        tau_extra_c, tau_core_c, tau_visc_c = model_taus(rpm_c)
        tau_extra_p, tau_core_p, tau_visc_p = model_taus(rpm_p)

        # Moog inferred loss torque: τ_loss = I·KtTR - τ_out
        tau_loss_c = i_c * kttr - tq_c
        tau_loss_p = i_p * kttr - tq_p

        # Reconstructed torque-out using current τ_extra model: τ_out_recon = I·KtTR - τ_extra_model
        tq_recon_c = i_c * kttr - tau_extra_c
        tq_recon_p = i_p * kttr - tau_extra_p

        # Clear axes
        for ax in (self.ax_tq, self.ax_i, self.ax_loss, self.ax_visc):
            ax.clear()

        title = preset_name.strip() or "Moog Curve"
        self.ax_tq.set_title(f"Torque-out vs speed ({title})")
        self.ax_tq.plot(rpm_c, tq_c, label="Moog torque-out (cont)")
        self.ax_tq.plot(rpm_c, tq_recon_c, label="Recon torque-out (model τ_extra)")
        self.ax_tq.plot(rpm_p, tq_p, linestyle="--", label="Moog torque-out (peak)")
        self.ax_tq.plot(rpm_p, tq_recon_p, linestyle="--", label="Recon torque-out (peak)")
        self.ax_tq.set_xlabel("Motor speed (rpm)")
        self.ax_tq.set_ylabel("Torque (lbf-in)")
        self.ax_tq.grid(True)
        self.ax_tq.legend(fontsize=8)

        self.ax_i.set_title("Current vs speed")
        self.ax_i.plot(rpm_c, i_c, label="Moog current (cont)")
        self.ax_i.plot(rpm_p, i_p, linestyle="--", label="Moog current (peak)")
        self.ax_i.set_xlabel("Motor speed (rpm)")
        self.ax_i.set_ylabel("Current (Arms)")
        self.ax_i.grid(True)
        self.ax_i.legend(fontsize=8)

        self.ax_loss.set_title("Inferred loss torque vs model τ_extra")
        self.ax_loss.plot(rpm_c, tau_loss_c, label="Moog inferred loss (cont)")
        self.ax_loss.plot(rpm_c, tau_extra_c, label="Model τ_extra (cont)")
        self.ax_loss.plot(rpm_c, tau_core_c, label="Model τ_core (cont)")
        self.ax_loss.plot(rpm_c, tau_visc_c, label="Model τ_visc (cont)")
        self.ax_loss.set_xlabel("Motor speed (rpm)")
        self.ax_loss.set_ylabel("Loss torque (lbf-in)")
        self.ax_loss.grid(True)
        self.ax_loss.legend(fontsize=8)

        self.ax_visc.set_title("Viscous drag vs model τ_visc")
        self.ax_visc.plot(rpm_c, visc_c, label="Moog visc drag (cont)")
        self.ax_visc.plot(rpm_c, tau_visc_c, label="Model τ_visc (cont)")
        self.ax_visc.plot(rpm_p, visc_p, linestyle="--", label="Moog visc drag (peak)")
        self.ax_visc.plot(rpm_p, tau_visc_p, linestyle="--", label="Model τ_visc (peak)")
        self.ax_visc.set_xlabel("Motor speed (rpm)")
        self.ax_visc.set_ylabel("Torque (lbf-in)")
        self.ax_visc.grid(True)
        self.ax_visc.legend(fontsize=8)

        # Simple RMSE metric on torque-out
        def rmse(a, b):
            a = np.asarray(a, dtype=float)
            b = np.asarray(b, dtype=float)
            return float(np.sqrt(np.mean((a - b) ** 2)))

        rmse_tq = rmse(tq_c, tq_recon_c)
        rmse_loss = rmse(tau_loss_c, tau_extra_c)

        self.txt.setPlainText(
            "\n".join([
                f"Preset: {title}",
                f"KtTR (Moog ref): {kttr:.4f} lbf-in/Arms",
                f"RMSE torque-out (cont): {rmse_tq:.3f} lbf-in",
                f"RMSE inferred loss vs τ_extra (cont): {rmse_loss:.3f} lbf-in",
                "",
                "Notes:",
                "• Spear/Annular presets are placeholders (currently using Milling dataset).",
                "• Recon torque-out uses Moog current curve and subtracts the current τ_extra model.",
            ])
        )
...
class NavigatorVfWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("QuadScope Mono v36.0")
        ...
        self.sweep_dclink = QuadSweepPane()
        ...
        self.sweep_tabs.addTab(self.sweep_dclink, "DC Link")
        ...
        self.mpl_tables = DecisionTablesPane()
    ...
    def _build_controls(self):
        title = QLabel("QuadScope: Navigator CCRS Monocable Static Solver")
        f = QFont()
        f.setPointSize(12)
        f.setBold(True)
        title.setFont(f)
        ...
        add_slider_row(G, 1, "Stuck/stall torque requirement (ft-lbf)", self.t_out_tq)
        ...
        self.chk_regen_cable.setChecked(bool(getattr(self.params, "regen_cable_limit_enabled", False)))
        ...
        # -----------------------------
        # v30: UPM + COAX (Monocable) blocks (UPM → COAX → Surface Inverter)
        # -----------------------------

        # UPM (DC source / stacking)
        gb_upm = QGroupBox("UPM (DC SOURCE / STACK)")
        U = QGridLayout(gb_upm)
        U.setContentsMargins(8, 8, 8, 8)
        U.setHorizontalSpacing(8)
        U.setVerticalSpacing(4)

        self.upm_enable = QCheckBox("Enable UPM block")
        self.upm_enable.setChecked(bool(getattr(self.params, "upm", UPMParams()).enabled))

        self.upm_use_as_vdc = QCheckBox("Use UPM voltage as Vdc source (override Vdc link)")
        self.upm_use_as_vdc.setChecked(bool(getattr(self.params, "upm", UPMParams()).use_as_vdc_source))

        U.addWidget(self.upm_enable, 0, 0, 1, 2)
        U.addWidget(self.upm_use_as_vdc, 0, 2, 1, 2)

        U.addWidget(QLabel("Module Vmax (V)"), 1, 0)
        self.upm_vmod = make_dspin(0.0, 2000.0, 10.0, float(self.params.upm.v_module_v), 1)
        U.addWidget(self.upm_vmod, 1, 1)

        U.addWidget(QLabel("Module Imax (A)"), 1, 2)
        self.upm_imod = make_dspin(0.0, 20.0, 0.1, float(self.params.upm.i_module_a), 2)
        U.addWidget(self.upm_imod, 1, 3)

        U.addWidget(QLabel("n_series"), 2, 0)
        self.upm_n_series = make_ispin(1, 3, 1, int(self.params.upm.n_series))
        U.addWidget(self.upm_n_series, 2, 1)

        U.addWidget(QLabel("n_parallel"), 2, 2)
        self.upm_n_parallel = make_ispin(1, 3, 1, int(self.params.upm.n_parallel))
        U.addWidget(self.upm_n_parallel, 2, 3)

        U.addWidget(QLabel("Vset/module (V)"), 3, 0)
        self.upm_vset = make_dspin(0.0, 2000.0, 10.0, float(self.params.upm.v_set_v), 1)
        U.addWidget(self.upm_vset, 3, 1)

        U.addWidget(QLabel("Rint/module (Ω)"), 3, 2)
        self.upm_rint = make_dspin(0.0, 50.0, 0.01, float(self.params.upm.r_internal_ohm), 3)
        U.addWidget(self.upm_rint, 3, 3)

        U.addWidget(QLabel("UPM η"), 4, 0)
        self.upm_eta = make_dspin(0.50, 1.00, 0.01, float(self.params.upm.eta), 2)
        U.addWidget(self.upm_eta, 4, 1)

        self.lbl_upm_stack = QLabel("")
        self.lbl_upm_stack.setWordWrap(True)
        self.lbl_upm_stack.setStyleSheet("color:#444;")
        U.addWidget(self.lbl_upm_stack, 4, 2, 1, 2)

        self.left_layout.addWidget(gb_upm)

        # COAX / monocable (DC link between UPM and surface inverter)
        gb_coax = QGroupBox("HEPTACABLE HVDC (DC LINK)")
        Cx = QGridLayout(gb_coax)
        Cx.setContentsMargins(8, 8, 8, 8)
        Cx.setHorizontalSpacing(8)
        Cx.setVerticalSpacing(4)

        self.coax_enable = QCheckBox("Enable HVDC block")
        self.coax_enable.setChecked(bool(getattr(self.params, "coax", CoaxParams()).enabled))
        self.coax_enforce_ilim = QCheckBox("Enforce HVDC current rating")
        self.coax_enforce_ilim.setChecked(bool(getattr(self.params, "coax", CoaxParams()).enforce_i_limit))
        Cx.addWidget(self.coax_enable, 0, 0, 1, 2)
        Cx.addWidget(self.coax_enforce_ilim, 0, 2, 1, 2)

        Cx.addWidget(QLabel("HVDC wiring (main)"), 1, 0)
        self.coax_return = QComboBox()
        self.coax_return.addItems([k for (k, _) in self.params.coax.wiring_catalog()])
        self.coax_return.setCurrentText(str(getattr(self.params.coax, "wiring_sel", "C+1H|3H")))
        Cx.addWidget(self.coax_return, 1, 1, 1, 3)

        Cx.addWidget(QLabel("Compare A"), 2, 0)
        self.coax_wiring_a = QComboBox()
        self.coax_wiring_a.addItems([k for (k, _) in self.params.coax.wiring_catalog()])
        self.coax_wiring_a.setCurrentText(str(getattr(self.params.coax, "wiring_a", getattr(self.params.coax, "wiring_sel", "C+1H|3H"))))
        Cx.addWidget(self.coax_wiring_a, 2, 1)

        Cx.addWidget(QLabel("Compare B"), 2, 2)
        self.coax_wiring_b = QComboBox()
        self.coax_wiring_b.addItems([k for (k, _) in self.params.coax.wiring_catalog()])
        self.coax_wiring_b.setCurrentText(str(getattr(self.params.coax, "wiring_b", "C+2H|2H")))
        Cx.addWidget(self.coax_wiring_b, 2, 3)

        Cx.addWidget(QLabel("I_cond (A)"), 3, 0)
        self.coax_ilim = make_dspin(0.0, 20.0, 0.1, float(self.params.coax.i_limit_a), 2)
        Cx.addWidget(self.coax_ilim, 3, 1)

        Cx.addWidget(QLabel("L_loop (uH/m)"), 3, 2)
        self.coax_lloop_uhpm = make_dspin(0.0, 20.0, 0.01, float(self.params.coax.l_loop_h_per_m) * 1e6, 3)
        Cx.addWidget(self.coax_lloop_uhpm, 3, 3)

        # Resistances shown as Ω/km (datasheet style)
        Cx.addWidget(QLabel("R_center (Ω/km)"), 4, 0)
        self.coax_rcenter_okm = make_dspin(0.0, 100.0, 0.1, float(self.params.coax.r_center_ohm_per_m) * 1000.0, 2)
        Cx.addWidget(self.coax_rcenter_okm, 4, 1)

        Cx.addWidget(QLabel("R_helical (Ω/km)"), 4, 2)
        self.coax_rserve_okm = make_dspin(0.0, 100.0, 0.1, float(self.params.coax.r_serve_ohm_per_m) * 1000.0, 2)
        Cx.addWidget(self.coax_rserve_okm, 4, 3)

        Cx.addWidget(QLabel("R_armor (Ω/km)"), 5, 0)
        self.coax_rarmor_okm = make_dspin(0.0, 100.0, 0.1, float(self.params.coax.r_armor_ohm_per_m) * 1000.0, 2)
        Cx.addWidget(self.coax_rarmor_okm, 5, 1)

        Cx.addWidget(QLabel("Tref (°C)"), 5, 2)
        self.coax_tref = make_dspin(-20.0, 250.0, 1.0, float(self.params.coax.temp_ref_C), 1)
        Cx.addWidget(self.coax_tref, 5, 3)

        self.coax_temp_model_5seg = QCheckBox("5-seg temperature model")
        self.coax_temp_model_5seg.setChecked(bool(self.params.coax.temp_model_5seg))
        Cx.addWidget(self.coax_temp_model_5seg, 6, 0, 1, 2)

        Cx.addWidget(QLabel("α (1/°C)"), 6, 2)
        self.coax_alpha = make_dspin(0.0005, 0.01, 0.0001, float(self.params.coax.temp_alpha_per_C), 5)
        Cx.addWidget(self.coax_alpha, 6, 3)

# 5-seg controls
        self.coax_seg_len = []
        self.coax_seg_temp = []
        for i in range(5):
            Cx.addWidget(QLabel(f"Seg {i+1} L (m)"), 7 + i, 0)
            wL = make_dspin(0.0, 10000.0, 10.0, float(self.params.coax.temp5_seg_len_m[i]), 1)
            Cx.addWidget(wL, 7 + i, 1)
            Cx.addWidget(QLabel("T (°C)"), 7 + i, 2)
            wT = make_dspin(-20.0, 250.0, 1.0, float(self.params.coax.temp5_seg_temp_C[i]), 1)
            Cx.addWidget(wT, 7 + i, 3)
            self.coax_seg_len.append(wL)
            self.coax_seg_temp.append(wT)

        # fallback uniform scaling (used only if 5-seg is disabled)
        Cx.addWidget(QLabel("Length (m)"), 12, 0)
        self.coax_len = make_dspin(0.0, 20000.0, 10.0, float(self.params.coax.length_m), 1)
        Cx.addWidget(self.coax_len, 12, 1)

        Cx.addWidget(QLabel("R factor"), 12, 2)
        self.coax_rfactor = make_dspin(0.5, 4.0, 0.05, float(self.params.coax.temp_factor_r), 2)
        Cx.addWidget(self.coax_rfactor, 12, 3)

        self.lbl_coax_info = QLabel("")
        self.lbl_coax_info.setWordWrap(True)
        self.lbl_coax_info.setStyleSheet("color:#444;")
        self.lbl_coax_info.setAlignment(Qt.AlignLeft | Qt.AlignTop)
        self.lbl_coax_info.setMinimumHeight(110)
        Cx.addWidget(self.lbl_coax_info, 13, 0, 1, 4)
        Cx.setRowStretch(13, 1)

        self.left_layout.addWidget(gb_coax)

        # V/f + inverter + field weakening
        gb_vf = QGroupBox("DOWNHOLE INVERTER")
        ...
        gb_sf = QGroupBox("SINE FILTER (BYPASS)")
        ...
        # Cable
        gb_cable = QGroupBox("MOTOR HARNESS")
        ...
        self.c_len_band = make_dspin(0.0, 20000.0, 100.0, 0.0, 0)
        ...
        # Presets (selection only; values are applied on change)
        ...
        # rows
        M.addWidget(QLabel("Motor preset"), 0, 0)
        ...
        self.x_core_cl = make_dspin(0.0, 50.0, 0.0005, self.params.extra.core_cL, 5)
        ...
        self.x_k_c = make_dspin(0.0, 100.0, 0.000001, self.params.extra.visc_k_couette, 8)
        self.x_k_tr = make_dspin(0.0, 100.0, 0.0005, self.params.extra.visc_k_transition, 5)
        self.x_n_tr = make_dspin(0.5, 3.0, 0.05, self.params.extra.visc_n_transition, 2)
        self.x_k_tb = make_dspin(0.0, 1.0, 0.0001, self.params.extra.visc_k_turb, 6)
        ...
        self.x_preset.addItems(["Milling Moog Curve", "Spear Moog Curve", "Annular Windings Curve", "User Defined"])
        ...
        Gg.addWidget(QLabel("Gearbox preset"), 0, 0)
    ...
    def _on_motor_preset_changed(self):
        ...
        # Auto-link: motor preset → Extra-Torque preset (ON)
        if hasattr(self, "x_preset"):
            m2x = {
                "Milling motor": "Milling Moog Curve",
                "Spear motor": "Spear Moog Curve",
                "Annular Motor - John": "Annular Windings Curve",
                "User defined": "User Defined",
            }
            target = m2x.get((name or "").strip(), None)
            if target:
                try:
                    self.x_preset.blockSignals(True)
                    self.x_preset.setCurrentText(target)
                finally:
                    self.x_preset.blockSignals(False)
                # Apply immediately
                self._apply_extra_torque_preset(target)



    def _on_extra_torque_preset_changed(self):
        """Handle Extra-Torque preset changes."""
        if getattr(self, "_bulk_updating", False):
            return
        name = self.x_preset.currentText() if hasattr(self, "x_preset") else ""
        self._apply_extra_torque_preset(name)
        # refresh computed results / plots
        try:
            self.update_all()
        except Exception:
            pass

    def _apply_extra_torque_user_defaults(self) -> None:
        """Restore the shipped code defaults for the Extra Torque block."""
        if not all(hasattr(self, a) for a in (
            "x_enable_extra", "x_enable_ktT", "x_enable_rsT", "x_wind_dT", "x_rs_tc",
            "x_temp", "x_tref", "x_kt_tc",
            "x_core_en", "x_core_cl", "x_core_exp",
            "x_visc_en", "x_visc_model", "x_k_c", "x_k_tr", "x_n_tr", "x_k_tb", "x_rpm1", "x_rpm2",
            "x_visc_T", "x_visc_a", "x_visc_beta", "x_smooth", "x_smooth_f",
        )):
            return

        ex = ExtraTorqueParams()  # class defaults represent "current defaults in the code"

        self.x_enable_extra.setChecked(bool(getattr(ex, "extra_enabled", True)))
        self.x_enable_ktT.setChecked(bool(getattr(ex, "kt_temp_enabled", True)))
        self.x_enable_rsT.setChecked(bool(getattr(ex, "rs_temp_enabled", False)))
        self.x_wind_dT.setValue(float(getattr(ex, "winding_rise_C", 10.0)))
        self.x_rs_tc.setValue(100.0 * float(getattr(ex, "rs_temp_coeff_per_C", 0.00393)))  # UI uses %/°C

        self.x_temp.setValue(float(getattr(ex, "temp_C", 175.0)))
        self.x_tref.setValue(float(getattr(ex, "temp_ref_C", 25.0)))
        self.x_kt_tc.setValue(100.0 * float(getattr(ex, "kt_temp_coeff_per_C", -0.0004)))  # UI uses %/°C

        self.x_core_en.setChecked(bool(getattr(ex, "core_enabled", True)))
        self.x_core_cl.setValue(float(getattr(ex, "core_cL", 0.0)))
        self.x_core_exp.setValue(float(getattr(ex, "core_exp", 0.5)))

        self.x_visc_en.setChecked(bool(getattr(ex, "visc_enabled", True)))
        self.x_visc_model.setCurrentText(str(getattr(ex, "visc_model", "Piecewise (Couette→Transition→Turbulent)")))
        self.x_k_c.setValue(float(getattr(ex, "visc_k_couette", 0.0)))
        self.x_k_tr.setValue(float(getattr(ex, "visc_k_transition", 0.0)))
        self.x_n_tr.setValue(float(getattr(ex, "visc_n_transition", 1.5)))
        self.x_k_tb.setValue(float(getattr(ex, "visc_k_turb", 0.0)))

        # breakpoints (UI uses aliases)
        self.x_rpm1.setValue(float(getattr(ex, "visc_rpm1", getattr(ex, "visc_rpm_1", 500.0))))
        self.x_rpm2.setValue(float(getattr(ex, "visc_rpm2", getattr(ex, "visc_rpm_2", 2000.0))))

        self.x_visc_T.setCurrentText(str(getattr(ex, "visc_temp_scaling", "None")))
        self.x_visc_a.setValue(100.0 * float(getattr(ex, "visc_lin_coeff_per_C", 0.0)))  # UI uses %/°C
        self.x_visc_beta.setValue(float(getattr(ex, "visc_beta_per_C", 0.0)))

        self.x_smooth.setChecked(bool(getattr(ex, "smooth_transitions", True)))
        self.x_smooth_f.setValue(float(getattr(ex, "smooth_frac", 0.15)))

    def _apply_extra_torque_preset(self, name: str) -> None:
        """Apply named Extra-Torque presets by updating the UI fields."""
        n = (name or "").strip()
        if not n:
            return

        # User-defined = shipped defaults
        if n.lower().startswith("user"):
            self._apply_extra_torque_user_defaults()
            return

        # Placeholder presets (Spear/Annular) are identical to Milling for now.
        self._apply_milling_motor_extra_torque()

        # For MOOG curve matching we keep viscous T scaling off and disable Kt(T) scaling (so Kt basis is consistent).
        if hasattr(self, "x_enable_ktT"):
            self.x_enable_ktT.setChecked(False)
        if hasattr(self, "x_visc_T"):
            self.x_visc_T.setCurrentText("None")
        if hasattr(self, "x_visc_a"):
            self.x_visc_a.setValue(0.0)
        if hasattr(self, "x_visc_beta"):
            self.x_visc_beta.setValue(0.0)
        if hasattr(self, "x_temp"):
            self.x_temp.setValue(175.0)
        if hasattr(self, "x_tref"):
            self.x_tref.setValue(25.0)
    def _apply_milling_motor_extra_torque(self) -> None:
        """Seed Extra Torque parameters from the Moog milling motor operating curve.

        Uses:
          - CLDF = 0.0049 lbf-in/(rpm)^0.5  -> core term C_L in Nm/(rad/s)^0.5
          - Viscous drag ~ 0.155 lbf-in at 11424.2 rpm (continuous table) -> Couette k in Nm/(rad/s)

        Mapping in this UI:
          tau_core = C_L * |omega|^0.5
          tau_visc = k_couette * |omega|

        where omega is motor mechanical speed in rad/s.
        """
        if not all(hasattr(self, a) for a in (
            'x_enable_extra','x_core_en','x_core_cl','x_core_exp',
            'x_visc_en','x_visc_model','x_k_c','x_k_tr','x_k_tb'
        )):
            return
        ...
        # viscous Couette coefficient (through-origin slope from max-speed point)
        visc_lbf_in = 0.155
        rpm_ref = 11424.2
    ...
    def _apply_motor_preset(self, name: str) -> None:
            """Apply named PMSM presets by updating the UI fields.
    
            Note: presets are expressed as per-phase (wye-equivalent) electrical parameters and use:
              - Ke in Vll_peak/krpm
              - Kt in lb-in/Arms
            """
            n = (name or "").strip()
            ...
            presets = {
                ...
                "User defined": {
                    "pole_pairs": MotorParams().pole_pairs,
                    "rs_ohm": MotorParams().rs_ohm,
                    "ld_h": MotorParams().ld_h,
                    "lq_h": MotorParams().lq_h,
                    "kt_lbin_arms": (MotorParams().kt_nm_per_arms / LBIN_TO_NM),
                    "ke_vll_peak_krpm": (MotorParams().ke_vll_rms_per_krpm * math.sqrt(2.0)),
                },
            ...
            try:
                ...
                # Milling motor: apply viscous drag curve (MOOG thermal operating curve)
                # Curve is near-linear: τ_visc ≈ k·|ω| with τ≈0.155 lbf-in @ 11424.2 rpm (T_oil≈175°C).
                if n == "Milling motor":
                    k_couette = 1.4638528253760041e-05  # Nm/(rad/s)
                    self.x_enable_extra.setChecked(True)
                    self.x_temp.setValue(175.0)
                    self.x_visc_en.setChecked(True)
                    self.x_visc_model.setCurrentText("Couette (τ=k·ω)")
                    ...
                    self.x_k_tr.setValue(0.0)
                    self.x_n_tr.setValue(1.5)
                    self.x_k_tb.setValue(0.0)
                    self.x_visc_T.setCurrentText("None")
                    self.x_visc_a.setValue(0.0)
                    self.x_visc_beta.setValue(0.0)
    
                    self.x_core_en.setChecked(False)
                    self.x_core_cl.setValue(0.0)
            finally:
                self._bulk_updating = False
    
            if self.chk_auto.isChecked() and (not self._bulk_updating):
                self.update_all()
    
    def _apply_gearbox_preset(self, name: str) -> None:
        ...
        presets = {
            ...
            "User defined": {
                "stage1": GearboxParams().stage1,
                "stage2": GearboxParams().stage2,
                "stage3": GearboxParams().stage3,
                "override_eta": bool(GearboxParams().override_total_eta),
                "eta_total": float(GearboxParams().eta_total_override),
            },
        ...
        try:
            ...
            self.chk_eta_override.setChecked(bool(d.get("override_eta", True)))
            self.eta_override.setValue(float(d.get("eta_total", self.eta_override.value())))
        finally:
            self._bulk_updating = False
    ...
    def _connect_signals(self):
        self.g_preset.currentTextChanged.connect(self._apply_gearbox_preset)

        self.m_preset.currentTextChanged.connect(self._on_motor_preset_changed)
        if hasattr(self, "x_preset"):
        ...
        # v30: add UPM/COAX widgets to the global change-hook list
        if hasattr(self, "upm_enable"):
            widgets += [
                self.upm_enable, self.upm_use_as_vdc, self.upm_vmod, self.upm_imod,
                self.upm_n_series, self.upm_n_parallel, self.upm_vset, self.upm_rint, self.upm_eta
            ]
        if hasattr(self, "coax_enable"):
            widgets += [
                self.coax_enable, self.coax_enforce_ilim, self.coax_return, self.coax_ilim,
                self.coax_lloop_uhpm, self.coax_rcenter_okm, self.coax_rserve_okm, self.coax_rarmor_okm,
                self.coax_tref, self.coax_alpha, self.coax_temp_model_5seg, self.coax_len, self.coax_rfactor
            ]
            widgets += list(getattr(self, "coax_seg_len", [])) + list(getattr(self, "coax_seg_temp", []))
    ...
    def _sync_params_from_ui(self):
        ...
        # v30: UPM + COAX blocks (UPM → COAX → Surface Inverter)
        if hasattr(p, "upm") and hasattr(self, "upm_enable"):
            p.upm.enabled = bool(self.upm_enable.isChecked())
            p.upm.use_as_vdc_source = bool(self.upm_use_as_vdc.isChecked())
            # UX: if the user asks to override the inverter Vdc from UPM, implicitly enable the UPM block.
            # This avoids the confusing state where override is checked but "Enable UPM" is unchecked.
            if p.upm.use_as_vdc_source and (not p.upm.enabled):
                p.upm.enabled = True
                self.upm_enable.blockSignals(True)
                self.upm_enable.setChecked(True)
                self.upm_enable.blockSignals(False)
            p.upm.v_module_v = float(self.upm_vmod.value())
            p.upm.i_module_a = float(self.upm_imod.value())
            p.upm.n_series = int(self.upm_n_series.value())
            p.upm.n_parallel = int(self.upm_n_parallel.value())
            p.upm.v_set_v = float(self.upm_vset.value())
            p.upm.r_internal_ohm = float(self.upm_rint.value())
            p.upm.eta = float(self.upm_eta.value())
            if bool(p.upm.enforce_stack_limits):
                p.upm.v_set_v = min(p.upm.v_set_v, p.upm.v_module_v)

        if hasattr(p, "coax") and hasattr(self, "coax_enable"):
            p.coax.enabled = bool(self.coax_enable.isChecked())
            p.coax.enforce_i_limit = bool(self.coax_enforce_ilim.isChecked())
            p.coax.wiring_sel = str(self.coax_return.currentText())
            p.coax.wiring_a = str(getattr(self, 'coax_wiring_a', self.coax_return).currentText())
            p.coax.wiring_b = str(getattr(self, 'coax_wiring_b', self.coax_return).currentText())
            # legacy field kept for compatibility
            p.coax.return_path = str(p.coax.wiring_sel)
            p.coax.i_limit_a = float(self.coax_ilim.value())
            p.coax.l_loop_h_per_m = float(self.coax_lloop_uhpm.value()) * 1e-6

            # Resistances are entered as Ω/km (datasheet style)
            p.coax.r_center_ohm_per_m = float(self.coax_rcenter_okm.value()) / 1000.0
            p.coax.r_serve_ohm_per_m = float(self.coax_rserve_okm.value()) / 1000.0
            p.coax.r_armor_ohm_per_m = float(self.coax_rarmor_okm.value()) / 1000.0

            p.coax.temp_ref_C = float(self.coax_tref.value())
            p.coax.temp_alpha_per_C = float(self.coax_alpha.value())
            p.coax.temp_model_5seg = bool(self.coax_temp_model_5seg.isChecked())
            p.coax.temp5_seg_len_m = [float(w.value()) for w in self.coax_seg_len]
            p.coax.temp5_seg_temp_C = [float(w.value()) for w in self.coax_seg_temp]

            p.coax.length_m = float(self.coax_len.value())
            p.coax.temp_factor_r = float(self.coax_rfactor.value())

        # Per user instruction: restrict n_series=1 in COAX mode (disable series stacking).
        if hasattr(p, 'coax') and hasattr(p, 'upm') and bool(getattr(p.coax, 'enabled', False)):
            p.upm.n_series = 1
            if hasattr(self, 'upm_n_series'):
                self.upm_n_series.blockSignals(True)
                self.upm_n_series.setMaximum(1)
                self.upm_n_series.setValue(1)
                self.upm_n_series.blockSignals(False)
        else:
            # restore the UI range for non-COAX architectures
            if hasattr(self, 'upm_n_series'):
                self.upm_n_series.blockSignals(True)
                self.upm_n_series.setMaximum(3)
                self.upm_n_series.blockSignals(False)
        # v30: If requested, drive the Surface Inverter DC-link (Vdc) directly from the UPM stack.
        # This is intentionally minimal/non-invasive: it simply overrides the v22 Vdc knob value
        # (and visually locks the knob) so the rest of v22 behaves exactly as-is.
        upm_override_vdc = False
        vdc_override = None
        if hasattr(p, "upm") and bool(getattr(p.upm, "enabled", False)) and bool(getattr(p.upm, "use_as_vdc_source", False)):
            coax_mode = bool(getattr(p, "coax", CoaxParams()).enabled)
            try:
                vdc_override = float(p.upm.stack_vdc(coax_mode=coax_mode))
                upm_override_vdc = True
            except Exception:
                # If anything goes wrong, fall back to user-entered Vdc.
                upm_override_vdc = False
                vdc_override = None

        # UI effect: when overridden, set + lock the Surface Inverter Vdc knob.
        if hasattr(self, "vdc_link"):
            if upm_override_vdc and (vdc_override is not None):
                self.vdc_link.blockSignals(True)
                self.vdc_link.setValue(vdc_override)
                self.vdc_link.blockSignals(False)
                self.vdc_link.setEnabled(False)
            else:
                self.vdc_link.setEnabled(True)

        # vf
        p.vf.v_limit_type = self.v_limit_type.currentText()
        p.vf.voltage_entry_basis = self.v_entry_basis.currentText()
        # If the UPM override is active, the spinbox was already updated above.
        ...
        # COAX-Mono architecture: 'cable' block is a short downhole harness between inverter and PMSM.
        # Treat harness electrical drop as negligible, but retain phase current rating as inverter limit.
        try:
            coax_mode = bool(getattr(getattr(p, "coax", None), "enabled", False))
        except Exception:
            coax_mode = False
        if coax_mode:
            # Force harness assumptions (no 1-wire/2-wire concept here)
            p.cable.drop_negligible = True
            p.cable.wires_per_phase = 1
            p.cable.i_limit_basis = "Per phase"
            # Disable temperature and segment modeling (not meaningful for short harness)
            p.cable.temp_model_5seg = False
            p.cable.i_limit_derate_with_temp = False
            # Best-effort: lock UI controls if present
            try:
                if hasattr(self, "c_wires"):
                    self.c_wires.blockSignals(True)
                    self.c_wires.setCurrentIndex(0)
                    self.c_wires.blockSignals(False)
                    self.c_wires.setEnabled(False)
                if hasattr(self, "c_ilim_basis"):
                    # ensure the combo shows "Per phase" if it exists
                    for i in range(self.c_ilim_basis.count()):
                        if str(self.c_ilim_basis.itemText(i)).strip().lower().startswith("per phase"):
                            self.c_ilim_basis.blockSignals(True)
                            self.c_ilim_basis.setCurrentIndex(i)
                            self.c_ilim_basis.blockSignals(False)
                            break
                    self.c_ilim_basis.setEnabled(False)
                # disable harness drop knobs (kept visible but non-applicable)
                for wname in (
                "c_len", "c_rpm", "c_lpm", "c_temp", "c_temp_ref", "c_temp_alpha", "c_lpar", "c_temp_5seg",
                "c_temp_derate"):
                    if hasattr(self, wname):
                        getattr(self, wname).setEnabled(False)
                if hasattr(self, "c_5seg_L"):
                    for w in self.c_5seg_L: w.setEnabled(False)
                if hasattr(self, "c_5seg_T"):
                    for w in self.c_5seg_T: w.setEnabled(False)
            except Exception:
                pass
        else:
            # non-COAX: allow normal heptacable-style settings
            try:
                if hasattr(self, "c_wires"): self.c_wires.setEnabled(True)
                if hasattr(self, "c_ilim_basis"): self.c_ilim_basis.setEnabled(True)
                for wname in (
                "c_len", "c_rpm", "c_lpm", "c_temp", "c_temp_ref", "c_temp_alpha", "c_lpar", "c_temp_5seg",
                "c_temp_derate"):
                    if hasattr(self, wname):
                        getattr(self, wname).setEnabled(True)
            except Exception:
                pass
    ...
    def update_all(self):
        try:
            ...
            # v30: derived label updates + UI enforcement for UPM/COAX
            if hasattr(self, "lbl_upm_stack") and hasattr(self.params, "upm"):
                upm = self.params.upm
                coax_mode = bool(getattr(self.params, "coax", CoaxParams()).enabled)
                ns = 1 if coax_mode else max(1, int(upm.n_series))
                npar = max(1, int(upm.n_parallel))
                v_stack = max(0.0, float(upm.v_set_v)) * ns
                i_stack = max(0.0, float(upm.i_module_a)) * npar
                self.lbl_upm_stack.setText(f"Stack: Vdc≈{v_stack:.0f} V, Imax≈{i_stack:.2f} A")
                if hasattr(self, "upm_n_series"):
                    self.upm_n_series.setEnabled(not coax_mode)

            if hasattr(self, "lbl_coax_info") and hasattr(self.params, "coax"):
                cx = self.params.coax
                r_loop = float(cx.loop_resistance_ohm())
                l_loop = float(cx.loop_inductance_h())
                L_total = float(sum(cx.temp5_seg_len_m)) if bool(cx.temp_model_5seg) else float(cx.length_m)
                self.lbl_coax_info.setText(f"Loop: L≈{L_total:.0f} m, R≈{r_loop:.2f} Ω, L≈{l_loop*1e3:.3f} mH, I_limit≈{float(cx.i_limit_a):.2f} A")

                seg_en = bool(cx.temp_model_5seg)
                for w in getattr(self, "coax_seg_len", []):
                    w.setEnabled(seg_en)
                for w in getattr(self, "coax_seg_temp", []):
                    w.setEnabled(seg_en)
                if hasattr(self, "coax_len"):
                    self.coax_len.setEnabled(not seg_en)
                if hasattr(self, "coax_rfactor"):
                    self.coax_rfactor.setEnabled(not seg_en)
        ...
        if self.params.vf.voltage_entry_basis.strip().upper().startswith('DC'):
            vdc_set = float(self.params.vf.vdc_link_v)
            mod = (self.params.vf.modulation or '').strip().upper()
            util = float(self.params.vf.v_util)
            util = min(1.0, max(0.0, util))

            # If UPM/COAX DC-link model is active, show the *effective* inverter DC bus estimate too.
            vdc_eff = getattr(self, '_last_vdc_at_inv', None)
            vdc_src = getattr(self, '_last_vdc_source', None)
            use_eff = (vdc_eff is not None) and (self.params.coax.enabled or self.params.upm.enabled)

            def _ac_limits_from_vdc(vdc_val: float):
                if mod.startswith('SV'):
                    vph = util * float(vdc_val) / (6.0 ** 0.5)
                else:
                    vph = util * float(vdc_val) / (2.0 * (2.0 ** 0.5))
                vll = (3.0 ** 0.5) * vph
                return vph, vll

            if use_eff:
                vph_eff, vll_eff = _ac_limits_from_vdc(float(vdc_eff))
                src_txt = f"Vdc_set≈{vdc_set:.0f} V"
                if vdc_src is not None:
                    src_txt = f"Vsource≈{float(vdc_src):.0f} V"
                self.lbl_vlimits.setText(
                    f"Derived AC limit from Vdc@inv (drooped): VLL,rms,max = {vll_eff:.1f}  |  Vϕ,rms,max = {vph_eff:.1f}  "
                    f"(Vdc@inv≈{float(vdc_eff):.0f} V, {src_txt}, {self.params.vf.modulation}, util={util:.2f})"
                )
            else:
                # Default behavior (no DC-link droop model)
                vph, vll = self.params.vf.ac_limits_from_vdc()
                self.lbl_vlimits.setText(
                    f"Derived AC limit from Vdc: VLL,rms,max = {vll:.1f}  |  Vϕ,rms,max = {vph:.1f}  (Vdc={vdc_set:.0f} V, {self.params.vf.modulation}, util={util:.2f})"
                )
        ...
        # v30: interface readouts (UPM → COAX → Inverter → Motor) at the target point
        try:
            self._update_interface_readouts(res_sel)
        except Exception:
            # Never block updates due to UI-only diagnostics
            pass
        ...
        # COAX-Mono replaced: HVDC Heptacable (downhole inverter DC-link) — compare two HVDC wirings (A/B).
        coax_mode = bool(getattr(getattr(self.params, "coax", None), "enabled", False))
        if coax_mode:
            import copy as _copy

            # Use a single CableParams object (compatibility). The HVDC split is modeled inside CoaxParams.
            cab1 = CableParams(**vars(self.params.cable))
            cab1.drop_negligible = True
            cab1.wires_per_phase = 1
            cab1.i_limit_basis = "Per phase"
            cab2 = CableParams(**vars(cab1))

            # Main solve uses the selected HVDC wiring (wiring_sel)
            res_sel = self.model.solve_target(cable_override=cab1)

            def _solve_env_for_wiring(wkey: str):
                p2 = _copy.deepcopy(self.params)
                try:
                    p2.coax.wiring_sel = str(wkey)
                except Exception:
                    pass
                m2 = SystemModel(p2)
                r = m2.solve_target(cable_override=cab1)
                out_rpm_max = float(self.env_out_rpm_max.value())
                env = m2.compute_envelope(out_rpm_max=out_rpm_max, n=200, cable_override=cab1)
                return r, env

            wA = str(getattr(self.params.coax, "wiring_a", getattr(self.params.coax, "wiring_sel", "C+1H|3H")))
            wB = str(getattr(self.params.coax, "wiring_b", "C+2H|2H"))

            res_1, (env_rpm_1, env_tq_1, env_iq_1, env_loss_1) = _solve_env_for_wiring(wA)
            res_2, (env_rpm_2, env_tq_2, env_iq_2, env_loss_2) = _solve_env_for_wiring(wB)
        else:

            # Legacy behavior (non-COAX): compare 1-wire vs 2-wire
            cab1 = CableParams(**vars(self.params.cable))
            cab1.wires_per_phase = 1
            ...
            out_rpm_max = float(self.env_out_rpm_max.value())
            env_rpm_1, env_tq_1, env_iq_1, env_loss_1 = self.model.compute_envelope(
                out_rpm_max=out_rpm_max, n=200, cable_override=cab1
            )
            env_rpm_2, env_tq_2, env_iq_2, env_loss_2 = self.model.compute_envelope(
                out_rpm_max=out_rpm_max, n=200, cable_override=cab2
            )

        # Cache latest solve results for tab-change rendering (Tables / Presets).
        self._last_res_sel = res_sel
        self._last_res_1 = res_1
        self._last_res_2 = res_2
        ...
        # VLL along envelope for utilization plots (downhole line-line RMS at motor terminals)
        # We compute VLL from the motor phase voltage at the max-torque operating point for each RPM.
        def _env_vll(env_rpm: np.ndarray, cable: CableParams) -> np.ndarray:
            vll = np.zeros_like(env_rpm, dtype=float)
            G_local = float(self.params.gearbox.ratio())
            for i, out_rpm in enumerate(env_rpm):
                motor_rpm = float(out_rpm) * G_local
                iq_max, id_best, _, _, _, _, _ = self.model.max_iq_given_limits(motor_rpm, cable)
                vph_motor = self.model.motor_voltage_required_phase_rms(motor_rpm, float(id_best), float(iq_max))
                vll[i] = math.sqrt(3.0) * float(vph_motor)
            return vll

        vll_1 = _env_vll(env_rpm_1, cab1)
        vll_2 = _env_vll(env_rpm_2, cab2)

        # Cache latest envelope + cable configs for on-demand panes (Motor / Tables / Presets)
        self._last_env_1 = (env_rpm_1, env_tq_1, env_iq_1, env_loss_1)
        self._last_env_2 = (env_rpm_2, env_tq_2, env_iq_2, env_loss_2)
        self._last_vll_1 = vll_1
        self._last_vll_2 = vll_2
        self._last_cab1 = cab1
        self._last_cab2 = cab2
        self._last_coax_mode = coax_mode

        self._plot_band_torque(env_rpm_1, env_tq_1, env_rpm_2, env_tq_2, vll_1, vll_2)

        # Motor tab (compute only when visible)
        try:
            if hasattr(self, "motor_pane") and (self.tabs.currentWidget() == self.motor_pane):
                self._plot_motor_tab(res_sel, res_1, res_2)
        except Exception:
            pass

        # Moog Curves tab (compute only when visible)
        try:
            if hasattr(self, "moog_pane") and (self.tabs.currentWidget() == self.moog_pane):
                self._plot_moog_curves_tab()
        except Exception:
            pass

        

        # Tables tab (compute only when visible)
        try:
            if hasattr(self, "tables_pane") and (self.tabs.currentWidget() == self.tables_pane):
                self._plot_decision_tables(res_sel, res_1, res_2)
        except Exception:
            pass
    ...
    # ------------------------------------------------------------------
    # v30: Interface readouts (UPM → COAX → Inverter → Motor)
    # ------------------------------------------------------------------
    def _update_interface_readouts(self, res: SolveResult) -> None:
        """Populate UI labels with approximate interface voltages/currents at the target point.

        This is intentionally *diagnostic only* (no changes to the v22 solver). Values are
        computed from the current UI settings and the most recent target-point solution.
        """
        p = self.params
        ...
        # --- Basic electrical outputs from the solved target point (v22 outputs) ---
        vph_inv_req = float(getattr(res, "v_inverter_req", 0.0))
        iph_inv_rms = float(getattr(res, "i_inverter_rms", 0.0))
        vph_cmd = float(getattr(res, "v_surface_cmd", 0.0))
        vph_lim = float(getattr(res, "v_surface_limit", 0.0))
        vph_mtr_req = float(getattr(res, "v_motor_req", 0.0))
        vph_cab_drop = float(getattr(res, "v_cable_drop", 0.0))
        i_mag_used = float(getattr(res, "i_mag_used_rms", 0.0))
        iq_used = float(getattr(res, "iq_req_rms", 0.0))
        id_used = float(getattr(res, "id_used_rms", 0.0))

        # --- DC link estimate (only meaningful when using Vdc entry) ---
        basis_is_vdc = str(p.vf.voltage_entry_basis).strip().upper().startswith("DC")

        # Source-side stack parameters
        ns = 1
        npar = 1
        eta_upm = 1.0
        vdc_stack_set = float(p.vf.vdc_link_v)
        vdc_stack_max = float(p.vf.vdc_link_v)
        r_src = 0.0
        i_lim_upm = float("inf")

        if upm_en:
            ns = 1 if coax_mode else max(1, int(getattr(upm, "n_series", 1)))
            npar = max(1, int(getattr(upm, "n_parallel", 1)))
            eta_upm = max(1e-3, float(getattr(upm, "eta", 1.0)))
            # Totals
            vdc_stack_set = float(getattr(upm, "v_set_v", 0.0)) * ns
            vdc_stack_max = float(getattr(upm, "v_module_v", 0.0)) * ns
            r_src = float(upm.stack_r_internal(coax_mode=coax_mode))
            i_lim_upm = float(upm.stack_imax())
        ...
        if coax_en:
            wkey = str(getattr(coax, 'wiring_sel', 'C+1H|3H'))
            r_coax = float(coax.loop_resistance_ohm(wkey))
            return_desc = str(getattr(coax, "return_path", "Serve only"))
            l_total_m = float(sum(getattr(coax, "temp5_seg_len_m", [0, 0, 0, 0, 0]))) if bool(
                getattr(coax, "temp_model_5seg", False)) else float(getattr(coax, "length_m", 0.0))
            if bool(getattr(coax, "enforce_i_limit", True)):
                i_lim_coax = float(getattr(coax, "i_limit_a", float("inf")))

        i_lim_dc = min(i_lim_upm, i_lim_coax)

        # Choose a "DC source" readout value.
        # If UPM is enabled and configured as source, show that; otherwise show the inverter Vdc knob.
        vdc_source = float(p.vf.vdc_link_v)
        if upm_en and bool(getattr(upm, "use_as_vdc_source", False)):
            vdc_source = float(upm.stack_vdc(coax_mode=coax_mode))
        elif upm_en:
            # Even if not used as source, it's helpful to show what the UPM setpoint would be.
            vdc_source = float(p.vf.vdc_link_v)

        # Cache for other UI elements (e.g., Surface Inverter label)
        self._last_vdc_source = float(vdc_source)

        # Estimate DC bus current from solved AC requirement (conservative: VA-based)
        idc_est = 0.0
        vdc_at_inv = vdc_source
        vdrop_src = 0.0
        vdrop_coax = 0.0
        ploss_src = 0.0
        ploss_coax = 0.0
        ...
        if basis_is_vdc and (upm_en or coax_en):
            s_va = max(0.0, 3.0 * vph_inv_req * iph_inv_rms)
            r_tot = float(r_src + r_coax)
            # 2-pass droop estimate
            vdc_guess = max(1e-6, float(vdc_source))
            for _ in range(2):
                idc_est = (s_va / eta_upm) / max(1e-6, vdc_guess)
                vdc_at_inv = max(0.0, float(vdc_source) - float(idc_est) * r_tot)
                vdc_guess = max(1e-6, vdc_at_inv)

            vdrop_src = float(idc_est) * float(r_src)
            vdrop_coax = float(idc_est) * float(r_coax)
            ploss_src = (float(idc_est) ** 2) * float(r_src)
            ploss_coax = (float(idc_est) ** 2) * float(r_coax)

            # Derived AC limits *at the inverter* using the drooped Vdc estimate
            util = float(p.vf.v_util)
            util = min(1.0, max(0.0, util))
            mod = (p.vf.modulation or "").strip().upper()
            if mod.startswith("SV"):
                vph_lim_if_inv = util * float(vdc_at_inv) / math.sqrt(6.0)
            else:
                vph_lim_if_inv = util * float(vdc_at_inv) / (2.0 * math.sqrt(2.0))
            vll_lim_if_inv = math.sqrt(3.0) * float(vph_lim_if_inv)

        # Cache effective inverter DC bus estimate
        self._last_vdc_at_inv = float(vdc_at_inv)

        # --- Update UI labels ---
        if hasattr(self, "lbl_upm_stack"):
            if upm_en:
                txt = (
                    f"UPM stack: {ns}S×{npar}P | Vset={vdc_stack_set:.0f} V (Vmax={vdc_stack_max:.0f} V) | "
                    f"Imax={i_lim_upm:.2f} A | η={eta_upm:.2f}\n"
                    f"Source Rint≈{r_src:.3f} Ω  |  Enforced DC I-limit≈{(i_lim_dc if math.isfinite(i_lim_dc) else float('inf')):.2f} A"
                )
            else:
                txt = "UPM: disabled"
            self.lbl_upm_stack.setText(txt)

        if hasattr(self, "lbl_coax_info"):
            lines: List[str] = []
            if coax_en:
                lines.append(
                    f"HVDC: wiring='{return_desc}' | L≈{l_total_m:.0f} m | Rloop≈{r_coax:.2f} Ω | I_limit≈{float(getattr(coax, 'i_limit_a', 0.0)):.2f} A"
                )
            else:
                lines.append("COAX: disabled")

            # DC link diagnostics
            if basis_is_vdc and (upm_en or coax_en):
                lim_txt = f"{i_lim_dc:.2f} A" if math.isfinite(i_lim_dc) else "—"
                lines.append(
                    f"DC link (target est.): Vsource≈{vdc_source:.0f} V | Idc_est≈{idc_est:.2f} A (limit {lim_txt}) | Vdc@inv≈{vdc_at_inv:.0f} V"
                )
                if (upm_en or coax_en) and (r_src + r_coax) > 0.0:
                    lines.append(
                        f"Vdrop: src≈{vdrop_src:.1f} V + coax≈{vdrop_coax:.1f} V  |  Ploss: src≈{ploss_src:.0f} W + coax≈{ploss_coax:.0f} W"
                    )
                if vph_lim_if_inv is not None and vll_lim_if_inv is not None:
                    lines.append(
                        f"At Vdc@inv: VLL,rms,max≈{vll_lim_if_inv:.1f}  |  Vϕ,rms,max≈{vph_lim_if_inv:.1f}  ({p.vf.modulation}, util={p.vf.v_util:.2f})"
                    )
            else:
                if (upm_en or coax_en) and (not basis_is_vdc):
                    lines.append("DC link diagnostics are shown only when Voltage entry basis = 'DC link (Vdc)'.")

            # AC / motor-side interfaces (always meaningful)
            vll_motor = math.sqrt(3.0) * vph_mtr_req
            vll_inv_req = math.sqrt(3.0) * vph_inv_req
            lines.append(
                f"Inverter→Motor (target): VLL_req@inv≈{vll_inv_req:.1f} (Vϕ_req≈{vph_inv_req:.1f}) | Iinv_rms≈{iph_inv_rms:.2f} A"
            )
            lines.append(
                f"Motor terminals: VLL_req≈{vll_motor:.1f} (Vϕ_req≈{vph_mtr_req:.1f}) | Cable drop≈{math.sqrt(3.0)*vph_cab_drop:.1f} VLL | Iphase≈{i_mag_used:.2f} A (Iq≈{iq_used:.2f}, Id≈{id_used:.2f})"
            )
            # Downhole limits
            vdh_ph = getattr(res, "v_downhole_phase_limit", None)
            vdh_ll = getattr(res, "v_downhole_ll_limit", None)
            if vdh_ph is not None or vdh_ll is not None:
                ph_txt = f"{float(vdh_ph):.1f} Vϕ" if vdh_ph is not None else "—"
                ll_txt = f"{float(vdh_ll):.1f} VLL" if vdh_ll is not None else "—"
                lines.append(f"Downhole V limits: {ll_txt} | {ph_txt}")

            # Surface-commanded fundamental (for context)
            lines.append(f"Surface V/f cmd: Vϕ_cmd≈{vph_cmd:.1f} (limit≈{vph_lim:.1f})")

            self.lbl_coax_info.setText("\n".join(lines))
    ...
    def _on_main_tab_changed(self, idx: int):
        ...
        Fixes: Stonehouse Presets pane was blank unless the user hit 'Rerun',
        because it only renders inside update_all() when the tab is already active.
        """
        try:
        ...
        # Render Motor pane on-demand when it becomes visible.
        if hasattr(self, "motor_pane") and (w == self.motor_pane):
            try:
                self._plot_motor_tab()
            except Exception:
                pass


        
        # Render Moog Curves pane on-demand when it becomes visible.
...
# Render Tables pane on-demand when it becomes visible.
        if hasattr(self, "tables_pane") and (w == self.tables_pane):
            try:
                self._plot_decision_tables()
            except Exception:
                pass
    ...
    def _state_widgets_for_defaults(self):
        ...
        if hasattr(self, "coax_enable"):
            ...
            w += list(getattr(self, "coax_seg_len", [])) + list(getattr(self, "coax_seg_temp", []))
    ...
    def _update_status(self, res_sel: SolveResult, res_1: SolveResult, res_2: SolveResult):
        ...
        lines = []
        lines.append(
            "✅ PASS — target point feasible" if res.feasible else "❌ FAIL — target violates one or more constraints")
        lines.append("")
        ...
        if bool(getattr(p, "stuck_mode", False)):
            lines.append(
                f"Mode: STUCK/STALL (RPM forced 0). Stall torque requirement: {p.target.out_torque_ftlbf:.0f} ft-lbf")
        else:
            lines.append(
                f"Mode: CONTINUOUS ROTATION. Stall torque requirement (for stuck): {p.target.out_torque_ftlbf:.0f} ft-lbf")
            if p.bha.enabled:
                lines.append(
    ...
    def _plot_envelope(self, res: SolveResult, env1, env2):
        ...
        tq1_mot = tq1_sym * s1
        tq2_mot = tq2_sym * s2
        if bool(getattr(getattr(self.params, "coax", None), "enabled", False)):
            wA = str(getattr(self.params.coax, "wiring_a", getattr(self.params.coax, "wiring_sel", "C+1H|3H")))
            wB = str(getattr(self.params.coax, "wiring_b", "C+2H|2H"))
            ax1.plot(rpm1_sym, tq1_mot, linewidth=2, label=f'HVDC A ({wA})')
            ax1.plot(rpm2_sym, tq2_mot, linewidth=2, linestyle='--', label=f'HVDC B ({wB})')
        else:
        ...
        if braking_ok:
            ...
            try:
                cab1.drop_negligible = bool(getattr(getattr(self.params, "coax", None), "enabled", False))         or bool(getattr(cab1, "drop_negligible", False))
            except Exception:
                pass
            cab1.wires_per_phase = 1
            cab2 = _copy.deepcopy(cab1)
            if not coax_mode:
                cab2.wires_per_phase = 2
            ...
            if bool(getattr(getattr(self.params, "coax", None), "enabled", False)):
                ax1.plot(rpm1_sym, tq1_regen, linewidth=1.5, linestyle=':', label='COAX mono (regen cap)')
            else:
        ...
        ymax = self._compute_quad_ylim_ftlbf(
            arrays=[tq1a, tq2a],
            extras=[tau_ccrs_signed, tau_drive_signed, tau_brake_signed, tau_load_signed],
            min_abs=200.0,
            pad=0.12,
        )
    ...
    def _compute_quad_ylim_ftlbf(self,
                                 arrays,
                                 extras=(),
                                 min_abs: float = 200.0,
                                 pad: float = 0.12) -> float:
        """Compute a *nice* symmetric +/- y-limit (ft-lbf) for 4-quadrant torque plots."""
        import math
        max_abs = 0.0
        ...
        if not math.isfinite(max_abs) or max_abs < 1e-9:
            max_abs = float(min_abs)
        max_abs = max(max_abs, float(min_abs))

        lim = max_abs * (1.0 + float(pad))
        ...
        lim_nice = float(math.ceil(lim / step) * step)
        return lim_nice
    ...
    def _plot_band_torque(self,
        ...
        def _plot_motoring_and_bands(ax, rpm_mag: np.ndarray, tq_mag: np.ndarray, cable_override: CableParams,
            ...
            # --- Regen/braking cap (QII and QIV)
            if regen_ok:
                c_use = cable_override if (cable_override is not None) else p.cable
                regen_cap = np.array([self.model.regen_cap_output_torque_ftlbf(r, c_use)
                                      for r in rpm_mag], float)
                # avoid silly huge values (regen cap should not exceed current-limited torque)
                regen_cap = np.minimum(regen_cap, tq_mag)

                # QIV: +rpm, -tq
                ax.plot(rpm_mag, -regen_cap, ls=':',
                        label=f'{label_prefix} regen cap' if show_legend else None)
                # QII: -rpm, +tq
                ax.plot(rpm_neg, regen_cap[::-1], ls=':')
        ...
        show_leg1 = True
        _plot_motoring_and_bands(self.band_pane.ax1, env_rpm_1, env_tq_1, cable_1, 'COAX mono' if bool(getattr(getattr(p, 'coax', None), 'enabled', False)) else '1 wire/phase', show_leg1)
        self.band_pane.ax1.set_title('Torque band (COAX mono, 4Q view)')
        self.band_pane.ax1.set_ylabel('Output Torque (ft-lbf)')
        ymax_4q = self._compute_quad_ylim_ftlbf(
            arrays=[env_tq_1, env_tq_2],
            extras=[float(getattr(p.target, 'out_torque_ftlbf', 0.0))],
            min_abs=200.0,
            pad=0.12,
        )
        self.band_pane.ax1.set_ylim(-ymax_4q, ymax_4q)

        show_leg2 = True
        if bool(getattr(getattr(p, 'coax', None), 'enabled', False)):
            self.band_pane.ax2.text(0.5, 0.5, 'N/A for COAX-Mono\n(1-wire/2-wire not applicable)', ha='center', va='center', transform=self.band_pane.ax2.transAxes)
            self.band_pane.ax2.set_axis_off()
        else:
            _plot_motoring_and_bands(self.band_pane.ax2, env_rpm_2, env_tq_2, cable_2, '2 wires/phase', show_leg2)
        if not bool(getattr(getattr(p, 'coax', None), 'enabled', False)):
        ...
        self.band_pane.ax2.set_ylim(-ymax_4q, ymax_4q)
    ...
    def _plot_stonehouse_presets(self):
        ...
        def _plot_4q_band(ax,
            ...
            if regen_ok:
                regen_cap = np.array([model_for_regen.regen_cap_output_torque_ftlbf(r, cable_for_regen)
                                      for r in rpm_mag], float)
                regen_cap = np.minimum(regen_cap, tq_nom)
                ax.plot(rpm_mag, -regen_cap, ls=":", lw=1.6, label="Regen cap (if applicable)" if show_legend else None)
                ax.plot(rpm_neg, regen_cap[::-1], ls=":", lw=1.6)

            ax.axhline(0.0, lw=1.0)
            ax.axvline(0.0, lw=1.0)
            ax.grid(True, alpha=0.25)
            ax.set_xlim(-rpm_axis, rpm_axis)
            # Use a shared, data-driven y-limit for all 4Q preset plots
            cand = [tq_lo, tq_nom, tq_hi]
            for _k in ("eta", "vll", "len"):
                _b = bands.get(_k, None)
                if isinstance(_b, dict) and ("lo" in _b) and ("hi" in _b):
                    try:
                        cand.append(np.asarray(_b["lo"], dtype=float))
                        cand.append(np.asarray(_b["hi"], dtype=float))
                    except Exception:
                        pass
            ymax_4q = self._compute_quad_ylim_ftlbf(
                arrays=cand,
                extras=[float(getattr(self.params.target, "out_torque_ftlbf", 0.0))],
                min_abs=200.0,
                pad=0.12,
            )
            ax.set_ylim(-ymax_4q, ymax_4q)
            ax.set_title(title)
            if ylabel:
    ...
    def _plot_architecture(self) -> None:
        ...
        # --- diagram blocks (left) ---
        # Interface values from the last solve_target() (if available)
        vdc_src = float(getattr(self, '_last_vdc_source', float(p.vf.vdc_link_v)))
        vdc_inv = float(getattr(self, '_last_vdc_at_inv', float(vdc_src)))
        upm_on = bool(hasattr(p, 'upm') and (getattr(p, 'upm', None) is not None) and bool(getattr(p.upm, 'enabled', False)))
        coax_on = bool(hasattr(p, 'coax') and (getattr(p, 'coax', None) is not None) and bool(getattr(p.coax, 'enabled', False)))
        try:
            rloop_coax = float(p.coax.loop_resistance_ohm()) if coax_on else 0.0
        except Exception:
            rloop_coax = 0.0
        blocks = [
            ("UPM\nDC Source",
             f"{p.upm.n_series}S{p.upm.n_parallel}P | Vmod={p.upm.v_set_v:.0f} V\nImax={p.upm.stack_imax():.2f} A" if upm_on else "disabled",
             upm_on),
            ("HEPTA HVDC\n(5-seg DC)",
             f"L={p.coax.length_m / 1000:.1f} km | {p.coax.return_path}\nRloop={rloop_coax:.0f} Ω | Imax={p.coax.i_limit_a:.2f} A" if coax_on else "disabled",
             coax_on),
            ("Downhole\nInverter",
             f"{p.vf.modulation}\nVdc_set(UI)={p.vf.vdc_link_v:.0f} V\nVsource={vdc_src:.0f} V | Vdc@inv={vdc_inv:.0f} V",
             True),
            ("Motor\nHarness",
             f"L={p.cable.length_m / 1000:.1f} km | {p.cable.wires_per_phase} wire/phase",
             True),
            ("Downhole\nV Limits",
             f"VLL: {'ON' if p.limits.enforce_downhole_vll_limit else 'OFF'} ({p.limits.downhole_vll_rms_limit:.0f} V)\nVph: {'ON' if p.limits.enforce_downhole_vphase_limit else 'OFF'} ({p.limits.downhole_v_phase_rms_limit:.0f} V)",
             bool(p.limits.enforce_downhole_vll_limit or p.limits.enforce_downhole_vphase_limit)),
            ("PMSM\nMotor",
             f"{p.motor.pole_pairs} pp | Kt={p.motor.kt_nm_per_arms:.2f} Nm/Arms",
             True),
            ("Magnetic\nCoupler",
             f"Tslip={nm_to_ft_lbf(p.mag_coupler.t_slip_nm):.0f} ft-lbf",
             mag_on),
            ("Gearbox",
             f"G={p.gearbox.ratio():.0f}:1 | η={p.gearbox.eff_total():.3f}\nParasitics: {'ON' if paras_on else 'OFF'}",
             True),
            ("CCRS\nOutput",
             f"Cmd: {cmd_dir} | {op_mode}\nRPMcmd={0.0 if stuck_on else p.target.out_rpm:.2f}",
             True),
            ("BHA Load\n(External)",
             f"TOB={p.bha.drilling_tob_ftlbf:.0f} ft-lbf\nFric Tc={p.bha.fric_tc_nm:.0f} Nm",
             bha_on),
        ]
        ...
        ax.text(0.02, 0.96, "Architecture", fontsize=15, weight="bold", color=title_color, va="top")
    ...
    def _plot_sweep_surface_inverter(self):
        ...
        cab = p0.cable
        basis_is_vdc = str(getattr(p0.vf, 'voltage_entry_basis', 'DC')).strip().upper().startswith('DC')
        use_upm_as_vdc = bool(basis_is_vdc and hasattr(p0, 'upm') and (getattr(p0, 'upm', None) is not None) and bool(getattr(p0.upm, 'enabled', False)) and bool(getattr(p0.upm, 'use_as_vdc_source', False)))
        ...
        for v in vlims:
            ...
            if p.vf.voltage_entry_basis.strip().upper().startswith('DC'):
                # In COAX/UPM mode the meaningful sweep variable is the UPM setpoint, not the inverter UI Vdc knob.
                if use_upm_as_vdc and hasattr(p, 'upm') and (getattr(p, 'upm', None) is not None):
                    p.upm.v_set_v = float(v)
                else:
        ...
        ax1.axhline(1000.0, linestyle='-.', linewidth=2, label='1000 peak')
        if basis_is_vdc:
            current_v = float(p0.upm.v_set_v) if (use_upm_as_vdc and hasattr(p0, 'upm') and (getattr(p0, 'upm', None) is not None)) else float(p0.vf.vdc_link_v)
        else:
            current_v = float(p0.vf.v_limit_value)
        ax1.axvline(current_v, linestyle='--', linewidth=1.0, label='Current voltage (entry basis)')
        ttl1 = 'Inverter sweep: Vdc source → torque capability @ 1 rpm' if (use_upm_as_vdc and basis_is_vdc) else 'Inverter sweep: Vlimit → torque capability @ 1 rpm'
        ax1.set_title(ttl1)
        ax1.set_xlabel('Vdc set (UPM per-module, V)' if (use_upm_as_vdc and basis_is_vdc) else 'Vlimit value (UI units)')
        ax1.set_ylabel('ft-lbf')
        ax1.grid(True, alpha=0.3)
        ax1.legend(fontsize=8, loc='best')
        ...
        ax2.set_title('Inverter sweep: Vdc source → Vcmd at operating point' if (use_upm_as_vdc and basis_is_vdc) else 'Inverter sweep: Vlimit → Vcmd at operating point')
        ax2.set_xlabel('Vdc set (UPM per-module, V)' if (use_upm_as_vdc and basis_is_vdc) else 'Vlimit value (UI units)')
        ax2.set_ylabel('Vrms (phase)')
        ax2.grid(True, alpha=0.3)
        ax2.legend(fontsize=8, loc='best')
    ...
    def _plot_sweep_dclink(self):
        """DC-link (UPM + COAX) sweep across the RPM envelope using MAX achievable torque.

        This tab is primarily for interface visibility:
          - Vsource (UPM setpoint / UI Vdc)
          - Vdc@inv (after COAX drop)
          - Idc (DC-link current)
          - COAX Vdrop and DC loss (I^2 R)
        """
        p0 = self.params
        fast = bool(self.chk_fast_sweeps.isChecked())

        ax1, ax2, ax3, ax4 = self.sweep_dclink.ax1, self.sweep_dclink.ax2, self.sweep_dclink.ax3, self.sweep_dclink.ax4
        for ax in (ax1, ax2, ax3, ax4):
            ax.clear()

        # Use the same envelope span as the main UI
        out_rpm_max = float(self.env_out_rpm_max.value()) if hasattr(self, "env_out_rpm_max") else 1.5
        n = 200
        out_rpms = np.linspace(0.0, out_rpm_max, n)

        # Single model instance (methods internally restore any temporary Vdc edits)
        p = copy.deepcopy(p0)
        model = SystemModel(p)
        ...
        tmax = np.zeros_like(out_rpms, dtype=float)
        vsrc = np.zeros_like(out_rpms, dtype=float)
        vinv = np.zeros_like(out_rpms, dtype=float)
        idc = np.zeros_like(out_rpms, dtype=float)
        vdrop = np.zeros_like(out_rpms, dtype=float)
        ploss = np.zeros_like(out_rpms, dtype=float)

        # COAX-only resistance (for loss display)
        r_coax = 0.0
        if hasattr(p, "coax") and bool(getattr(p.coax, "enabled", False)):
            try:
                r_coax = float(p.coax.loop_resistance_ohm())
            except Exception:
                r_coax = 0.0

        for i, out_rpm in enumerate(out_rpms):
            motor_rpm = float(out_rpm) * float(G)

            if fast:
                iq_max, idb, im, *_ = model.max_iq_given_limits_fast(motor_rpm, p.cable)
            else:
                iq_max, idb, im, *_ = model.max_iq_given_limits(motor_rpm, p.cable)

            kt_eff = model.kt_effective_nm_per_arms()
            tau_extra, _, _ = model.tau_extra_nm(rpm_to_rad_s(motor_rpm))
            t_motor_useful = max(0.0, float(kt_eff) * float(iq_max) - float(tau_extra))
            t_out_nm = t_motor_useful * float(G) * float(eta)
            tmax[i] = nm_to_ft_lbf(t_out_nm)

            vsrc[i] = float(getattr(model, "_dc_last_vdc_source", float(p.vf.vdc_link_v)))
            vinv[i] = float(getattr(model, "_dc_last_vdc_at_inv", vsrc[i]))
            idc[i] = float(getattr(model, "_dc_last_idc", 0.0))
            vdrop[i] = max(0.0, vsrc[i] - vinv[i])
            ploss[i] = (idc[i] * idc[i]) * float(r_coax)

        # --- Plot 1: Max torque envelope ---
        ax1.plot(out_rpms, tmax, linewidth=2, label="Max torque envelope")
        ax1.axhline(250.0, linestyle=":", linewidth=2, label="250 cont")
        ax1.axhline(1000.0, linestyle="-.", linewidth=2, label="1000 peak")
        ax1.plot([p0.target.out_rpm], [p0.target.out_torque_ftlbf], marker="o", markersize=7, linestyle="None", label="UI target")
        ax1.set_title("DC Link: Max torque envelope")
        ax1.set_xlabel("Output RPM")
        ax1.set_ylabel("Torque (ft-lbf)")
        ax1.grid(True, alpha=0.3)
        ax1.legend(fontsize=8, loc="best")

        # --- Plot 2: Vsource vs Vdc@inv ---
        ax2.plot(out_rpms, vsrc, linewidth=2, label="Vsource (UPM/UI)")
        ax2.plot(out_rpms, vinv, linewidth=2, label="Vdc@inv (after COAX)")
        ax2.set_title("DC Link: Voltage at interfaces (source vs inverter)")
        ax2.set_xlabel("Output RPM")
        ax2.set_ylabel("Vdc (V)")
        ax2.grid(True, alpha=0.3)
        ax2.legend(fontsize=8, loc="best")

        # --- Plot 3: Idc ---
        ax3.plot(out_rpms, idc, linewidth=2, label="Idc (DC link)")
        # limits if any
        i_lim = float(getattr(getattr(p, "coax", None), "i_limit_a", float("inf"))) if hasattr(p, "coax") else float("inf")
        if i_lim < float("inf"):
            ax3.axhline(i_lim, linestyle=":", linewidth=2, label="COAX I-limit")
        if hasattr(p, "upm") and bool(getattr(p.upm, "enabled", False)):
            ax3.axhline(float(p.upm.stack_imax()), linestyle="--", linewidth=1.5, label="UPM Imax")
        ax3.set_title("DC Link: Current at interfaces")
        ax3.set_xlabel("Output RPM")
        ax3.set_ylabel("A")
        ax3.grid(True, alpha=0.3)
        ax3.legend(fontsize=8, loc="best")

        # --- Plot 4: Vdrop and COAX loss ---
        ax4.plot(out_rpms, vdrop, linewidth=2, label="Vdrop = Vsource - Vdc@inv (V)")
        ax4.plot(out_rpms, ploss, linewidth=2, label="COAX loss I²R (W)")
        ax4.set_title("DC Link: Drop and DC loss (COAX)")
        ax4.set_xlabel("Output RPM")
        ax4.set_ylabel("V / W")
        ax4.grid(True, alpha=0.3)
        ax4.legend(fontsize=8, loc="best")

        self.sweep_dclink.canvas.draw_idle()
    ...
    def _plot_sweep_load_braking(self):
        ...
        ax3.plot(abs_rpm, regen_cap_1, ls='--', label='Regen cap (1 wire/phase)')
        ax3.plot(abs_rpm, regen_cap_2, ls='--', label='Regen cap (2 wires/phase)')
    ...
    def _plot_motor_tab(self,
                        res_sel: Optional[SolveResult] = None,
                        res_1: Optional[SolveResult] = None,
                        res_2: Optional[SolveResult] = None) -> None:
        """Render the Motor tab.

        Shows:
          - Motor torque capability vs motor RPM (Te and useful torque after τ_extra)
          - Output torque capability vs output RPM
          - Current/voltage headroom vs motor RPM
          - A compact table with key operating points (including the UI target)
        """
        if not hasattr(self, "motor_pane"):
            return

        pane = self.motor_pane

        # Prefer explicit args, else fall back to last computed results.
        if res_sel is None:
            res_sel = getattr(self, "_last_res_sel", None)
        if res_1 is None:
            res_1 = getattr(self, "_last_res_1", None)
        if res_2 is None:
            res_2 = getattr(self, "_last_res_2", None)

        env1 = getattr(self, "_last_env_1", None)
        env2 = getattr(self, "_last_env_2", None)
        cab1 = getattr(self, "_last_cab1", None)
        cab2 = getattr(self, "_last_cab2", None)
        coax_mode = bool(getattr(self, "_last_coax_mode", False))

        if env1 is None or cab1 is None:
            return
        ...
        # Clear axes
        for ax in (pane.ax_motor, pane.ax_out, pane.ax_curr, pane.ax_volt, pane.ax_tbl):
            ax.cla()

        pane.ax_tbl.set_axis_off()

        # --- helpers ---


    def _plot_moog_curves_tab(self):
        """Plot Moog reference curves vs the currently implemented τ_extra model."""
        if not hasattr(self, 'moog_pane'):
            return

        preset = 'Milling Moog Curve'
        if hasattr(self, 'x_preset'):
            try:
                preset = self.x_preset.currentText().strip() or preset
            except Exception:
                preset = preset

        # Placeholders (Spear/Annular) currently share the Milling dataset;
        # we keep the preset label so the user sees what was selected.
        self.moog_pane.render(self.model, preset_name=preset)


    def _plot_decision_tables(self, res_sel: Optional[SolveResult] = None,
                              res_1: Optional[SolveResult] = None,
                              res_2: Optional[SolveResult] = None):
        """Render the Tables tab (decision/summary tables)."""
        ...
        axes_to_clear = [pane.ax1, pane.ax2, pane.ax3, pane.ax4]
        if hasattr(pane, "ax5"):
            axes_to_clear.append(pane.ax5)
        if hasattr(pane, "ax6"):
            axes_to_clear.append(pane.ax6)
        ...
        def _make_table(ax, title: str, columns: List[str], rows: List[List[str]],
                        ...
                        cell_bg_overrides: Optional[Dict[Tuple[int, int], str]] = None,
                        row_bg: Optional[List[str]] = None,
                        fontsize: int = 9):
            ax.set_axis_off()
            ax.set_title(title, loc="left", fontsize=11, pad=6, fontweight="600")

            cell_bg_overrides = cell_bg_overrides or {}
            header_bg = "#111827"
            header_fg = "white"
            zebra = ["#f8fafc", "#eef2f7"]
            ...
            tbl = ax.table(cellText=[columns] + rows,
                           cellLoc="center",
            ...
            for r in range(n_rows):
                for c in range(n_cols):
                    ...
                    if r == 0:
                        ...
                        if row_bg and (r - 1) < len(row_bg) and row_bg[r - 1]:
                            base_bg = row_bg[r - 1]
                        else:
                            base_bg = zebra[(r - 1) % 2]
                        cell.set_facecolor(base_bg)
                    ...
                    if (r, c) in cell_bg_overrides:
                        cell.set_facecolor(cell_bg_overrides[(r, c)])

            return tbl

        def _fmt_ratio(val: float, lim: float, unit: str = "", pct: bool = True) -> Tuple[str, float]:
            lim = max(1e-12, float(lim))
            u = float(val) / lim
            if pct:
                s = f"{val:.2f} / {lim:.2f}{unit}\n({100.0*u:.0f}%)"
            else:
                s = f"{val:.2f} / {lim:.2f}{unit}"
            return s, u

        def _gate_text(u: float) -> str:
            if not math.isfinite(u):
                return "—"
            ok = (u <= 1.0 + 1e-6)
            return f"{100.0*u:.0f}% {'✓' if ok else '✗'}"

        def _winner_from_ratios(ratios: Dict[str, float]) -> str:
            # Pick the most-utilized active constraint (closest to 1.0).
            best_k = "—"
            best_u = -1.0
            for k, u in ratios.items():
                if u is None:
                    continue
                try:
                    uu = float(u)
        ...
        def _diag_at_out_rpm(out_rpm: float, cab_use: CableParams) -> Dict[str, float]:
            """Compute key envelope diagnostics at Iq_max for a given output rpm.

            Returns a dict with raw values + utilizations for each gate.
            """
            G_loc = float(p.gearbox.ratio())
            motor_rpm = float(max(0.0, out_rpm)) * G_loc

            iq_max, id_best, im_best, f_e, v_cmd, v_surface_limit, v_dh_phase = model.max_iq_given_limits(motor_rpm, cab_use)
            v_motor = model.motor_voltage_required_phase_rms(motor_rpm, float(id_best), float(iq_max))
            v_drop = model.cable_drop_phase_rms(motor_rpm, float(im_best), cab_use)
            v_node = float(v_motor) + float(v_drop)
            i_inv = model.inverter_current_mag_rms(motor_rpm, float(v_node), float(im_best))
            v_inv = model.inverter_voltage_required_phase_rms(motor_rpm, float(v_node), float(im_best))

            i_lim = float(cab_use.i_phase_limit())
            # In this simulator, inverter current limit is also the same rating (phase current) unless user set otherwise.
            i_lim_inv = i_lim

            u_i_mag = float(im_best) / max(1e-12, i_lim)
            u_i_inv = float(i_inv) / max(1e-12, i_lim_inv)
            u_v_inv = float(v_inv) / max(1e-12, float(v_cmd))
            u_v_dh = 0.0
            if v_dh_phase is not None and float(v_dh_phase) > 1e-9:
                u_v_dh = float(v_motor) / max(1e-12, float(v_dh_phase))

            # Optional DC-link gate (COAX/UPM): use cached last fixed-point results from model
            u_idc = float("nan")
            try:
                enabled, _vdc_src, _r_tot, i_lim_dc, _eta_src, _coax_mode = model._dc_link_model_params()
                if enabled and (i_lim_dc < float('inf')):
                    idc = float(getattr(model, "_dc_last_idc", 0.0))
                    u_idc = idc / max(1e-12, float(i_lim_dc))
            except Exception:
                pass

            return {
                "out_rpm": float(out_rpm),
                ...
                "v_inv": float(v_inv),
                "v_cmd": float(v_cmd),
                "v_dh": float(v_dh_phase) if v_dh_phase is not None else float('nan'),
                "u_i_mag": float(u_i_mag),
                "u_i_inv": float(u_i_inv),
                "u_v_inv": float(u_v_inv),
                "u_v_dh": float(u_v_dh),
                "u_idc": float(u_idc),
            }
        ...
        # ------------------------------------------------------------------
        # COAX-Mono tables (single architecture)
        # ------------------------------------------------------------------
        try:
            coax_mode = bool(getattr(getattr(p, "coax", None), "enabled", False))
        except Exception:
        ...
        if coax_mode:
            # Choose a single harness override (drop negligible) for any envelope point sampling
            cab = copy.deepcopy(p.cable)
            try:
                cab.drop_negligible = True
            except Exception:
                pass
            cab.wires_per_phase = 1
            cab.i_limit_basis = "Per phase"

            rpm_mag = np.arange(0.0, 5.0 + 1e-9, 0.5, dtype=float)
            rpm_cols = ["0 rpm"] + [f"{r:.1f} rpm" for r in rpm_mag[1:]]

            def _envelope_at_points() -> Tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
                rp, tq, iq, loss = model.compute_envelope(out_rpm_max=float(np.max(rpm_mag)),
                                                          n=int(len(rpm_mag)),
                                                          cable_override=cab)
                rp = np.asarray(rp, dtype=float)
                tq = np.asarray(tq, dtype=float)
                ...
                tq_i = np.interp(rpm_mag, rp, tq)
                iq_i = np.interp(rpm_mag, rp, iq)
                loss_i = np.interp(rpm_mag, rp, loss)
                return rpm_mag, tq_i, iq_i, loss_i

            rp, tq1, iq1, loss1 = _envelope_at_points()
            s_eta_lo, s_eta_hi = self._efficiency_torque_scalers()

            # Combine bands similar to v35 (η + downhole VLL band if available)
            def _combine_bands(tq_nom: np.ndarray) -> Tuple[np.ndarray, np.ndarray]:
                lows = [tq_nom * float(s_eta_lo)]
                highs = [tq_nom * float(s_eta_hi)]

                try:
                    vll_band = self._downhole_vll_band(rpm_mag, cable_override=cab)
                    if vll_band:
                        lows.append(np.asarray(vll_band["lo"], dtype=float))
                        highs.append(np.asarray(vll_band["hi"], dtype=float))
                except Exception:
                    pass

                try:
                    len_band = self._cable_length_torque_band(rpm_mag, tq_nom, cable_override=cab)
                    if len_band:
                        lows.append(np.asarray(len_band["lo"], dtype=float))
                        highs.append(np.asarray(len_band["hi"], dtype=float))
                except Exception:
                    pass
            ...
            lo1, hi1 = _combine_bands(tq1)

            # --- Table 1: Max torque vs RPM (bands) ---
            target_rpm = float(getattr(p.target, "out_rpm", 1.0))
            target_tq = float(getattr(p.target, "out_torque_ftlbf", 0.0))
            j_tgt = int(np.argmin(np.abs(rpm_mag - target_rpm)))
            col_tgt = 1 + j_tgt  # account for first "Arch" column

            def _fmt_band_cell(nom: float, lo: float, hi: float) -> str:
                return f"{nom:.0f}\n({lo:.0f}–{hi:.0f})"

            torque_cols = ["Arch"] + rpm_cols
            torque_rows = []
            w_sel = str(getattr(p.coax, "wiring_sel", "C+1H|3H"))
            wA = str(getattr(p.coax, "wiring_a", w_sel))
            wB = str(getattr(p.coax, "wiring_b", "C+2H|2H"))

            # Build rows for all HVDC wiring combinations (8 canonical splits)
            for key, _label in getattr(p.coax, "wiring_catalog", lambda: [])():
                try:
                    p2 = copy.deepcopy(p)
                    p2.coax.wiring_sel = str(key)
                    m2 = SystemModel(p2)
                    rp2, tq2, iq2, loss2 = m2.compute_envelope(out_rpm_max=float(np.max(rpm_mag)),
                                                               n=int(len(rpm_mag)),
                                                               cable_override=cab)
                    rp2 = np.asarray(rp2, dtype=float)
                    tq2 = np.asarray(tq2, dtype=float)
                    tq_i = np.interp(rpm_mag, rp2, tq2)
                    lo_i, hi_i = _combine_bands(tq_i)
                    ...
                    row_label = f"{key}{tag}"
                    torque_rows.append([row_label] + [_fmt_band_cell(tq_i[i], lo_i[i], hi_i[i]) for i in range(len(rpm_mag))])
                except Exception:
                    # keep table robust
                    continue

            if not torque_rows:
                torque_rows = [["(no wiring rows)"] + ["—"] * len(rpm_mag)]


            torque_overrides: Dict[Tuple[int, int], str] = {}
            if target_tq > 0.0:
                tn = tq1[j_tgt]; tlo = lo1[j_tgt]
                if tlo >= target_tq:
                    torque_overrides[(1, col_tgt)] = green
                elif tn >= target_tq:
                    torque_overrides[(1, col_tgt)] = amber
                else:
                    torque_overrides[(1, col_tgt)] = red

            _make_table(
                pane.ax1,
                f"Max torque vs RPM (ft·lbf) — bands shown as [low–high]  |  UI target: {target_tq:.0f} ft·lbf @ {target_rpm:.2f} rpm",
                torque_cols,
                torque_rows,
                col_widths=[0.24] + [max(0.05, (1.0 - 0.24) / max(1, len(rpm_cols)))] * len(rpm_cols),
                cell_bg_overrides=torque_overrides,
                fontsize=9,
            )

            # --- Table 2: Electrical utilization at target (single arch) ---
            elect_cols = ["Arch", "Feas.", "Iphase (Arms)", "VLL@motor (Vrms)", "Vϕ@inv (Vrms)", "Limiter"]
            elect_rows: List[List[str]] = []
            elect_overrides: Dict[Tuple[int, int], str] = {}
            ...
            rr = res_sel
            if rr is None:
                elect_rows.append(["COAX mono", "—", "—", "—", "—", "—"])
            else:
                feas = "PASS" if bool(getattr(rr, "feasible", False)) else "FAIL"
                elect_overrides[(1, 1)] = green if feas == "PASS" else red

                i_used = _safe(getattr(rr, "i_mag_used_rms", 0.0))
                i_lim = _safe(getattr(rr, "i_limit_phase_mag", max(1e-12, cab.i_phase_limit())))
                s_i, u_i = _fmt_ratio(i_used, i_lim, unit="")

                vph_inv = _safe(getattr(rr, "v_inverter_req", 0.0))
                vph_lim = _safe(getattr(rr, "v_surface_limit", max(1e-12, p.vf.v_phase_rms_limit())))
                s_vph, u_vph = _fmt_ratio(vph_inv, vph_lim, unit="")

                vph_motor = _safe(getattr(rr, "v_motor_req", 0.0))
                vll_motor = math.sqrt(3.0) * vph_motor

                vll_lim = getattr(rr, "v_downhole_ll_limit", None)
                if vll_lim is None or float(vll_lim) <= 0.0:
                    vll_lim = float(getattr(p.limits, "downhole_vll_rms_limit", 0.0) or 0.0)

                if vll_lim and float(vll_lim) > 0.0:
                    s_vll, u_vll = _fmt_ratio(vll_motor, float(vll_lim), unit="")
                else:
                    s_vll, u_vll = (f"{vll_motor:.1f}\n(—)", 0.0)
                ...
                if u_i >= 0.99:
                    ...
                    lim = "Vinv"
                elif (vll_lim and float(vll_lim) > 0.0) and (u_vll >= 0.99):
                    lim = "VLL(dh)"

                elect_overrides[(1, 2)] = _util_bg(u_i)
                elect_overrides[(1, 3)] = _util_bg(u_vll if (vll_lim and float(vll_lim) > 0.0) else 0.0)
                elect_overrides[(1, 4)] = _util_bg(u_vph)

                elect_rows.append(["COAX mono", feas, s_i, s_vll, s_vph, lim])

            _make_table(
                pane.ax2,
                "Electrical utilization at UI target (COAX mono). Harness drop neglected; Iphase is inverter rating.",
                elect_cols,
                elect_rows,
                col_widths=[0.20, 0.10, 0.22, 0.23, 0.20, 0.05],
                cell_bg_overrides=elect_overrides,
                fontsize=9,
            )

            # --- Table 3: Diagnostics along the envelope (Iq_max point) ---
            # Sample points emphasize the common "knee" region (~0.6–0.75 rpm) plus the UI target.
            G_local = float(p.gearbox.ratio())
            rpm_samples = [0.50, 0.60, 0.65, 0.70, 0.75, 1.00]
            try:
                if target_rpm not in rpm_samples and (0.05 <= target_rpm <= 2.0):
                    rpm_samples.append(float(target_rpm))
            except Exception:
                pass
            rpm_samples = sorted({float(max(0.01, r)) for r in rpm_samples})

            sf = getattr(p, "sine_filter", None)
            inv_i_limit_on = bool(getattr(sf, "enabled", False)) and bool(getattr(sf, "enforce_inv_current_limit", True))

            def _diag_iqmax(out_rpm: float, cab_use: CableParams) -> Dict[str, object]:
                motor_rpm = float(out_rpm) * G_local
                iq_max, id_best, i_mag, f_e, v_cmd, _v_surf_lim, v_dh_phase = model.max_iq_given_limits(motor_rpm, cab_use)

                v_motor = model.motor_voltage_required_phase_rms(motor_rpm, float(id_best), float(iq_max))
                v_drop = model.cable_drop_phase_rms(motor_rpm, float(i_mag), cab_use)
                v_node = float(v_motor) + float(v_drop)
                i_inv = model.inverter_current_mag_rms(motor_rpm, v_node, float(i_mag))
                v_inv = model.inverter_voltage_required_phase_rms(motor_rpm, v_node, float(i_mag))

                i_lim = float(cab_use.i_phase_limit())

                # Downhole phase limit (if active)
                v_dh_eff = v_dh_phase
                if v_dh_eff is None:
                    try:
                        v_dh_eff = model._effective_downhole_phase_limit()
                    except Exception:
                        v_dh_eff = None
                ...
                i_lim_dc = None
                try:
                    enabled_dc, _vdc_src, _r_tot, i_lim_dc, _eta_src, _ = model._dc_link_model_params()
                    if enabled_dc:
                        idc = float(getattr(model, "_dc_last_idc", 0.0))
                        vdc_at_inv = float(getattr(model, "_dc_last_vdc_at_inv", 0.0))
                except Exception:
                    pass

                # Utilizations
                u = {}
                u["I|"] = (float(i_mag) / max(1e-12, i_lim))
                if inv_i_limit_on:
                    u["Iinv"] = (float(i_inv) / max(1e-12, i_lim))
                u["Vinv"] = (float(v_inv) / max(1e-12, float(v_cmd)))
                if v_dh_eff is not None and float(v_dh_eff) > 1e-9:
                    u["Vdh"] = (float(v_motor) / max(1e-12, float(v_dh_eff)))
                if (idc is not None) and (i_lim_dc is not None) and math.isfinite(float(i_lim_dc)) and float(i_lim_dc) > 1e-9:
                    u["Idc"] = (float(idc) / max(1e-12, float(i_lim_dc)))

                winner = "—"
                if u:
                    winner = max(u.items(), key=lambda kv: kv[1])[0]
                ...
                return {
                    ...
                    "v_cmd": float(v_cmd),
                    "v_m": float(v_motor),
                    "v_dh": (None if v_dh_eff is None else float(v_dh_eff)),
                    "idc": idc,
                    "vdc_inv": vdc_at_inv,
                    "u": u,
                    "winner": winner,
            ...
            diags = [_diag_iqmax(r, cab) for r in rpm_samples]

            diag_cols = ["RPM", "f_e (Hz)", "Iq_max", "Id", "|I|", "Iinv", "Vinv", "Vcmd", "Vm", "Vdh"]
            diag_rows: List[List[str]] = []
            for d in diags:
                vdh_s = "—" if d["v_dh"] is None else f"{float(d['v_dh']):.1f}"
                diag_rows.append([
                    f"{d['out_rpm']:.2f}",
                    f"{d['f_e']:.1f}",
                    f"{d['iq_max']:.2f}",
            ...
            _make_table(
                pane.ax3,
                "Key electrical measures along Iq_max envelope (COAX mono)",
                diag_cols,
                diag_rows,
                col_widths=[0.09, 0.11, 0.10, 0.08, 0.08, 0.09, 0.09, 0.09, 0.08, 0.09],
                fontsize=8,
            )

            # --- Table 4: Gate utilization + winning limiter (per point) ---
            def _gate_cell(u_val: Optional[float]) -> Tuple[str, float]:
                if u_val is None:
                    return "—", 0.0
                u_val = float(u_val)
                ok = (u_val <= 1.0 + 1e-9)
                return f"{100.0*u_val:.0f}% {'✓' if ok else '✗'}", u_val

            gate_cols = ["RPM", "I|", "Iinv", "Vinv", "Vdh", "Idc", "Winner"]
            gate_rows: List[List[str]] = []
            gate_over: Dict[Tuple[int, int], str] = {}

            for r_i, d in enumerate(diags, start=1):
                u = d["u"]
                s_im, uim = _gate_cell(u.get("I|"))
                s_iinv, uiinv = _gate_cell(u.get("Iinv"))
                s_vinv, uvinv = _gate_cell(u.get("Vinv"))
                s_vdh, uvdh = _gate_cell(u.get("Vdh"))
                s_idc, uidc = _gate_cell(u.get("Idc"))
                gate_rows.append([
                    f"{d['out_rpm']:.2f}",
                    s_im,
                    s_iinv,
                ...
                gate_over[(r_i, 1)] = _util_bg(uim)
                gate_over[(r_i, 2)] = _util_bg(uiinv)
                gate_over[(r_i, 3)] = _util_bg(uvinv)
                gate_over[(r_i, 4)] = _util_bg(uvdh)
                gate_over[(r_i, 5)] = _util_bg(uidc)

            _make_table(
                pane.ax4,
                "Gate utilization at Iq_max envelope points (percent of limit) + winning limiter",
                gate_cols,
                gate_rows,
                col_widths=[0.10, 0.13, 0.13, 0.13, 0.13, 0.13, 0.25],
                cell_bg_overrides=gate_over,
                fontsize=8,
            )

            # --- Table 5: DC link + COAX summary at target ---
            dc_cols = ["Item", "Value"]
            ...
            eta_upm = 1.0
            vdc_source = float(getattr(p.vf, "vdc_link_v", 0.0))
            r_src = 0.0
            i_lim_upm = float("inf")
            if upm_en:
                try:
                ...
                try:
                    if bool(getattr(upm, "use_as_vdc_source", False)):
                        vdc_source = float(upm.stack_vdc(coax_mode=True))
                except Exception:
                    pass
                try:
                    r_src = float(upm.stack_r_internal(coax_mode=True))
                except Exception:
                    r_src = 0.0
                try:
                    i_lim_upm = float(upm.stack_imax())
            ...
            if coax_en:
                ...
                try:
                    if bool(getattr(coax, "enforce_i_limit", True)):
                        if hasattr(coax, "dc_current_limit_a"):
                            i_lim_coax = float(coax.dc_current_limit_a(wkey))
                        else:
                            i_lim_coax = float(getattr(coax, "i_limit_a", float("inf")))
                except Exception:
                    pass
                try:
                    # legacy: show key in the UI
                    ret = str(getattr(coax, "return_path", wkey)) or wkey
                except Exception:
                    ret = wkey

            i_lim_dc = min(i_lim_upm, i_lim_coax)

            # Estimate Idc and drooped Vdc@inv only when DC basis is used
            basis_is_vdc = str(p.vf.voltage_entry_basis).strip().upper().startswith("DC")
            vdc_at_inv = float(vdc_source)
            idc_est = 0.0
            vdrop = 0.0
            ploss = 0.0
            if basis_is_vdc and (upm_en or coax_en) and rr is not None:
                vph_inv_req = _safe(getattr(rr, "v_inverter_req", 0.0))
                iph_inv_rms = _safe(getattr(rr, "i_inverter_rms", 0.0))
                s_va = max(0.0, 3.0 * vph_inv_req * iph_inv_rms)
                r_tot = float(r_src + r_coax)
                vguess = max(1e-6, float(vdc_source))
                for _ in range(2):
                    idc_est = (s_va / eta_upm) / max(1e-6, vguess)
                    vdc_at_inv = max(0.0, float(vdc_source) - float(idc_est) * r_tot)
                    vguess = max(1e-6, vdc_at_inv)
                vdrop = float(idc_est) * r_tot
                ploss = (float(idc_est) ** 2) * r_tot

            dc_rows.append(["Return path", f"{ret}"])
            dc_rows.append(["Vdc source", f"{vdc_source:.0f} V"])
            if basis_is_vdc and (upm_en or coax_en):
                lim_txt = f"{i_lim_dc:.2f} A" if math.isfinite(i_lim_dc) else "—"
                dc_rows.append(["Idc est.", f"{idc_est:.2f} A (limit {lim_txt})"])
                dc_rows.append(["Vdc @ inverter", f"{vdc_at_inv:.0f} V"])
                dc_rows.append(["Rsrc + Rcoax", f"{(r_src + r_coax):.2f} Ω"])
                dc_rows.append(["Vdrop (src+coax)", f"{vdrop:.1f} V"])
                dc_rows.append(["Ploss (src+coax)", f"{ploss:.0f} W"])
            else:
                dc_rows.append(["DC droop model", "Shown only when Voltage entry basis = 'DC link (Vdc)'"])

            _make_table(
                pane.ax5,
                "DC link + COAX summary (target point)",
                dc_cols,
                dc_rows,
            ...
            # --- Table 6: Limits + assumptions summary ---
            lim_cols = ["Assumption / Limit", "Value"]
            lim_rows: List[List[str]] = []
            lim_rows.append(["Harness drop", "Negligible (R≈0, L≈0)"])
            lim_rows.append(["Inverter phase current rating", f"{cab.i_phase_limit():.2f} Arms"])
            vll_lim_eff = float(getattr(p.limits, "downhole_vll_rms_limit", 0.0) or 0.0)
            lim_rows.append(["Contact-block VLL,rms limit", f"{vll_lim_eff:.1f} Vrms"])
            vph_lim_eff = float(getattr(p.vf, "v_phase_rms_limit", lambda: 0.0)())
            lim_rows.append(["Inverter Vϕ,rms ceiling", f"{vph_lim_eff:.1f} Vrms"])
            lim_rows.append(["Note", "2-wire / 1-wire is not applicable in COAX-Mono"])

            _make_table(
                pane.ax6,
                "Model limits + retained constraints (COAX mono)",
                lim_cols,
                lim_rows,
        ...
        cab1 = copy.deepcopy(p.cable); cab1.wires_per_phase = 1
        cab2 = copy.deepcopy(p.cable); cab2.wires_per_phase = 2

        def _envelope_at_points(cab: CableParams) -> Tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
            rp, tq, iq, loss = model.compute_envelope(out_rpm_max=float(np.max(rpm_mag)),
                                                      n=int(len(rpm_mag)),
                                                      cable_override=cab)
            rp = np.asarray(rp, dtype=float)
            ...
            # Guard in case model changes spacing.
            tq_i = np.interp(rpm_mag, rp, tq)
            iq_i = np.interp(rpm_mag, rp, iq)
            loss_i = np.interp(rpm_mag, rp, loss)
            return rpm_mag, tq_i, iq_i, loss_i

        rp, tq1, iq1, loss1 = _envelope_at_points(cab1)
        _,  tq2, iq2, loss2 = _envelope_at_points(cab2)

        s_eta_lo, s_eta_hi = self._efficiency_torque_scalers()

        def _combine_bands(tq_nom: np.ndarray, cab: CableParams) -> Tuple[np.ndarray, np.ndarray]:
            lows = [tq_nom * float(s_eta_lo)]
            highs = [tq_nom * float(s_eta_hi)]
        ...
        lo1, hi1 = _combine_bands(tq1, cab1)
        lo2, hi2 = _combine_bands(tq2, cab2)
        ...
        target_tq = float(getattr(p.target, "out_torque_ftlbf", 0.0))
        j_tgt = int(np.argmin(np.abs(rpm_mag - target_rpm)))
        col_tgt = 1 + j_tgt  # account for first "Wiring" column

        def _fmt_band_cell(nom: float, lo: float, hi: float) -> str:
            return f"{nom:.0f}\n({lo:.0f}–{hi:.0f})"

        torque_cols = ["Wiring"] + rpm_cols
        torque_rows = [
            ["1-wire/phase"] + [_fmt_band_cell(tq1[i], lo1[i], hi1[i]) for i in range(len(rpm_mag))],
            ["2-wire/phase"] + [_fmt_band_cell(tq2[i], lo2[i], hi2[i]) for i in range(len(rpm_mag))],
        ]

        torque_overrides: Dict[Tuple[int, int], str] = {}
        if target_tq > 0.0:
            for r_i, (tn, tlo) in enumerate([(tq1[j_tgt], lo1[j_tgt]), (tq2[j_tgt], lo2[j_tgt])], start=1):
                if tlo >= target_tq:
                    torque_overrides[(r_i, col_tgt)] = green
                elif tn >= target_tq:
                    torque_overrides[(r_i, col_tgt)] = amber
                else:
                    torque_overrides[(r_i, col_tgt)] = red
        ...
        tbl1 = _make_table(
            ...
            torque_cols,
            torque_rows,
            col_widths=[0.22] + [max(0.05, (1.0 - 0.22) / max(1, len(rpm_cols)))] * len(rpm_cols),
            cell_bg_overrides=torque_overrides,
            fontsize=9,
        )
        ...
        # ------------------------------------------------------------------
        # Table 2: Electrical utilization at target (both wirings)
        # ------------------------------------------------------------------
        elect_cols = ["Wiring", "Feas.", "I (Arms)", "VLL@motor (Vrms)", "Vϕ@inv (Vrms)", "Limiter"]
        elect_rows: List[List[str]] = []
        elect_overrides: Dict[Tuple[int, int], str] = {}

        def _row_from_res(label: str, res: Optional[SolveResult], r_index: int):
            if res is None:
                elect_rows.append([label, "—", "—", "—", "—", "—"])
                return

            feas = "PASS" if bool(getattr(res, "feasible", False)) else "FAIL"
            feas_bg = green if feas == "PASS" else red
            elect_overrides[(r_index, 1)] = feas_bg

            i_used = float(getattr(res, "i_mag_used_rms", 0.0))
            i_lim = float(getattr(res, "i_limit_phase_mag", max(1e-12, p.cable.i_phase_limit())))
            s_i, u_i = _fmt_ratio(i_used, i_lim, unit="")

            vph_inv = float(getattr(res, "v_inverter_req", 0.0))
            vph_lim = float(getattr(res, "v_surface_limit", max(1e-12, p.vf.v_phase_rms_limit())))
            s_vph, u_vph = _fmt_ratio(vph_inv, vph_lim, unit="")

            vph_motor = float(getattr(res, "v_motor_req", 0.0))
            vll_motor = math.sqrt(3.0) * vph_motor

            vll_lim = float(getattr(res, "v_downhole_ll_limit", math.sqrt(3.0) * float(getattr(res, "v_downhole_phase_limit", 0.0))))
            if vll_lim <= 0.0:
                # fall back to effective downhole limit from params (if any)
                try:
                    vll_lim = float(self._effective_downhole_vll_limit()) if hasattr(self, "_effective_downhole_vll_limit") else 0.0
                except Exception:
                    vll_lim = 0.0

            if vll_lim > 0.0:
                s_vll, u_vll = _fmt_ratio(vll_motor, vll_lim, unit="")
            else:
                s_vll, u_vll = (f"{vll_motor:.1f}\n(—)", 0.0)
            ...
            if u_i >= 0.99:
                ...
                lim = "Vsurf"
            elif (vll_lim > 0.0) and (u_vll >= 0.99):
                lim = "Vdh"

            # utilization coloring
            elect_overrides[(r_index, 2)] = _util_bg(u_i)
            elect_overrides[(r_index, 3)] = _util_bg(u_vll if vll_lim > 0.0 else 0.0)
            elect_overrides[(r_index, 4)] = _util_bg(u_vph)

            elect_rows.append([label, feas, s_i, s_vll, s_vph, lim])

        _row_from_res("1-wire/phase", res_1, 1)
        _row_from_res("2-wire/phase", res_2, 2)

        _make_table(
            pane.ax2,
            "Electrical utilization at UI target (colored by utilization; PASS/FAIL from solver)",
            elect_cols,
            elect_rows,
            col_widths=[0.22, 0.10, 0.20, 0.22, 0.20, 0.06],
            cell_bg_overrides=elect_overrides,
            fontsize=9,
        )

        # ------------------------------------------------------------------
        # Table 3: Key electrical measures along the Iq_max envelope (both wirings)
        # Table 4: Gate utilization + winning limiter (both wirings)
        # ------------------------------------------------------------------
        G_local = float(p.gearbox.ratio())
        rpm_samples = [0.50, 0.60, 0.65, 0.70, 0.75, 1.00]
        try:
        ...
        diag_rows: List[List[str]] = []
        diags_flat: List[Dict[str, object]] = []
        for cfg, cab_use in [("1w", cab1), ("2w", cab2)]:
            for r in rpm_samples:
                d = _diag_iqmax(r, cab_use)
                d["cfg"] = cfg
                diags_flat.append(d)
                vdh_s = "—" if d["v_dh"] is None else f"{float(d['v_dh']):.1f}"
                diag_rows.append([
        ...
        _make_table(
            pane.ax3,
            "Key electrical measures along Iq_max envelope (1-wire and 2-wire)",
            ["Cfg", "RPM", "f_e", "Iq_max", "Id", "|I|", "Iinv", "Vinv", "Vcmd", "Vm", "Vdh"],
            diag_rows,
            col_widths=[0.07, 0.08, 0.08, 0.09, 0.07, 0.07, 0.08, 0.08, 0.08, 0.07, 0.07],
            fontsize=8,
        )
        ...
        gate_cols = ["Cfg", "RPM", "I|", "Iinv", "Vinv", "Vdh", "Idc", "Winner"]
        gate_rows: List[List[str]] = []
        gate_over: Dict[Tuple[int, int], str] = {}

        for r_i, d in enumerate(diags_flat, start=1):
            u = d["u"]
            s_im, uim = _gate_cell(u.get("I|"))
            s_iinv, uiinv = _gate_cell(u.get("Iinv"))
            s_vinv, uvinv = _gate_cell(u.get("Vinv"))
            ...
            # util coloring (skip columns 0-1)
            gate_over[(r_i, 2)] = _util_bg(uim)
            gate_over[(r_i, 3)] = _util_bg(uiinv)
            gate_over[(r_i, 4)] = _util_bg(uvinv)
            gate_over[(r_i, 5)] = _util_bg(uvdh)
            gate_over[(r_i, 6)] = _util_bg(uidc)

        _make_table(
            pane.ax4,
            "Gate utilization (percent of limit) + winning limiter (at Iq_max envelope)",
            gate_cols,
            gate_rows,
            col_widths=[0.07, 0.08, 0.12, 0.12, 0.12, 0.12, 0.12, 0.25],
            cell_bg_overrides=gate_over,
            fontsize=8,
        )

        # ------------------------------------------------------------------
        # Table 3: Along-envelope values (selected wiring) @ a few RPM points
        # ------------------------------------------------------------------
        sel_wires = int(getattr(p.cable, "wires_per_phase", 1))
        sel_label = "1-wire/phase" if sel_wires == 1 else "2-wire/phase"
        cab_sel = cab1 if sel_wires == 1 else cab2
        _, tq_sel, iq_sel, _loss_sel = _envelope_at_points(cab_sel)

        def _env_vll(cab: CableParams) -> np.ndarray:
            vll = np.zeros_like(rpm_mag, dtype=float)
            G_local = float(p.gearbox.ratio())
            for i, out_rpm in enumerate(rpm_mag):
                motor_rpm = float(out_rpm) * G_local
                iq_max, id_best, _, _, _, _, _ = model.max_iq_given_limits(motor_rpm, cab)
                vph_motor = model.motor_voltage_required_phase_rms(motor_rpm, float(id_best), float(iq_max))
                vll[i] = math.sqrt(3.0) * float(vph_motor)
            return vll

        vll_sel = _env_vll(cab_sel)

        env_cols = ["RPM", "Tmax (ft·lbf)", "Iq,max (Arms)", "VLL@motor (Vrms)"]
        env_rows = [[f"{rpm_mag[i]:.1f}", f"{tq_sel[i]:.0f}", f"{iq_sel[i]:.2f}", f"{vll_sel[i]:.1f}"]
                    for i in range(len(rpm_mag))]

        env_overrides: Dict[Tuple[int, int], str] = {}
        # highlight the row nearest UI target rpm
        env_overrides[(1 + j_tgt, 0)] = ui_row_bg
        env_overrides[(1 + j_tgt, 1)] = ui_row_bg
        env_overrides[(1 + j_tgt, 2)] = ui_row_bg
        env_overrides[(1 + j_tgt, 3)] = ui_row_bg

        _make_table(
            pane.ax5,
            f"Selected wiring envelope @ max torque (wiring = {sel_label})",
            env_cols,
            env_rows,
            col_widths=[0.16, 0.28, 0.28, 0.28],
            cell_bg_overrides=env_overrides,
            fontsize=9,
        )

        # ------------------------------------------------------------------
        # Table 4: Loss + DC/UPM/COAX summary (if present)
        # ------------------------------------------------------------------
        loss_cols = ["Item", "Value"]
        loss_rows: List[List[str]] = []

        # Motor copper loss at target (approx)
        if res_sel is not None:
            i_used = float(getattr(res_sel, "i_mag_used_rms", 0.0))
            rs = float(getattr(res_sel, "rs_eff_ohm", 0.0))
            p_cu = 3.0 * (i_used ** 2) * rs
            p_cable = float(getattr(res_sel, "p_cable_loss_w", 0.0))
            loss_rows.append(["Motor copper loss @ target", f"{p_cu:.1f} W"])
            loss_rows.append(["Cable loss @ target", f"{p_cable:.1f} W"])
            loss_rows.append(["Total (cu+cable) @ target", f"{(p_cu+p_cable):.1f} W"])

            vfd = float(getattr(res_sel, "v_filter_drop", 0.0))
            if vfd > 0.0:
                loss_rows.append(["Sine filter drop (phase RMS)", f"{vfd:.2f} V"])
            if float(getattr(res_sel, "i_filter_cap", 0.0)) > 0.0:
                loss_rows.append(["Sine filter cap current", f"{float(getattr(res_sel, 'i_filter_cap', 0.0)):.2f} Arms"])

        # DC link info (v35 COAX/UPM paths may populate these)
        vdc_src = getattr(self, "_last_vdc_source", None)
        vdc_inv = getattr(self, "_last_vdc_at_inv", None)
        if vdc_src is not None:
            loss_rows.append(["Vdc source (last)", f"{float(vdc_src):.1f} V"])
        if vdc_inv is not None:
            loss_rows.append(["Vdc at inverter (last)", f"{float(vdc_inv):.1f} V"])

        # UPM / COAX params if available on this build
        if hasattr(p, "upm"):
            try:
                upm = p.upm
                loss_rows.append(["UPM stack Vdc (set)", f"{float(upm.stack_vdc(coax_mode=bool(getattr(p, 'coax', None) and getattr(p.coax, 'enabled', False)))):.0f} V"])
                loss_rows.append(["UPM stack Imax", f"{float(upm.stack_imax()):.2f} A"])
                loss_rows.append(["UPM Rint (stack)", f"{float(upm.stack_r_internal(coax_mode=bool(getattr(p, 'coax', None) and getattr(p.coax, 'enabled', False)))):.3f} Ω"])
            except Exception:
                pass
        ...
        if hasattr(p, "coax"):
            try:
                cx = p.coax
                if bool(getattr(cx, "enabled", False)):
                    loss_rows.append(["COAX loop R (5-seg)", f"{float(cx.loop_resistance_ohm()):.3f} Ω"])
                    loss_rows.append(["COAX I-limit", f"{float(cx.i_limit_a):.2f} A"])
                    loss_rows.append(["COAX return path", str(getattr(cx, 'return_path', '—'))])
            except Exception:
                pass
        ...
        _make_table(
            pane.ax6,
            "Loss + DC/UPM/COAX summary (target point; where available)",
            loss_cols,
            loss_rows,
            col_widths=[0.46, 0.54],
            fontsize=9,
        )
