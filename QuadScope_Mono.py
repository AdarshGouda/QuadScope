...
@dataclass
class CoaxParams:
    """Monocable / COAX DC-link model (UPM → inverter).

    Default wiring is center conductor (+) and serve/shield (−), with armor NOT participating,
    consistent with the StreamLINE termination concept (serve isolated from armor at collector end).
    """

    enabled: bool = True
    enforce_i_limit: bool = True

    # Return path option (UI-managed)
    return_path: str = "Serve only"  # "Serve only", "Armor only", "Serve || Armor (parallel)"

    # Resistances at 20°C (ohm/m). Defaults derived from 102686324 nominal properties:
    #  - Center (conductor): 14.1 Ω/km
    #  - Serve (shield):     13.4 Ω/km
    #  - Armor:              11.2 Ω/km
    r_center_ohm_per_m: float = 14.1 / 1000.0
    r_serve_ohm_per_m: float = 13.4 / 1000.0
    r_armor_ohm_per_m: float = 11.2 / 1000.0

    # Simple loop inductance (optional; steady-state solver uses R only)
    l_loop_h_per_m: float = 2.0e-7

    # Current rating (A) for the power path
    i_limit_a: float = 2.61

    # Temperature model (mirrors CableParams 5-seg, but applied to the DC loop resistance)
    ...
    temp5_seg_len_m: List[float] = field(default_factory=lambda: [1500.0, 1100.0,  900.0, 1800.0, 1860.0])
    temp5_seg_temp_C: List[float] = field(default_factory=lambda: [100.0,   125.0, 160.0, 175.0, 175.0])
    temp_factor_r: float = 1.0  # used if temp_model_5seg=False

    def _temp_scale(self, temp_C: float) -> float:
        a = float(self.temp_alpha_per_C)
        return max(0.0, 1.0 + a * (float(temp_C) - float(self.temp_ref_C)))

    def _iter_segments(self):
        if bool(self.temp_model_5seg) and len(self.temp5_seg_len_m) == 5 and len(self.temp5_seg_temp_C) == 5:
            for L, T in zip(self.temp5_seg_len_m, self.temp5_seg_temp_C):
                yield float(L), float(T)
        else:
            yield float(self.length_m), float(self.temp_ref_C)

    def loop_resistance_ohm(self) -> float:
        """Total DC loop resistance (Ω) from center conductor out and selected return path back."""
        r_total = 0.0
        for L, T in self._iter_segments():
            s = self._temp_scale(T)
            r_out = float(self.r_center_ohm_per_m) * s * L

            if self.return_path == "Armor only":
                r_ret = float(self.r_armor_ohm_per_m) * s * L
            elif self.return_path == "Serve || Armor (parallel)":
                # parallel return (only valid if serve+armor bonded at BOTH ends; kept as a UI option)
                r_s = float(self.r_serve_ohm_per_m) * s * L
                r_a = float(self.r_armor_ohm_per_m) * s * L
                r_ret = 1.0 / max(1e-12, (1.0 / max(1e-12, r_s)) + (1.0 / max(1e-12, r_a)))
            else:
                # default
                r_ret = float(self.r_serve_ohm_per_m) * s * L

            r_total += (r_out + r_ret)

        # optional uniform scaling if not using 5-seg
        if not bool(self.temp_model_5seg):
            r_total *= max(0.0, float(self.temp_factor_r))
        return max(0.0, r_total)

    def loop_inductance_h(self) -> float:
        Ltot = 0.0
        for L, _T in self._iter_segments():
            Ltot += float(self.l_loop_h_per_m) * L
        return max(0.0, Ltot)
...
class SystemModel:
    ...
    def _dc_link_model_params(self):
        ...
        if coax_en:
            r_coax = float(coax.loop_resistance_ohm())
...
class NavigatorVfWindow(QMainWindow):
    ...
    def _build_controls(self):
        ...
        self.x_k_c = make_dspin(0.0, 100.0, 0.000001, self.params.extra.visc_k_couette, 7)  # finer for milling-motor drag fit
        self.x_k_tr = make_dspin(0.0, 100.0, 0.000001, self.params.extra.visc_k_transition, 7)
    ...
    def _apply_motor_preset(self, name: str) -> None:
        ...
        try:
            ...
            if hasattr(self, "x_preset"):
                ...
                sel = m2x.get(n, "User Defined")
                self.x_preset.setCurrentText(sel)
                # Apply the extra-torque preset immediately (signals are blocked by _bulk_updating)
                self._apply_extra_torque_preset(sel)
            else:
                if n == "Milling motor":
                    self._apply_milling_motor_extra_torque()
        finally:
    ...
    def _apply_milling_motor_extra_torque(self) -> None:
        ...
        """
        # UI may not have the Extra-Torque pane in some reduced builds
        if not all(hasattr(self, a) for a in ("x_enable_extra", "x_core_en", "x_core_cl", "x_core_exp",
    ...
    def _sync_params_from_ui(self):
        ...
        if hasattr(p, "coax") and hasattr(self, "coax_enable"):
            ...
            p.coax.return_path = str(self.coax_return.currentText())
    ...
    def update_all(self):
        ...
        # COAX-Mono: no 1-wire/2-wire comparison (heptacable concept). Keep a single harness model.
        ...
        if coax_mode:
            ...
            # COAX-Mono: harness is negligible; keep a single 'cable' object for compatibility.
            ...
            env_rpm_2, env_tq_2, env_iq_2, env_loss_2 = env_rpm_1, env_tq_1, env_iq_1, env_loss_1
        else:
    ...
    def _update_interface_readouts(self, res: SolveResult) -> None:
        ...
        if hasattr(self, "lbl_coax_info"):
            ...
            if coax_en:
                lines.append(
                    f"COAX: return='{return_desc}' | L≈{l_total_m:.0f} m | Rloop≈{r_coax:.2f} Ω | I_limit≈{float(getattr(coax, 'i_limit_a', 0.0)):.2f} A"
                )
            else:
...
# Render Tables pane on-demand when it becomes visible.
    ...
    def _state_widgets_for_defaults(self):
        ...
        w = [
            ...
            self.c_len_band,
            # motor
            self.m_preset, self.m_pp, self.m_rs, self.m_ld, self.m_lq, self.m_link, self.m_mode, self.m_kt_basis, self.m_ke_basis,
            self.m_lambda, self.m_kt, self.m_ke,
            # extra torque + Kt(T)
            self.x_enable_extra, self.x_enable_ktT, self.x_enable_rsT, self.x_temp, self.x_tref, self.x_kt_tc,
            self.x_wind_dT, self.x_rs_tc,
            ...
            self.x_smooth, self.x_smooth_f,
            # gearbox
            self.g_preset, self.g1, self.g2, self.g3, self.e1, self.e2, self.e3, self.eta_misc, self.chk_eta_override,
    ...
    def _plot_envelope(self, res: SolveResult, env1, env2):
        ...
        if bool(getattr(getattr(self.params, "coax", None), "enabled", False)):
            ax1.plot(rpm1_sym, tq1_mot, linewidth=2, label='COAX mono (motoring)')
        else:
    ...
    def _plot_architecture(self) -> None:
        ...
        ax.text(0.02, 0.96, "Architecture (static model)", fontsize=15, weight="bold", color=title_color, va="top")
    ...
    def _plot_decision_tables(self, res_sel: Optional[SolveResult] = None,
        ...
        if coax_mode:
            ...
            rpm_mag = np.round(np.arange(0.0, 5.0 + 0.5, 0.5), 3).astype(float)
            ...
            torque_rows = [
                ["COAX mono"] + [_fmt_band_cell(tq1[i], lo1[i], hi1[i]) for i in range(len(rpm_mag))],
            ]
            ...
            _make_table(
                ...
                torque_cols,
                torque_rows,
                col_widths=[0.24] + [0.76 / max(1, len(rpm_cols))] * len(rpm_cols),
            ...
            try:
                if target_rpm not in rpm_samples and (0.05 <= target_rpm <= 5.0):
            ...
            if coax_en:
                ...
                try:
                    ...
                    pass
                try:
                    ret = str(getattr(coax, "return_path", "Serve only"))
                except Exception:
                    ret = "—"
        ...
        tbl1 = _make_table(
            ...
            torque_cols,
            torque_rows,
            col_widths=[0.22] + [0.78 / max(1, len(rpm_cols))] * len(rpm_cols),
            cell_bg_overrides=torque_overrides,
            fontsize=9,
        )
