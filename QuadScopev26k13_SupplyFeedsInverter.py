"""
Navigator V/f Power-Chain Feasibility UI (v26k)
...
- Heptacable datasheet presets for 7-wire cable families (7-46A-XS-CB, 7-39AI-XXS, 7-56ZA-SLC, 7-59ZA-SUS-SLC, 7-48 StreamLINE (16AWG))
...
v25 Adds:
- Motor Side Torque tab: first-quadrant motor-side torque (lb-in) vs motor RPM.
- Separate 1-wire/phase and 2-wires/phase live plots with UI target markers.
- Supporting current and voltage diagnostic plots plus a compact feasibility snapshot table.

v26 Adds:
- Motor preset: Windings_AASLB_VS_6in using Windings ALT rotor topology electrical constants.
  Ke uses the explicit ALT supplier value (peak line-line Back-EMF over speed).
  Kt is intentionally unlinked and uses the simulated shaft torque over RMS current value.

v26k Adds:
- Export current UI settings to JSON and import them later to restore complete setup state.

v26k.10 Adds:
- Adds optional post-filter three-phase common-mode choke (CMC) between the sine filter and heptacable.
- Adds GUI enable/disable wiring for the CMC, Würth 744839208072 defaults, rating checks, JSON persistence, and dashboard/report diagnostics.
- Models the CMC as a primarily common-mode EMI block; only Rdc and user-entered differential leakage are included in the fundamental power-chain voltage drop.

v26k.13 Adds:
- Reworks the power-chain UI so the upstream DC supply feeds the surface inverter.
- Moves Vdc into the DC supply block and removes the duplicate DC-link slider from the inverter block.
- Clarifies that the surface inverter block only converts the upstream Vdc into available AC fundamental voltage using modulation and utilization.

v26k.12 Adds:
- Adds DC-link supply current/power gating for motoring feasibility and I/F LUT generation.
- Adds I/F Feasibility Map tab with fixed Navigator output-rpm points and supply A/B comparison.
- Adds DC-current, DC-power, and limiting-factor diagnostics to the SLIM I/F LUT table.

v26k.11 Adds:
- Adds a dedicated CMC / LTS telemetry tab.
- Keeps the CMC out of the balanced three-phase differential motor model except Rdc/leakage, but now shows the common-mode insertion-loss benefit in the LTS band and around PWM-related frequencies.
- Adds explicit plots explaining why the CMC may not move torque/voltage feasibility plots while still changing telemetry/coexistence risk.

v26k.8 Adds:
- Adds a SLIM I/F LUT tab that generates a NaviSpeed-compatible two-column table: motor mechanical RPM and Iq reference [pu].
- Includes copy-to-clipboard, Excel-like table view, visual LUT plot, and math notes for deriving the LUT from QuadScope motor-side torque/current capability.

v26k.6 Adds:
- Fixes Motor Side Torque NaN/Inf axis-limit failure by filtering non-finite envelope points before plotting.

v26k.4 Adds:
- Simplifies Concept Trade voltage budget: continuous high-speed case only, with utilization labels.
- Removes misleading Peak Back-EMF bars because TF correction is low-speed and not voltage-limited.
- Restores current-limit line and value labels in current required plot.

v26k.5 Adds:
- Fixes Motor Side Torque tab on-demand rendering and exposes render errors instead of swallowing them.
- Makes Concept Trade consume the currently edited PMSM UI values by replacing the matching concept or adding an Active UI Motor concept.
- Refreshes dependent tabs reliably after motor preset changes and when switching to Motor Side Torque / Concept Trade.

v26k.3 Adds:
- Fixes missing output-to-motor torque helper definition.
- Makes Concept Trade envelope method signature compatible with v26k.1 mission-band calls.
- Keeps Concept Trade render errors visible in the plot area.

v26k.2 Adds:
- Fixes Concept Trade blank-plot issue by adding missing axis aliases and output-to-motor torque helper.
- Concept Trade now uses explicit continuous RPM range, continuous torque band, and peak torque line.
- Continuous margin and loaded headroom are evaluated as worst case across the continuous RPM range.
- Voltage budget, current, and copper-loss views show both continuous and peak cases.
- Future Concept Trade render errors are displayed in the plot instead of being silently swallowed.
...
# -----------------------------
# Heptacable datasheet presets (7-wire families)
# -----------------------------

HEPTACABLE_PRESETS: Dict[str, Dict[str, object]] = {
    "7-46A-XS-CB": {
        "doc": "102987893D_AE 7-46A-XS-CB",
        "family": "Heptacable",
        "model_basis": "Helical conductor",
        "helical_awg": "#18 AWG",
        "center_awg": "#18 AWG",
        "r_helical_ohm_per_m": 24.0 / 1000.0,
        "r_center_ohm_per_m": 23.0 / 1000.0,
        "armor_r_ohm_per_m": 4.3 / 1000.0,
        "current_helical_arms": 1.61,
        "current_center_arms": 1.61,
        "voltage_helical_vrms": 800.0,
        "voltage_center_vrms": 1250.0,
        "cap_helical_pf_per_m": 115.0,
        "cap_center_pf_per_m": 102.0,
        "od_mm": 11.786,
        "temp_max_C": 232.0,
        "temp_min_C": -40.0,
        "notes": "Two XS GIPS armor layers; use helical-conductor values for 3-phase power-path modeling.",
    },
    "7-39AI-XXS": {
        "doc": "100870896D_AF 7-39AI-XXS",
        "family": "Heptacable",
        "model_basis": "Helical conductor",
        "helical_awg": "#20 AWG",
        "center_awg": "#19 AWG",
        "r_helical_ohm_per_m": 35.8 / 1000.0,
        "r_center_ohm_per_m": 34.1 / 1000.0,
        "armor_r_ohm_per_m": 6.0 / 1000.0,
        "current_helical_arms": 1.10,
        "current_center_arms": 1.10,
        "voltage_helical_vrms": 720.0,
        "voltage_center_vrms": 720.0,
        "cap_helical_pf_per_m": 40.0 * 3.280839895013123,
        "cap_center_pf_per_m": 40.0 * 3.280839895013123,
        "od_mm": 10.0,
        "temp_max_C": 232.0,
        "temp_min_C": -40.0,
        "notes": "All-conductor capacitance is listed as 40 pF/ft; modeled here as the same nominal value for helical and center conductors.",
    },
    "7-56ZA-SLC": {
        "doc": "104670190D_AC 7-56ZA-SLC Zilift Cable",
        "family": "Heptacable",
        "model_basis": "Helical conductor",
        "helical_awg": "#16 AWG",
        "center_awg": "#16 AWG",
        "r_helical_ohm_per_m": 14.6 / 1000.0,
        "r_center_ohm_per_m": 14.1 / 1000.0,
        "armor_r_ohm_per_m": 5.3 / 1000.0,
        "current_helical_arms": 2.60,
        "current_center_arms": 2.60,
        "voltage_helical_vrms": 778.0,
        "voltage_center_vrms": 1235.0,
        "cap_helical_pf_per_m": 148.0,
        "cap_center_pf_per_m": 131.0,
        "od_mm": 14.224,
        "temp_max_C": 191.0,
        "temp_min_C": -40.0,
        "notes": "US GIPS armored Zilift heptacable.",
    },
    "7-59ZA-SUS-SLC": {
        "doc": "105951488D_AA 7-59ZA-SUS-SLC Hepta StreamLINE Cable",
        "family": "Heptacable",
        "model_basis": "Helical conductor",
        "helical_awg": "#16 AWG",
        "center_awg": "#16 AWG",
        ...
        "temp_max_C": 191.0,
        "temp_min_C": -40.0,
        "notes": "SUS-armored Hepta StreamLINE cable with lower armor resistance and higher tensile capacity.",
    },
    "7-48 StreamLINE (16AWG)": {
        "doc": "7-48 StreamLINE (16AWG)",
        "family": "Heptacable",
        "model_basis": "Helical conductor",
        "helical_awg": "#16 AWG",
        "center_awg": "#16 AWG",
        ...
        "temp_max_C": 191.0,
        "temp_min_C": -40.0,
        "notes": "Added as a 16 AWG StreamLINE preset. Values currently mirror the existing 16 AWG Hepta StreamLINE entry until the exact 7-48 datasheet values are substituted.",
    },
    "User defined": {
        "doc": "Custom",
        "family": "Heptacable",
        "model_basis": "User defined",
        "helical_awg": "—",
        "center_awg": "—",
        "r_helical_ohm_per_m": None,
        "r_center_ohm_per_m": None,
        "armor_r_ohm_per_m": None,
        "current_helical_arms": None,
        "current_center_arms": None,
        "voltage_helical_vrms": None,
        "voltage_center_vrms": None,
        "cap_helical_pf_per_m": None,
        "cap_center_pf_per_m": None,
        "od_mm": None,
        "temp_max_C": None,
        "temp_min_C": None,
        "notes": "Manual cable properties. Length, wires/phase, and temperature profile remain scenario inputs even when a preset is selected.",
    },
}
...
@dataclass
class SupplyParams:
    """Upstream DC supply feeding the surface inverter.

    The DC supply defines the inverter DC-link voltage available at the surface
    box input and the companion DC current / real-power limit. The surface
    inverter then converts this Vdc into the available AC fundamental voltage
    according to the selected PWM modulation and utilization factor.
    """
    enabled: bool = True
    preset: str = "1000 V / 4.0 A"  # "615 V / 1.3 A" | "1000 V / 4.0 A" | "Custom"
    dc_current_limit_a: float = 4.0
    enforce_dc_current_limit: bool = True
    inverter_efficiency: float = 0.95
    dc_power_margin: float = 0.90
    aux_power_w: float = 0.0
    include_mechanical_power: bool = True
...
@dataclass
class CommonModeChokeParams:
    """Optional three-phase common-mode choke after the sine filter and before the heptacable.

    Physical placement:
        surface inverter → sine filter L/C → CMC → heptacable → downhole motor

    Modeling intent:
      - The CMC is primarily a common-mode EMI / telemetry-coexistence block.
      - A balanced 3-phase motor fundamental should see only winding Rdc plus any differential
        leakage inductance. The datasheet CM inductance is therefore NOT added as a normal
        differential-mode motor-drive inductance.
      - The user can optionally enter leakage inductance if measured/estimated.

    Defaults are for Würth Elektronik WE-TPB HV 3-phase CMC, order code 744839208072:
      Lcm = 208 mH, Irated = 7.2 A, Rdc = 85 mΩ, Vrated = 760 Vac, Vtest = 3000 Vac.
    """
    enabled: bool = True
    part_number: str = "Würth 744839208072"

    # Common-mode properties / ratings from the datasheet
    cm_l_mh: float = 208.0
    rated_current_arms: float = 7.2
    rdc_ohm_per_phase: float = 0.085
    rated_voltage_vac: float = 760.0
    insulation_test_vac: float = 3000.0

    # Differential-mode approximation used by the fundamental motor power model
    include_rdc_in_diff_model: bool = True
    include_leakage_in_diff_model: bool = False
    leakage_l_mh_per_phase: float = 0.0

    # Optional feasibility checks
    enforce_current_rating: bool = True
    enforce_voltage_rating: bool = False



# -----------------------------
# CMC telemetry / common-mode helper functions
# -----------------------------
# The Würth 744839208072 datasheet provides a typical insertion-loss plot rather
# than a full broadband equivalent circuit.  The points below are a conservative
# digitized engineering approximation of the plotted typical common-mode curve.
# They are used only for telemetry/coexistence visualization.  The motor-drive
# fundamental solver still treats the CMC correctly as mostly invisible to a
# balanced differential 3-phase current, except for Rdc and user-entered leakage.
_CMC_744839208072_FREQ_MHZ = np.array([
    0.001, 0.002, 0.005, 0.010, 0.020, 0.030, 0.050,
    0.100, 0.200, 0.500, 1.000, 2.000, 5.000, 10.000
], dtype=float)
_CMC_744839208072_CM_DB = np.array([
    24.0, 30.0, 38.0, 45.0, 52.0, 55.0, 53.0,
    47.0, 40.0, 31.0, 24.0, 18.0, 10.0, 5.0
], dtype=float)


def cmc_common_mode_attenuation_db(freq_hz, cmc=None) -> np.ndarray:
    """Return estimated/common-mode insertion loss in dB for the post-filter CMC.

    This is intentionally separated from the power-chain solver.  It is an EMI / LTS
    coexistence indicator, not a differential-mode voltage-drop model.
    """
    f_hz = np.asarray(freq_hz, dtype=float)
    out = np.zeros_like(f_hz, dtype=float)
    if cmc is not None and not bool(getattr(cmc, "enabled", False)):
        return out
    f_mhz = np.clip(f_hz / 1.0e6, _CMC_744839208072_FREQ_MHZ[0], _CMC_744839208072_FREQ_MHZ[-1])
    out = np.interp(np.log10(f_mhz), np.log10(_CMC_744839208072_FREQ_MHZ), _CMC_744839208072_CM_DB)

    # If user changes Lcm away from the Würth default, give a gentle engineering scaling.
    # This is not a substitute for measured insertion loss, but it prevents the UI from
    # falsely showing the exact same curve for a very different user-defined choke.
    if cmc is not None:
        L = max(1e-9, float(getattr(cmc, "cm_l_mh", 208.0)))
        scale_db = 20.0 * math.log10(L / 208.0)
        out = np.clip(out + scale_db, 0.0, 80.0)
    return out


def cmc_common_mode_linear_ratio(freq_hz, cmc=None) -> np.ndarray:
    """Return residual common-mode voltage ratio after the CMC, using insertion loss."""
    db = cmc_common_mode_attenuation_db(freq_hz, cmc)
    return np.power(10.0, -db / 20.0)


def cmc_db_at(freq_hz: float, cmc=None) -> float:
    return float(cmc_common_mode_attenuation_db(np.array([float(freq_hz)]), cmc)[0])
...
@dataclass
class TargetParams:
    # Continuous rotation operating speed at the CCRS output.
    # Existing name retained for backward compatibility with the solver and JSON files.
    # In Concept Trade this is treated as the upper end of the continuous RPM range.
    out_rpm: float = 1.0

    # Lower end of the continuous-rotation RPM range used by Concept Trade.
    cont_rpm_min: float = 0.5

    # Peak / TF-correction output torque requirement.
    # Existing name retained for backward compatibility with the solver and JSON files.
    out_torque_ftlbf: float = 1000.0

    # Low-speed TF-correction / peak-torque speed point at the CCRS output.
    tf_correction_rpm: float = 0.05

    # Continuous-rotation output torque range used by the Concept Trade tab.
    cont_torque_min_ftlbf: float = 250.0
    cont_torque_max_ftlbf: float = 350.0

    # If enabled, treat the Peak Torque value as the active solve torque target.
    # Concept Trade uses the explicit continuous torque range above.
...
class SystemParams:
    ...
    cmc: CommonModeChokeParams = field(default_factory=CommonModeChokeParams)
...
class SystemModel:
    ...
    def regen_cap_output_torque_ftlbf(self, out_rpm_mag: float, cable: CableParams) -> float:
        ...
        # Include sine-filter *series* impedance and post-filter CMC differential R/leakage in the
        # conservative regen cable limit (sine-filter shunt cap and CMC common-mode behavior ignored here).
        sf = getattr(p, "sine_filter", None)
        cmc = getattr(p, "cmc", None)
        ...
        if cmc is not None and bool(getattr(cmc, "enabled", False)):
            if bool(getattr(cmc, "include_rdc_in_diff_model", True)):
                R += float(getattr(cmc, "rdc_ohm_per_phase", 0.0))
            if bool(getattr(cmc, "include_leakage_in_diff_model", False)):
                L += float(getattr(cmc, "leakage_l_mh_per_phase", 0.0)) * 1e-3
    ...
    # -----------------------------
    # Optional post-filter common-mode choke (CMC)
    # -----------------------------
    def _cmc(self) -> CommonModeChokeParams:
        # Backward compatible if older settings are loaded without this block
        return getattr(self.p, "cmc", CommonModeChokeParams())

    def cmc_diff_series_impedance_mag(self, motor_rpm: float) -> float:
        """Differential-mode series impedance magnitude added by the post-filter CMC.

        The CMC common-mode inductance is intentionally not used as normal motor-drive
        differential inductance. Only Rdc and optional leakage L are included.
        """
        cmc = self._cmc()
        if not bool(getattr(cmc, "enabled", False)):
            return 0.0

        R = float(getattr(cmc, "rdc_ohm_per_phase", 0.0)) if bool(getattr(cmc, "include_rdc_in_diff_model", True)) else 0.0
        L = (float(getattr(cmc, "leakage_l_mh_per_phase", 0.0)) * 1e-3) if bool(getattr(cmc, "include_leakage_in_diff_model", False)) else 0.0
        R = max(0.0, R)
        L = max(0.0, L)
        if R <= 0.0 and L <= 0.0:
            return 0.0
    ...
    def cmc_series_drop_phase_rms(self, motor_rpm: float, i_load_phase_rms: float) -> float:
        """Approx phase RMS drop from the post-filter CMC differential Rdc/leakage path."""
        z = self.cmc_diff_series_impedance_mag(motor_rpm)
        if z <= 0.0:
            return 0.0
        return float(abs(i_load_phase_rms)) * float(z)

    def post_filter_node_phase_rms(self, motor_rpm: float, v_cable_input_phase_rms: float,
                                   i_load_phase_rms: float) -> float:
        """Voltage at sine-filter output / CMC input.

        v_cable_input_phase_rms is the voltage required at the heptacable input
        after the CMC. The CMC drop is inserted between the sine filter and cable.
        """
        return float(v_cable_input_phase_rms) + self.cmc_series_drop_phase_rms(motor_rpm, i_load_phase_rms)
    ...
    def inverter_current_mag_rms(self, motor_rpm: float, v_node_phase_rms: float, i_load_phase_rms: float) -> float:
        ...
        v_node_phase_rms is the voltage required at the heptacable input after the CMC.
        If the CMC is enabled, the sine-filter shunt capacitor sees the post-filter
        node voltage before the CMC.
        """
        sf = self._sf()
        if not bool(getattr(sf, "enabled", False)):
            return float(abs(i_load_phase_rms))

        v_post_filter = self.post_filter_node_phase_rms(motor_rpm, v_node_phase_rms, i_load_phase_rms)
        i_in, i_q, _ = self.sine_filter_shunt_current_components(motor_rpm, v_post_filter)
    ...
    def inverter_voltage_required_phase_rms(self, motor_rpm: float, v_node_phase_rms: float,
                                            i_load_phase_rms: float) -> float:
        """Approx required inverter phase RMS voltage (L-N).

        Chain used here:
            inverter → sine-filter series L/R → sine-filter output node → CMC → heptacable → motor

        v_node_phase_rms is the heptacable-input voltage after the CMC.
        """
        v_post_filter = self.post_filter_node_phase_rms(motor_rpm, v_node_phase_rms, i_load_phase_rms)

        sf = self._sf()
        if not bool(getattr(sf, "enabled", False)):
            return float(v_post_filter)
        ...
        return float(v_post_filter) + float(i_inv) * float(z_s)
    ...
    def motor_copper_loss_w(self, i_mag_rms: float) -> float:
        return 3.0 * (float(i_mag_rms) ** 2) * float(self.rs_effective_ohm())

    def cmc_diff_loss_w(self, i_load_rms: float) -> float:
        cmc = self._cmc()
        if not bool(getattr(cmc, "enabled", False)):
            return 0.0
        if not bool(getattr(cmc, "include_rdc_in_diff_model", True)):
            return 0.0
        return 3.0 * (float(i_load_rms) ** 2) * max(0.0, float(getattr(cmc, "rdc_ohm_per_phase", 0.0)))

    def sine_filter_series_loss_w(self, motor_rpm: float, v_node_phase_rms: float, i_load_phase_rms: float) -> float:
        sf = self._sf()
        if not bool(getattr(sf, "enabled", False)):
            return 0.0
        rf = max(0.0, float(getattr(sf, "rf_ohm", 0.0)))
        if rf <= 0.0:
            return 0.0
        i_inv = self.inverter_current_mag_rms(motor_rpm, v_node_phase_rms, i_load_phase_rms)
        return 3.0 * (float(i_inv) ** 2) * rf

    def dc_available_power_w(self) -> float:
        sp = getattr(self.p, "supply", SupplyParams())
        vdc = max(0.0, float(getattr(self.p.vf, "vdc_link_v", 0.0)))
        idc = max(0.0, float(getattr(sp, "dc_current_limit_a", 0.0)))
        margin = float(getattr(sp, "dc_power_margin", 1.0))
        margin = float(np.clip(margin, 0.0, 1.0))
        return vdc * idc * margin

    def dc_input_power_w(self, motor_rpm: float, id_rms: float, iq_rms: float, cable: CableParams) -> float:
        """Estimated DC-side motoring power for the commanded dq current.

        This is intentionally conservative for feasibility/LUT gating. It includes
        motor copper, cable copper, CMC Rdc, sine-filter series R, optional mechanical
        shaft power, and user-entered auxiliary power. Reactive power is not counted
        as real DC power, but it still affects voltage/inverter-current limits elsewhere.
        """
        sp = getattr(self.p, "supply", SupplyParams())
        im = math.sqrt(float(id_rms) * float(id_rms) + float(iq_rms) * float(iq_rms))
        v_motor = self.motor_voltage_required_phase_rms(motor_rpm, float(id_rms), float(iq_rms))
        v_drop = self.cable_drop_phase_rms(motor_rpm, im, cable)
        v_node = float(v_motor) + float(v_drop)

        p_real = 0.0
        p_real += self.motor_copper_loss_w(im)
        p_real += self.cable_loss_w(im, cable)
        p_real += self.cmc_diff_loss_w(im)
        p_real += self.sine_filter_series_loss_w(motor_rpm, v_node, im)

        if bool(getattr(sp, "include_mechanical_power", True)):
            t_em = max(0.0, float(self.kt_effective_nm_per_arms()) * float(iq_rms))
            omega_m = abs(rpm_to_rad_s(float(motor_rpm)))
            p_real += t_em * omega_m

        eff = float(getattr(sp, "inverter_efficiency", 0.95))
        eff = float(np.clip(eff, 0.05, 1.0))
        p_dc = p_real / eff + max(0.0, float(getattr(sp, "aux_power_w", 0.0)))
        return max(0.0, float(p_dc))

    def dc_input_current_a(self, motor_rpm: float, id_rms: float, iq_rms: float, cable: CableParams) -> float:
        vdc = max(1e-9, float(getattr(self.p.vf, "vdc_link_v", 0.0)))
        return self.dc_input_power_w(motor_rpm, id_rms, iq_rms, cable) / vdc

    def dc_power_margin_w(self, motor_rpm: float, id_rms: float, iq_rms: float, cable: CableParams) -> float:
        return self.dc_available_power_w() - self.dc_input_power_w(motor_rpm, id_rms, iq_rms, cable)

    def supply_gate_enabled(self) -> bool:
        sp = getattr(self.p, "supply", SupplyParams())
        return bool(getattr(sp, "enabled", False)) and bool(getattr(sp, "enforce_dc_current_limit", False))

    def supply_limit_residual_w(self, motor_rpm: float, id_rms: float, iq_rms: float, cable: CableParams) -> float:
        """Positive value means the point exceeds the active DC supply gate."""
        if not self.supply_gate_enabled():
            return -float("inf")
        return self.dc_input_power_w(motor_rpm, id_rms, iq_rms, cable) - self.dc_available_power_w()

    def infer_limit_reason(self, motor_rpm: float, id_rms: float, iq_rms: float, i_mag_rms: float, cable: CableParams) -> str:
        reasons = []
        tol = 1e-3
        i_lim = float(cable.i_phase_limit())
        if i_mag_rms >= 0.985 * max(1e-12, i_lim):
            reasons.append("phase/cable current")
        v_motor = self.motor_voltage_required_phase_rms(motor_rpm, id_rms, iq_rms)
        v_drop = self.cable_drop_phase_rms(motor_rpm, i_mag_rms, cable)
        v_node = float(v_motor) + float(v_drop)
        v_inv = self.inverter_voltage_required_phase_rms(motor_rpm, v_node, i_mag_rms)
        _, v_cmd, _ = self._v_cmd_phase(motor_rpm)
        if v_inv >= 0.985 * max(1e-12, float(v_cmd)):
            reasons.append("surface voltage")
        v_dh = self._effective_downhole_phase_limit()
        if v_dh is not None and v_motor >= 0.985 * max(1e-12, float(v_dh)):
            reasons.append("downhole voltage")
        if self.supply_gate_enabled() and self.dc_power_margin_w(motor_rpm, id_rms, iq_rms, cable) <= tol * max(1.0, self.dc_available_power_w()):
            reasons.append("DC supply current/power")
        if not reasons:
            return "margin"
        return " + ".join(reasons)
    ...
    def _max_iq_no_fw(self, motor_rpm: float, cable: CableParams) -> Tuple[
        ...
        if self.supply_gate_enabled():
            def f_supply(iq: float) -> float:
                return self.supply_limit_residual_w(motor_rpm, 0.0, float(iq), cable)
            iq_supply = self._bisect_root(f_supply, 0.0, float(i_lim))
        else:
            iq_supply = float(i_lim)

        iq_max = min(i_lim, iq_volt, iq_dh, iq_inv, iq_supply)
        # with Id=0: i_mag = iq
    ...
    def max_iq_given_limits(self, motor_rpm: float, cable: CableParams) -> Tuple[
        ...
        for id_rms in id_candidates:
            ...
            if self.supply_gate_enabled():
                def f_supply(iq: float) -> float:
                    return self.supply_limit_residual_w(motor_rpm, float(id_rms), float(iq), cable)
                iq_supply = self._bisect_root(f_supply, 0.0, float(iq_hi))
            else:
                iq_supply = float(iq_hi)

            iq_max_id = min(iq_hi, iq_volt, iq_dh, iq_inv, iq_supply)
    ...
    def max_iq_given_limits_fast(self, motor_rpm: float, cable: CableParams) -> Tuple[
        ...
        def violates(iq: float) -> bool:
            ...
            if v_dh_phase is not None and v_motor > v_dh_phase + 1e-12:
                return True
            if self.supply_gate_enabled() and self.supply_limit_residual_w(motor_rpm, id_try, iq, cable) > 1e-9:
                return True
            return False
    ...
    def find_feasible_id_for_iq(self, motor_rpm: float, iq_req: float, cable: CableParams) -> Tuple[
        ...
        for id_rms in id_candidates:
            ...
            if self.supply_gate_enabled() and self.supply_limit_residual_w(motor_rpm, id_rms, iq_req, cable) > 1e-9:
                continue
    ...
    def solve_target(self, cable_override: Optional[CableParams] = None) -> SolveResult:
        ...
        # Voltage at the heptacable input after the CMC.
        # Chain: inverter → sine-filter series → sine-filter output node → CMC → heptacable → motor.
        v_node_req = float(v_motor_req) + float(v_drop)
        v_cmc_drop = self.cmc_series_drop_phase_rms(motor_rpm, float(im_rep))
        v_post_filter_req = self.post_filter_node_phase_rms(motor_rpm, v_node_req, float(im_rep))

        # Optional sine filter adds inverter-side current and voltage demand.
        # The shunt capacitor is at the sine-filter output, before the CMC.
        i_inverter_rms = self.inverter_current_mag_rms(motor_rpm, v_node_req, float(im_rep))
        _, _, i_filter_cap_rms = self.sine_filter_shunt_current_components(motor_rpm, v_post_filter_req)

        v_inverter_req = self.inverter_voltage_required_phase_rms(motor_rpm, v_node_req, float(im_rep))
        v_filter_drop = max(0.0, float(v_inverter_req) - float(v_post_filter_req))
        fails: List[str] = []
        notes: List[str] = []
        ...
        cmc = self._cmc()
        if bool(getattr(cmc, "enabled", False)):
            if bool(getattr(cmc, "enforce_current_rating", True)):
                cmc_i_lim = float(getattr(cmc, "rated_current_arms", 0.0))
                if cmc_i_lim > 0.0 and abs(float(im_rep)) > cmc_i_lim + 1e-12:
                    fails.append(
                        f"CMC current rating exceeded: |I|={im_rep:.3f} Arms > Irated={cmc_i_lim:.3f} Arms")
            if bool(getattr(cmc, "enforce_voltage_rating", False)):
                cmc_v_lim = float(getattr(cmc, "rated_voltage_vac", 0.0))
                cmc_v_ll = math.sqrt(3.0) * max(float(v_node_req), float(v_post_filter_req))
                if cmc_v_lim > 0.0 and cmc_v_ll > cmc_v_lim + 1e-12:
                    fails.append(
                        f"CMC voltage rating exceeded: Vll≈{cmc_v_ll:.1f} Vrms > Vrated={cmc_v_lim:.1f} Vac")
        ...
        v_motor_avail = max(0.0, v_cmd - self.cable_drop_phase_rms(motor_rpm, im_rep, cable)
                            - self.cmc_series_drop_phase_rms(motor_rpm, im_rep)
                            - max(0.0, float(v_filter_drop)))
        omega_m = rpm_to_rad_s(motor_rpm)
        omega_e = max(1, int(mp.pole_pairs)) * omega_m
        if omega_e > 1e-9:
        ...
        return SolveResult(
            ...
            v_cmc_drop=v_cmc_drop,
            v_post_filter_req=v_post_filter_req,
...
class CmcTelemetryPane(QWidget):
    """CMC / LTS coexistence visualization pane.

    Shows why a post-filter CMC may have almost no impact on the torque/voltage
    envelope while still being a meaningful telemetry/noise mitigation block.
    """
    ...
    def render(self, model: SystemModel, params: SystemParams, res=None):
        p = params
        cmc = getattr(p, "cmc", CommonModeChokeParams())
        enabled = bool(getattr(cmc, "enabled", False))

        for ax in (self.ax1, self.ax2, self.ax3, self.ax4):
            ax.clear()

        f = np.logspace(3, 7, 300)  # 1 kHz to 10 MHz
        a_on = cmc_common_mode_attenuation_db(f, cmc)
        a_off = np.zeros_like(a_on)
        a_plot = a_on if enabled else a_off

        # Plot 1: common-mode insertion loss
        self.ax1.semilogx(f, a_plot, linewidth=2, label="CMC enabled" if enabled else "CMC bypassed")
        self.ax1.axvspan(5e3, 15e3, alpha=0.12, label="LTS band approx. 5–15 kHz")
        for ff, lab in [(5e3, "DL ~5 kHz"), (10e3, "UL ~10 kHz"), (20e3, "PWM 20 kHz")]:
            self.ax1.axvline(ff, linestyle=":", linewidth=1.4)
            self.ax1.text(ff, max(2.0, self.ax1.get_ylim()[1] * 0.08), lab, rotation=90, va="bottom", ha="right", fontsize=8)
        self.ax1.set_title("Post-filter CMC: common-mode insertion loss")
        self.ax1.set_xlabel("Frequency [Hz]")
        self.ax1.set_ylabel("Attenuation [dB]")
        self.ax1.grid(True, which="both", alpha=0.3)
        self.ax1.legend(fontsize=8, loc="best")

        # Plot 2: residual CM voltage ratio in telemetry band
        f_lts = np.linspace(3e3, 20e3, 240)
        ratio = cmc_common_mode_linear_ratio(f_lts, cmc) if enabled else np.ones_like(f_lts)
        self.ax2.plot(f_lts / 1000.0, ratio, linewidth=2)
        self.ax2.axvspan(5.0, 15.0, alpha=0.12)
        self.ax2.axvline(5.0, linestyle=":", linewidth=1.4)
        self.ax2.axvline(10.0, linestyle=":", linewidth=1.4)
        self.ax2.set_title("Estimated CM noise reaching heptacable")
        self.ax2.set_xlabel("Frequency [kHz]")
        self.ax2.set_ylabel("Residual CM voltage ratio")
        self.ax2.set_ylim(0.0, 1.05)
        self.ax2.grid(True, alpha=0.3)

        # Plot 3: differential-mode penalty; usually intentionally tiny
        rpm_max = max(100.0, float(getattr(p.target, "out_rpm", 1.0)) * float(p.gearbox.ratio()) * 1.2)
        motor_rpm = np.linspace(0.0, rpm_max, 200)
        i_used = 0.0
        if res is not None:
            i_used = float(getattr(res, "i_mag_rms", getattr(res, "iq_req_rms", 0.0)))
        if not np.isfinite(i_used) or i_used <= 0.0:
            try:
                i_used = float(p.cable.i_phase_limit())
            except Exception:
                i_used = 1.0
        vdrop = np.array([model.cmc_series_drop_phase_rms(float(r), i_used) for r in motor_rpm], dtype=float)
        self.ax3.plot(motor_rpm, vdrop, linewidth=2, label=f"I={i_used:.2f} Arms")
        self.ax3.set_title("Differential-mode voltage penalty")
        self.ax3.set_xlabel("Motor mechanical RPM")
        self.ax3.set_ylabel("CMC Vdrop [Vrms phase]")
        self.ax3.grid(True, alpha=0.3)
        self.ax3.legend(fontsize=8, loc="best")

        # Plot 4: summary text
        self.ax4.axis("off")
        a5 = cmc_db_at(5e3, cmc) if enabled else 0.0
        a10 = cmc_db_at(10e3, cmc) if enabled else 0.0
        a20 = cmc_db_at(20e3, cmc) if enabled else 0.0
        r5 = 10.0 ** (-a5 / 20.0)
        r10 = 10.0 ** (-a10 / 20.0)
        r20 = 10.0 ** (-a20 / 20.0)
        vdrop_now = 0.0
        if res is not None:
            vdrop_now = float(getattr(res, "v_cmc_drop", 0.0))
        text = (
            f"CMC state: {'ENABLED' if enabled else 'BYPASSED'}\n"
            f"Part: {getattr(cmc, 'part_number', 'CMC')}\n"
            f"Lcm={float(getattr(cmc, 'cm_l_mh', 0.0)):.1f} mH, "
            f"Rdc={float(getattr(cmc, 'rdc_ohm_per_phase', 0.0)):.4f} Ω/phase, "
            f"Irated={float(getattr(cmc, 'rated_current_arms', 0.0)):.2f} Arms\n\n"
            "Why torque plots barely move:\n"
            "• Balanced 3-phase motor current is differential mode.\n"
            "• A 3-phase CMC is primarily common-mode, so its CM flux cancels for balanced motor current.\n"
            "• The solver only adds Rdc and optional leakage Ldm to the motor voltage budget.\n\n"
            "Expected telemetry/coexistence effect:\n"
            f"• CM attenuation @ 5 kHz ≈ {a5:.1f} dB  → residual ≈ {100.0*r5:.2f}%\n"
            f"• CM attenuation @ 10 kHz ≈ {a10:.1f} dB → residual ≈ {100.0*r10:.2f}%\n"
            f"• CM attenuation @ 20 kHz ≈ {a20:.1f} dB → residual ≈ {100.0*r20:.2f}%\n\n"
            f"Current operating-point CMC differential Vdrop ≈ {vdrop_now:.4f} Vrms/phase.\n"
            "If the lab issue is LTS loss/noise, evaluate this tab, not the torque envelope."
        )
        self.ax4.text(0.02, 0.98, text, va="top", ha="left", fontsize=9.5, family="monospace",
                      bbox=dict(boxstyle="round,pad=0.45", facecolor="white", alpha=0.95, edgecolor="#D0D5DD"))
        self.fig.tight_layout(pad=2.0)
        self.canvas.draw_idle()


class ConceptTradePane(QWidget):
    """Trade-space pane for the four motor concept comparison.

    This pane is intentionally separate from the normal operating envelope tabs.
    It compares the four motor concepts on the same current UI system basis:
      - Overlay motor-side torque-speed envelopes
      - Continuous margin vs voltage headroom and peak torque at TF-correction RPM
      - Torque margin vs high-speed voltage headroom
      - Ke-Kt design map with burden indication
      - High-speed voltage budget indicator
      - Target-current and copper-loss mini-plots
    """
    ...
    def __init__(self, parent: Optional[QWidget] = None):
        ...
        self.ax_trade = self.fig.add_subplot(gs[0, 2])

        # v26k.1 names. Keep old aliases because earlier Concept Trade revisions used them.
        self.ax_cont = self.ax_zoom
        self.ax_peak = self.ax_trade

        self.ax_kekt = self.fig.add_subplot(gs[1, 0])
        self.ax_voltage = self.fig.add_subplot(gs[1, 1])
        gs_metrics = gs[1, 2].subgridspec(nrows=2, ncols=1, hspace=0.38)
        self.ax_iq = self.fig.add_subplot(gs_metrics[0, 0])
        self.ax_pcu = self.fig.add_subplot(gs_metrics[1, 0])

        # Backward-compatible alias: old code may still reference ax_burden.
        self.ax_burden = self.ax_iq

        self.fig.subplots_adjust(left=0.055, right=0.985, top=0.93, bottom=0.08)
...
class ApplesPane(QWidget):
    """Apples-to-apples comparison pane.

    Compares cable presets and wiring choices at the same fixed output requirement
    so losses and current demand are directly comparable.
    """
...
class MotorSideTorquePane(QWidget):
    """First-quadrant motor-side torque-speed pane.

    This pane intentionally mirrors the Feasibility tab idea, but it stays on the
    motor side of the gearbox:
      - x-axis: motor mechanical RPM
      - y-axis: motor-side torque in lb-in
      - separate first-quadrant plots for 1-wire/phase and 2-wires/phase
      - UI target markers update live with the rest of the app

    v26k.6:
      - Hardens plotting against NaN/Inf values from voltage/current-limited
        envelope points. Matplotlib raises "Axis limits cannot be NaN or Inf"
        when axis limits are computed from non-finite arrays; this pane now
        filters non-finite points before plotting and before setting limits.
      - Keeps the table alive even if one solve result contains a non-finite
        value.
    """
    ...
    def __init__(self, parent: Optional[QWidget] = None):
        ...
        gs = self.fig.add_gridspec(
            nrows=2,
            ncols=3,
            width_ratios=[1.55, 1.55, 1.05],
            height_ratios=[2.1, 1.0],
            hspace=0.28,
            wspace=0.20,
        )

        self.ax_1w = self.fig.add_subplot(gs[0, 0])
        self.ax_2w = self.fig.add_subplot(gs[0, 1])
        self.ax_i = self.fig.add_subplot(gs[1, 0])
        self.ax_v = self.fig.add_subplot(gs[1, 1])
        self.ax_tbl = self.fig.add_subplot(gs[:, 2])

        self.fig.subplots_adjust(left=0.055, right=0.99, top=0.94, bottom=0.07)

        self.canvas = FigureCanvas(self.fig)
        self.canvas.setSizePolicy(QSizePolicy.Expanding, QSizePolicy.Expanding)
        layout.addWidget(self.canvas)

        # Hover/crosshair on the two torque-speed plots. Re-refreshed after each render.
        self.interactor = QuadrantInteractor(self.canvas, [self.ax_1w, self.ax_2w], pixel_tol=12)

    @staticmethod
    def _nm_to_lbin(x_nm: float) -> float:
        return float(x_nm) / LBIN_TO_NM

    @staticmethod
    def _safe_arr(d: Dict[str, np.ndarray], key: str) -> np.ndarray:
        try:
            a = np.asarray(d.get(key, np.array([])), dtype=float)
        except Exception:
    ...
    @staticmethod
    def _finite_max(values, default: float = 1.0) -> float:
        vals = []
        for v in values:
            try:
    ...
    @staticmethod
    def _make_table(ax, title: str, columns: List[str], rows: List[List[str]], fontsize: int = 8):
        ax.set_axis_off()
        ax.set_title(title, loc="left", fontsize=11, pad=8, fontweight="600")

        tbl = ax.table(cellText=rows, colLabels=columns, loc="center", cellLoc="center", colLoc="center")
        tbl.auto_set_font_size(False)
        tbl.set_fontsize(fontsize)
    ...
    def _plot_motor_side_axis(self, ax, metrics: Dict[str, np.ndarray], res: Optional[SolveResult], label: str):
        motor_rpm = self._safe_arr(metrics, "motor_rpm")
        t_em_lbin = self._safe_arr(metrics, "t_motor_em_nm") / LBIN_TO_NM
        t_use_lbin = self._safe_arr(metrics, "t_motor_use_nm") / LBIN_TO_NM

        x_use, y_use = self._finite_pair(motor_rpm, t_use_lbin)
        x_em, y_em = self._finite_pair(motor_rpm, t_em_lbin)

        if x_use.size:
            ax.plot(x_use, y_use, linewidth=2.2, label=f"Useful shaft torque ({label})")
        if x_em.size:
            ax.plot(x_em, y_em, linewidth=1.6, linestyle=":", label=f"Raw EM torque ({label})")

        if (not x_use.size) and (not x_em.size):
            ax.text(
                0.5,
                0.52,
                "No finite motor-side torque envelope\nfor the current limits/settings.",
                ha="center",
                va="center",
                transform=ax.transAxes,
                fontsize=10,
                bbox=dict(boxstyle="round,pad=0.35", facecolor="white", alpha=0.92, edgecolor="#cbd5e1"),
            )

        x_max = self._finite_max([x_use, x_em, 1.0], default=1.0)
        y_max = self._finite_max([y_use, y_em, 1.0], default=1.0)

        if res is not None:
            try:
                rpm_ui = abs(float(res.motor_rpm))
                t_load_lbin = self._nm_to_lbin(float(res.motor_torque_nm))
                t_total_lbin = self._nm_to_lbin(float(res.motor_torque_total_nm))
                if np.isfinite(rpm_ui) and np.isfinite(t_total_lbin):
                    ax.scatter([rpm_ui], [t_total_lbin], marker="o", s=58, label="UI required total torque")
                    ax.annotate("UI target", (rpm_ui, t_total_lbin), textcoords="offset points", xytext=(7, 7), fontsize=9)
                    x_max = max(x_max, rpm_ui)
                    y_max = max(y_max, t_total_lbin)
                if np.isfinite(rpm_ui) and np.isfinite(t_load_lbin):
                    ax.scatter([rpm_ui], [t_load_lbin], marker="x", s=58, label="UI load torque before losses")
                    x_max = max(x_max, rpm_ui)
                    y_max = max(y_max, t_load_lbin)
            except Exception:
                pass

        ax.set_title(f"Motor-side torque-speed envelope — {label}")
        ax.set_xlabel("Motor mechanical RPM")
        ax.set_ylabel("Motor-side torque (lb-in)")
        ax.grid(True, alpha=0.28)
        if ax.lines or ax.collections:
            ax.legend(loc="best", fontsize=8, framealpha=0.88)

        # Always set finite, positive axis limits.  This is the specific guard for
        # "Axis limits cannot be NaN or Inf".
        if not np.isfinite(x_max) or x_max <= 0.0:
            x_max = 1.0
        if not np.isfinite(y_max) or y_max <= 0.0:
            y_max = 1.0
        ax.set_xlim(0.0, max(1.0, x_max * 1.05))
        ax.set_ylim(0.0, max(1.0, y_max * 1.14))
        return float(x_max), float(y_max)

    def _plot_finite_line(self, ax, x, y, *args, **kwargs):
        xp, yp = self._finite_pair(x, y)
        if xp.size:
            ax.plot(xp, yp, *args, **kwargs)
            return True
        return False

    def _plot_finite_scatter(self, ax, x, y, *args, **kwargs):
        xp, yp = self._finite_pair(x, y)
        if xp.size:
            ax.scatter(xp, yp, *args, **kwargs)
            return True
        return False
    ...
    def render(
        ...
        for ax in (self.ax_1w, self.ax_2w, self.ax_i, self.ax_v, self.ax_tbl):
            ax.cla()

        # Main first-quadrant motor-side torque-speed plots.
        self._plot_motor_side_axis(self.ax_1w, m1, res_1w, "1 wire/phase")
        self._plot_motor_side_axis(self.ax_2w, m2, res_2w, "2 wires/phase")

        # Keep both main panes on the same visible scale for quick comparison.
        try:
            lims = [self.ax_1w.get_xlim()[1], self.ax_2w.get_xlim()[1], self.ax_1w.get_ylim()[1], self.ax_2w.get_ylim()[1]]
            if all(np.isfinite(float(v)) for v in lims):
                xmax = max(float(self.ax_1w.get_xlim()[1]), float(self.ax_2w.get_xlim()[1]), 1.0)
                ymax = max(float(self.ax_1w.get_ylim()[1]), float(self.ax_2w.get_ylim()[1]), 1.0)
                self.ax_1w.set_xlim(0.0, xmax)
                self.ax_2w.set_xlim(0.0, xmax)
                self.ax_1w.set_ylim(0.0, ymax)
                self.ax_2w.set_ylim(0.0, ymax)
        except Exception:
            pass

        x1 = self._safe_arr(m1, "motor_rpm")
        x2 = self._safe_arr(m2, "motor_rpm")

        # Supporting current diagnostic in motor RPM coordinates.
        any_i = False
        any_i |= self._plot_finite_line(self.ax_i, x1, self._safe_arr(m1, "iq_max"), linewidth=2, label="Iq max — 1w")
        any_i |= self._plot_finite_line(self.ax_i, x1, self._safe_arr(m1, "i_mag"), linestyle=":", label="|I| at envelope — 1w")
        any_i |= self._plot_finite_line(self.ax_i, x1, self._safe_arr(m1, "i_limit"), linestyle="--", label="I limit — 1w")
        any_i |= self._plot_finite_line(self.ax_i, x2, self._safe_arr(m2, "iq_max"), linewidth=2, label="Iq max — 2w")
        any_i |= self._plot_finite_line(self.ax_i, x2, self._safe_arr(m2, "i_mag"), linestyle=":", label="|I| at envelope — 2w")
        any_i |= self._plot_finite_line(self.ax_i, x2, self._safe_arr(m2, "i_limit"), linestyle="--", label="I limit — 2w")
        if res_1w is not None:
            any_i |= self._plot_finite_scatter(self.ax_i, [abs(float(res_1w.motor_rpm))], [float(res_1w.iq_req_rms)], s=42, marker="o", label="UI Iq req — 1w")
        if res_2w is not None:
            any_i |= self._plot_finite_scatter(self.ax_i, [abs(float(res_2w.motor_rpm))], [float(res_2w.iq_req_rms)], s=42, marker="o", label="UI Iq req — 2w")
        self.ax_i.set_title("Current capability vs motor RPM")
        self.ax_i.set_xlabel("Motor mechanical RPM")
        self.ax_i.set_ylabel("Arms")
        self.ax_i.grid(True, alpha=0.28)
        if any_i:
            self.ax_i.legend(loc="best", fontsize=7.5, framealpha=0.88)
        else:
            self.ax_i.text(0.5, 0.5, "No finite current data", ha="center", va="center", transform=self.ax_i.transAxes)
            self.ax_i.set_xlim(0.0, 1.0)
            self.ax_i.set_ylim(0.0, 1.0)

        # Supporting voltage diagnostic in motor RPM coordinates.
        any_v = False
        any_v |= self._plot_finite_line(self.ax_v, x1, self._safe_arr(m1, "vll_motor"), linewidth=2, label="Motor VLL req — 1w")
        any_v |= self._plot_finite_line(self.ax_v, x1, self._safe_arr(m1, "emf_ll"), linestyle=":", label="Back-EMF VLL — 1w")
        any_v |= self._plot_finite_line(self.ax_v, x2, self._safe_arr(m2, "vll_motor"), linewidth=2, label="Motor VLL req — 2w")
        any_v |= self._plot_finite_line(self.ax_v, x2, self._safe_arr(m2, "emf_ll"), linestyle=":", label="Back-EMF VLL — 2w")
        try:
            vll_lim = float(getattr(params.limits, "downhole_vll_rms_limit", 0.0))
            if np.isfinite(vll_lim) and vll_lim > 0.0:
                self.ax_v.axhline(vll_lim, linestyle="--", linewidth=1.4, label=f"VLL limit = {vll_lim:.0f} V")
                any_v = True
        except Exception:
            pass
        if res_1w is not None:
            any_v |= self._plot_finite_scatter(self.ax_v, [abs(float(res_1w.motor_rpm))], [math.sqrt(3.0) * float(res_1w.v_motor_req)], s=42, marker="o", label="UI VLL — 1w")
        if res_2w is not None:
            any_v |= self._plot_finite_scatter(self.ax_v, [abs(float(res_2w.motor_rpm))], [math.sqrt(3.0) * float(res_2w.v_motor_req)], s=42, marker="o", label="UI VLL — 2w")
        self.ax_v.set_title("Motor terminal voltage vs motor RPM")
        self.ax_v.set_xlabel("Motor mechanical RPM")
        self.ax_v.set_ylabel("VLL,rms (V)")
        self.ax_v.grid(True, alpha=0.28)
        if any_v:
            self.ax_v.legend(loc="best", fontsize=7.5, framealpha=0.88)
        else:
            self.ax_v.text(0.5, 0.5, "No finite voltage data", ha="center", va="center", transform=self.ax_v.transAxes)
            self.ax_v.set_xlim(0.0, 1.0)
            self.ax_v.set_ylim(0.0, 1.0)

        # Compact snapshot table.
        def _row(name: str, res: Optional[SolveResult]) -> List[str]:
            if res is None:
                return [name, "—", "—", "—", "—", "—", "—", "—"]
            try:
                vll = math.sqrt(3.0) * float(res.v_motor_req)
            except Exception:
                vll = float("nan")
            feas = "YES" if bool(getattr(res, "feasible", False)) else "NO"
            return [
                name,
                self._fmt(abs(float(getattr(res, "motor_rpm", float("nan")))), "{:.0f}"),
                self._fmt(self._nm_to_lbin(float(getattr(res, "motor_torque_nm", float("nan")))), "{:.2f}"),
                self._fmt(self._nm_to_lbin(float(getattr(res, "tau_extra_nm", float("nan")))), "{:.2f}"),
                self._fmt(self._nm_to_lbin(float(getattr(res, "motor_torque_total_nm", float("nan")))), "{:.2f}"),
                f"{self._fmt(float(getattr(res, 'iq_req_rms', float('nan'))), '{:.2f}')} / {self._fmt(float(getattr(res, 'iq_max_rms', float('nan'))), '{:.2f}')}",
                self._fmt(vll, "{:.1f}"),
                feas,
            ]

        kt_lbin_per_arms = float(params.motor.kt_nm_per_arms) / LBIN_TO_NM
        try:
            kt_lbin_per_arms = float(SystemModel(params).kt_effective_nm_per_arms()) / LBIN_TO_NM
        except Exception:
            pass

        rows = [
            _row("1 wire/phase", res_1w),
            _row("2 wires/phase", res_2w),
            ["Gear ratio", self._fmt(float(params.gearbox.ratio()), "{:.1f}:1"), "", "", "", "", "", ""],
            ["Kt effective", self._fmt(kt_lbin_per_arms, "{:.2f} lb-in/Arms"), "", "", "", "", "", ""],
        ]
        cols = ["Case", "Motor RPM", "Tload", "Textra", "Ttotal", "Iq req/max", "VLL", "OK"]
        tbl = self._make_table(self.ax_tbl, "Live motor-side snapshot", cols, rows, fontsize=7.3)

        # Color the feasibility cells in the table.
        try:
            for r in (1, 2):
                cell = tbl[(r, 7)]
                val = cell.get_text().get_text().strip().upper()
                cell.set_facecolor("#dcfce7" if val == "YES" else "#fee2e2")
                cell.get_text().set_fontweight("600")
        except Exception:
            pass

        # Make hover/crosshair aware of the new artists after clearing/replotting.
        try:
...
class SlimIfLutPane(QWidget):
    """SLIM I/F LUT generator for copy/paste into NaviSpeed.

    The NaviSpeed Rev83 SLIM I/F LUT tab accepts two columns:
      1) motor mechanical speed [rpm]
      2) Iq reference [pu]

    In firmware/preview, effective commanded current is:
        Iq_arms = Iq_pu * CurrentBase_Arms_per_pu * NMotors * DemandDuty / 100

    This pane keeps the first two table columns paste-compatible and places
    diagnostics to the right so the user can copy only what NaviSpeed needs.
    """
    ...
    def __init__(self, parent: Optional[QWidget] = None):
        ...
        hdr = QLabel(
            "SLIM I/F LUT generator. Copy only the first two columns directly into NaviSpeed → Standalone SLIM I/f LUT. The default strategy now uses the active QuadScope peak/TF-correction output torque target. "
            "Column 1 is motor mechanical speed [rpm]. Column 2 is Iq reference [pu]."
        )
        hdr.setWordWrap(True)
        hdr.setStyleSheet("font-size: 12px; color:#111827; padding: 4px;")
        outer.addWidget(hdr)

        ctrl = QGroupBox("LUT setup")
        ...
        self.strategy.addItems([
            "Peak torque / TF correction load",
            "Peak-to-continuous blended load",
            "Balanced continuous load + losses",
            "Conservative continuous load + losses",
            "Startup boosted continuous load",
            "Capability fraction / bring-up",
            "Flat diagnostic current",
        ])

        self.wiring = QComboBox()
        self.wiring.addItems(["Use active QuadScope wiring", "1 wire/phase", "2 wires/phase"])

        self.load_basis = QComboBox()
        self.load_basis.addItems([
            "Use active peak torque target",
            "Use active continuous max torque",
            "Manual output torque",
        ])

        def _spin(minv, maxv, step, val, dec=3, suffix=""):
            w = QDoubleSpinBox()
            w.setDecimals(dec)
            w.setRange(minv, maxv)
            w.setSingleStep(step)
        ...
        self.max_rpm = _spin(100.0, 20000.0, 100.0, 7000.0, 1, " rpm")
        self.manual_output_torque = _spin(0.0, 5000.0, 50.0, 1250.0, 1, " ft-lbf")
        self.current_base = _spin(0.01, 100.0, 0.1, 2.0, 3, " Arms/pu")
        self.n_motors = _spin(0.1, 20.0, 0.1, 4.0, 2, "")
        self.duty = _spin(1.0, 200.0, 1.0, 100.0, 1, " %")
        self.margin = _spin(1.00, 3.00, 0.05, 1.15, 2, " ×")
        self.startup_boost_pu = _spin(0.0, 1.0, 0.01, 0.02, 4, " pu")
        self.capability_fraction = _spin(0.01, 1.00, 0.05, 0.30, 2, " ×")
        self.flat_iq_arms = _spin(0.0, 20.0, 0.1, 1.20, 3, " Arms")
        self.min_iq_pu = _spin(0.0, 1.0, 0.01, 0.00, 4, " pu")
        self.max_iq_pu = _spin(0.01, 2.0, 0.01, 0.25, 4, " pu")

        labels = [
            ("Strategy", self.strategy), ("Load basis", self.load_basis), ("Manual output torque", self.manual_output_torque), ("Max motor rpm", self.max_rpm),
            ("Wiring basis", self.wiring), ("CurrentBase", self.current_base), ("NMotors", self.n_motors), ("DemandDuty", self.duty),
            ("Torque margin", self.margin), ("Startup boost", self.startup_boost_pu), ("Capability fraction", self.capability_fraction), ("Flat Iq", self.flat_iq_arms),
            ("Min Iq pu", self.min_iq_pu), ("Max Iq pu", self.max_iq_pu),
        ]
        for k, (lab, w) in enumerate(labels):
            r = k // 4
            c = (k % 4) * 2
            grid.addWidget(QLabel(lab), r, c)
            grid.addWidget(w, r, c + 1)

        btn_row = QHBoxLayout()
        self.btn_copy = QPushButton("Copy NaviSpeed 2-column LUT")
        self.btn_select = QPushButton("Select first two columns")
        self.btn_recompute = QPushButton("Recompute LUT")
        btn_row.addWidget(self.btn_copy)
        btn_row.addWidget(self.btn_select)
        btn_row.addWidget(self.btn_recompute)
        btn_row.addStretch(1)
        grid.addLayout(btn_row, 4, 0, 1, 8)
        ...
        self.table = QTableWidget(25, 14)
        self.table.setHorizontalHeaderLabels([
            "Speed [rpm]", "Output rpm", "Iq Ref [pu]", "Iq eff [Arms]", "Output torque [ft-lbf]",
            "Req motor torque [lb-in]", "Useful capability [lb-in]", "Util [%]", "Iq max [Arms]",
            "Id best [Arms]", "Idc est [A]", "Pdc est [W]", "Limit", "Notes"
        ])
        ...
        self.copy_text = QPlainTextEdit()
        self.copy_text.setReadOnly(False)
        self.copy_text.setMaximumHeight(125)
        self.copy_text.setStyleSheet("font-family:Consolas,Menlo,monospace; font-size:11px;")
        left_lay.addWidget(QLabel("Paste-compatible text block (same content copied by the button):"))
        left_lay.addWidget(self.copy_text)
        ...
        self.ax_lut = self.fig.add_subplot(2, 1, 1)
        self.ax_current = self.fig.add_subplot(2, 1, 2)
        self.fig.subplots_adjust(left=0.11, right=0.98, top=0.94, bottom=0.10, hspace=0.32)
        self.canvas = FigureCanvas(self.fig)
        self.canvas.setSizePolicy(QSizePolicy.Expanding, QSizePolicy.Expanding)
        right_lay.addWidget(self.canvas)

        self.math = QPlainTextEdit()
        self.math.setReadOnly(True)
        self.math.setMaximumHeight(190)
        self.math.setStyleSheet("font-family:Consolas,Menlo,monospace; font-size:11px;")
        right_lay.addWidget(self.math)
        ...
        self._last_rows: List[Dict[str, float]] = []
        self._last_clipboard_text = ""
        self._model: Optional[SystemModel] = None
        self._params: Optional[SystemParams] = None

        self.btn_copy.clicked.connect(self.copy_lut_to_clipboard)
        self.btn_select.clicked.connect(self.select_first_two_columns)
        self.btn_recompute.clicked.connect(self._recompute_from_cached)
        for w in (
            self.strategy, self.load_basis, self.manual_output_torque, self.wiring, self.max_rpm, self.current_base, self.n_motors, self.duty,
            self.margin, self.startup_boost_pu, self.capability_fraction, self.flat_iq_arms,
            self.min_iq_pu, self.max_iq_pu,
        ):
            try:
                if isinstance(w, QComboBox):
                    w.currentIndexChanged.connect(self._recompute_from_cached)
                else:
                    w.valueChanged.connect(self._recompute_from_cached)
            except Exception:
                pass

    @staticmethod
    def _fmt(v: float, fmt: str = "{:.6g}") -> str:
        try:
            x = float(v)
    ...
    def select_first_two_columns(self):
        try:
    ...
    def copy_lut_to_clipboard(self):
        try:
            QApplication.clipboard().setText(self._last_clipboard_text or self.copy_text.toPlainText())
        except Exception:
            pass

    def _recompute_from_cached(self, *_):
        if self._model is not None and self._params is not None:
            self.render(self._model, self._params)

    def _selected_cable(self, params: SystemParams) -> CableParams:
        cab = CableParams(**vars(params.cable))
        basis = self.wiring.currentText()
        if basis.startswith("1"):
            cab.wires_per_phase = 1
        elif basis.startswith("2"):
            cab.wires_per_phase = 2
        return cab

    @staticmethod
    def _motor_useful_torque_for_output(params: SystemParams, out_ftlbf: float) -> float:
        G = max(1e-9, float(params.gearbox.ratio()))
        eta = max(1e-9, float(params.gearbox.eff_total()))
        t_gb_in_nm = ft_lbf_to_nm(abs(float(out_ftlbf))) / max(1e-9, G * eta)
        mc = getattr(params, "mag_coupler", None)
        if (mc is not None) and bool(getattr(mc, "enabled", False)):
            t_motor_nm = float(getattr(mc, "t_break_nm", 0.0)) + (
                t_gb_in_nm / max(1e-9, float(getattr(mc, "slope", 1.0)))
            )
        else:
            t_motor_nm = t_gb_in_nm
        return max(0.0, float(t_motor_nm))

    def _active_output_torque_ftlbf(self, params: SystemParams, continuous: bool = False) -> float:
        """Return the output torque target used to build the SLIM I/F LUT."""
        basis = self.load_basis.currentText()
        if basis.startswith("Manual"):
            return max(0.0, float(self.manual_output_torque.value()))
        if continuous or basis.startswith("Use active continuous"):
            return max(0.0, float(getattr(params.target, "cont_torque_max_ftlbf", 0.0)))
        return max(0.0, float(getattr(params.target, "out_torque_ftlbf", 0.0)))

    def _speed_dependent_output_torque_ftlbf(self, params: SystemParams, rpm: float, rpm_max: float, strategy: str) -> tuple[float, str]:
        """Map motor rpm to the output torque target for the selected LUT strategy.

        Peak torque is a low-speed TF-correction/stall requirement. Continuous torque is the
        steady rotation requirement. The blended option uses peak current in the pull-in /
        low-speed region and then tapers toward the continuous requirement.
        """
        peak = self._active_output_torque_ftlbf(params, continuous=False)
        cont = max(0.0, float(getattr(params.target, "cont_torque_max_ftlbf", peak)))

        # If the user explicitly selected continuous as the load basis, do not override it with peak.
        if self.load_basis.currentText().startswith("Use active continuous"):
            peak = cont

        if strategy.startswith("Balanced") or strategy.startswith("Conservative") or strategy.startswith("Startup"):
            return cont, "continuous"

        if strategy.startswith("Peak-to-continuous"):
            G = max(1.0, float(params.gearbox.ratio()))
            tf_rpm = max(0.0, float(getattr(params.target, "tf_correction_rpm", 0.05))) * G
            r1 = max(250.0, tf_rpm * 2.0)
            r2 = max(r1 + 1.0, min(max(1500.0, tf_rpm * 8.0), 0.35 * max(1.0, rpm_max)))
            if rpm <= r1:
                return peak, "peak"
            if rpm >= r2:
                return cont, "continuous"
            a = (rpm - r1) / max(1e-9, r2 - r1)
            # smoothstep taper
            a = a * a * (3.0 - 2.0 * a)
            return peak * (1.0 - a) + cont * a, "blend"

        # Default: system peak / TF-correction torque target across the generated LUT.
        return peak, "peak"

    def _make_rows(self, model: SystemModel, params: SystemParams) -> List[Dict[str, float]]:
        npts = 25
        rpm_max = max(1.0, float(self.max_rpm.value()))
        speeds = np.linspace(0.0, rpm_max, npts)
        scale = max(1e-9, float(self.current_base.value()) * float(self.n_motors.value()) * float(self.duty.value()) * 0.01)
        min_pu = float(self.min_iq_pu.value())
        max_pu = max(min_pu, float(self.max_iq_pu.value()))
        margin = max(1.0, float(self.margin.value()))
        cab = self._selected_cable(params)
        strategy = self.strategy.currentText()

        kt_eff = max(1e-12, float(model.kt_effective_nm_per_arms()))
        rows: List[Dict[str, float]] = []
        for rpm in speeds:
            rpm = float(rpm)
            try:
                iq_max, id_best, im_best, *_ = model.max_iq_given_limits(rpm, cab)
                iq_max = max(0.0, float(iq_max))
            except Exception:
                iq_max = 0.0
            try:
                tau_extra_nm, _, _ = model.tau_extra_nm(rpm_to_rad_s(rpm))
                tau_extra_nm = max(0.0, float(tau_extra_nm))
            except Exception:
                tau_extra_nm = 0.0

            cap_total_nm = max(0.0, kt_eff * iq_max)
            cap_use_nm = max(0.0, cap_total_nm - tau_extra_nm)
            note = ""

            out_tq_ftlbf, load_note = self._speed_dependent_output_torque_ftlbf(params, rpm, rpm_max, strategy)
            motor_use_nm = self._motor_useful_torque_for_output(params, out_tq_ftlbf)

            if strategy.startswith("Capability"):
                iq_req_arms = iq_max * float(self.capability_fraction.value())
                req_total_nm = kt_eff * iq_req_arms
                out_tq_ftlbf = 0.0
                note = f"{float(self.capability_fraction.value()):.0%} of available Iq"
            elif strategy.startswith("Flat"):
                iq_req_arms = max(0.0, float(self.flat_iq_arms.value()))
                req_total_nm = kt_eff * iq_req_arms
                out_tq_ftlbf = 0.0
                note = "flat diagnostic"
            else:
                local_margin = margin
                if strategy.startswith("Conservative"):
                    local_margin *= 1.15
                    note = "+15% conservative; " + load_note
                elif strategy.startswith("Startup"):
                    note = "low-speed boost; " + load_note
                else:
                    note = load_note
                req_total_nm = (motor_use_nm + tau_extra_nm) * local_margin
                iq_req_arms = req_total_nm / kt_eff

            iq_pu_raw = iq_req_arms / scale
            if strategy.startswith("Startup") and rpm <= min(1000.0, 0.20 * rpm_max):
                # taper the boost from full at zero speed to zero at the threshold
                rlim = max(1e-9, min(1000.0, 0.20 * rpm_max))
                iq_pu_raw += float(self.startup_boost_pu.value()) * max(0.0, 1.0 - rpm / rlim)
                iq_req_arms = iq_pu_raw * scale
                req_total_nm = kt_eff * iq_req_arms

            # Respect user pu clamp and also avoid asking beyond modeled Iq capability.
            cap_pu = iq_max / scale if scale > 1e-12 else max_pu
            iq_pu = min(max(iq_pu_raw, min_pu), max_pu, max(0.0, cap_pu))
            iq_eff_arms = iq_pu * scale
            util = 100.0 * iq_eff_arms / iq_max if iq_max > 1e-9 else 0.0
            if iq_pu < iq_pu_raw - 1e-9:
                note = (note + "; " if note else "") + "clamped"

            out_rpm = rpm / max(1e-9, float(params.gearbox.ratio()))
            try:
                # The LUT Iq is motor-side q-axis torque current; feasibility includes motor voltage + cable drop + CMC + sine filter + DC supply gate.
                id_diag = 0.0
                pdc_est = model.dc_input_power_w(rpm, id_diag, iq_eff_arms, cab)
                idc_est = model.dc_input_current_a(rpm, id_diag, iq_eff_arms, cab)
                limit_note = model.infer_limit_reason(rpm, float(id_best), float(iq_max), float(im_best), cab)
            except Exception:
                pdc_est = 0.0
                idc_est = 0.0
                limit_note = "—"
            ...
            rows.append({
                ...
                "out_rpm": out_rpm,
                "iq_pu": iq_pu,
                "iq_eff_arms": iq_eff_arms,
                "out_tq_ftlbf": out_tq_ftlbf,
                "req_lbin": req_total_nm / LBIN_TO_NM,
                "cap_lbin": cap_use_nm / LBIN_TO_NM,
                "util_pct": util,
                "iq_max": iq_max,
                "id_best": float(id_best),
                "idc_est": idc_est,
                "pdc_est": pdc_est,
                "limit": limit_note,
                "note": note,
    ...
    def render(self, model: SystemModel, params: SystemParams):
        self._model = model
        self._params = params
        rows = self._make_rows(model, params)
        self._last_rows = rows

        self.table.setRowCount(len(rows))
        for r, row in enumerate(rows):
            vals = [
                self._fmt(row["speed_rpm"], "{:.1f}"),
                self._fmt(row.get("out_rpm", 0.0), "{:.3f}"),
                self._fmt(row["iq_pu"], "{:.9f}"),
                self._fmt(row["iq_eff_arms"], "{:.4f}"),
                self._fmt(row.get("out_tq_ftlbf", 0.0), "{:.1f}"),
                self._fmt(row["req_lbin"], "{:.3f}"),
                self._fmt(row["cap_lbin"], "{:.3f}"),
                self._fmt(row["util_pct"], "{:.1f}"),
                self._fmt(row["iq_max"], "{:.3f}"),
                self._fmt(row.get("id_best", 0.0), "{:.3f}"),
                self._fmt(row.get("idc_est", 0.0), "{:.3f}"),
                self._fmt(row.get("pdc_est", 0.0), "{:.1f}"),
                str(row.get("limit", "")),
                str(row.get("note", "")),
            ]
        ...
        text = "\n".join(f"{row['speed_rpm']:.1f}\t{row['iq_pu']:.9f}" for row in rows)
        self._last_clipboard_text = text
        self.copy_text.setPlainText(text)

        speeds = np.array([row["speed_rpm"] for row in rows], dtype=float)
        iq_pu = np.array([row["iq_pu"] for row in rows], dtype=float)
        iq_eff = np.array([row["iq_eff_arms"] for row in rows], dtype=float)
        iq_max = np.array([row["iq_max"] for row in rows], dtype=float)
        cap_lbin = np.array([row["cap_lbin"] for row in rows], dtype=float)
        req_lbin = np.array([row["req_lbin"] for row in rows], dtype=float)

        self.ax_lut.clear()
        self.ax_lut.set_title("SLIM I/F LUT: NaviSpeed paste columns")
        self.ax_lut.set_xlabel("Motor mechanical speed [rpm]")
        self.ax_lut.set_ylabel("Iq Ref [pu]")
        self.ax_lut.plot(speeds, iq_pu, marker="o", label="Iq Ref [pu]")
        self.ax_lut.grid(True, alpha=0.28)
        self.ax_lut.legend(loc="best", fontsize=8)

        self.ax_current.clear()
        self.ax_current.set_title("Current / torque check against QuadScope motor-side capability")
        self.ax_current.set_xlabel("Motor mechanical speed [rpm]")
        self.ax_current.set_ylabel("Current [Arms] / Torque [lb-in]")
        self.ax_current.plot(speeds, iq_eff, marker="o", label="Effective Iq [Arms]")
        self.ax_current.plot(speeds, iq_max, linestyle="--", label="Modeled Iq max [Arms]")
        self.ax_current.plot(speeds, req_lbin, linestyle=":", label="Req total torque [lb-in]")
        self.ax_current.plot(speeds, cap_lbin, linestyle="-.", label="Useful torque capability [lb-in]")
        self.ax_current.grid(True, alpha=0.28)
        self.ax_current.legend(loc="best", fontsize=7)
        self.canvas.draw_idle()

        scale = float(self.current_base.value()) * float(self.n_motors.value()) * float(self.duty.value()) * 0.01
        try:
            kt_lbin = float(model.kt_effective_nm_per_arms()) / LBIN_TO_NM
        except Exception:
            kt_lbin = float(params.motor.kt_nm_per_arms) / LBIN_TO_NM
        math_lines = [
            "SLIM I/F LUT math used here:",
            "  Iq_effective[Arms] = Iq_LUT[pu] × CurrentBase[Arms/pu] × NMotors × DemandDuty/100",
            f"  Current scale in this tab = {scale:.4f} Arms per 1.0 pu",
            f"  Kt_eff used for torque mapping = {kt_lbin:.4f} lb-in/Arms",
            "  Motor torque required from system target:",
            "    T_motor[Nm] = T_output[ft-lbf] × 1.35582 / (gear_ratio × gearbox_efficiency)",
            "    If magnetic-coupler breakaway is enabled, the breakaway torque is added on the motor side.",
            "  Required Iq[Arms] = margin × (T_motor + τ_extra) / Kt_eff",
            "  Capability shown here is the useful motor-side torque left after modeled speed-dependent losses.",
            "  Final Iq_LUT[pu] is clamped by Min/Max pu and by modeled Iq capability.",
            "  Iq is treated as motor-side q-axis torque current; the capability check includes motor voltage + cable drop + CMC + sine-filter demand.",
            "  When the DC supply gate is enabled, Iq capability is also clamped by estimated DC input current/power.",
            "",
            f"Active gear ratio = {float(params.gearbox.ratio()):.3f}; active gearbox efficiency = {float(params.gearbox.eff_total()):.3f}",
            f"Active peak output torque = {float(getattr(params.target, 'out_torque_ftlbf', 0.0)):.1f} ft-lbf; continuous max = {float(getattr(params.target, 'cont_torque_max_ftlbf', 0.0)):.1f} ft-lbf",
            "",
            "Recommended use:",
            "  1) Use Peak torque / TF correction load when checking low-speed torque requirement.",
            "  2) Use Peak-to-continuous blended load when you want high pull-in current but lower high-speed current.",
            "  3) Use Capability fraction / bring-up for safe no-load spin testing only; it is not a system-load LUT.",
        ]
        self.math.setPlainText("\n".join(math_lines))


class IfFeasibilityPane(QWidget):
    """Navigator I/F-focused forward feasibility map."""
    ...
    def __init__(self, parent: Optional[QWidget] = None):
        ...
        outer.setContentsMargins(10, 10, 10, 10)
        outer.setSpacing(8)
        hdr = QLabel("I/F feasibility map. Iq is motor-side q-axis current; max Iq includes motor voltage, cable drop, sine filter, CMC differential drop, current limits, and the active DC supply gate.")
        hdr.setWordWrap(True)
        hdr.setStyleSheet("font-size: 12px; color:#111827; padding: 4px;")
        outer.addWidget(hdr)
        self.table = QTableWidget(6, 16)
        self.table.setHorizontalHeaderLabels([
            "Out rpm", "Motor rpm", "Elec Hz", "EMF VLL", "Iq max", "Id best",
            "Motor Tq [lb-in]", "Output Tq [ft-lbf]", "Idc est [A]", "Pdc est [W]",
            "Pdc margin [W]", "Iq@1000", "Iq@Target", "Feasible 1000?", "Feasible Target?", "Limit"
        ])
        self.table.horizontalHeader().setSectionResizeMode(QHeaderView.ResizeToContents)
        self.table.horizontalHeader().setStretchLastSection(True)
        self.table.setAlternatingRowColors(True)
        outer.addWidget(self.table)
        self.fig = Figure(figsize=(10.5, 7.0), dpi=100)
        self.ax_iq = self.fig.add_subplot(2, 2, 1)
        self.ax_tq = self.fig.add_subplot(2, 2, 2)
        self.ax_idc = self.fig.add_subplot(2, 2, 3)
        self.ax_margin = self.fig.add_subplot(2, 2, 4)
        self.fig.subplots_adjust(left=0.08, right=0.98, top=0.93, bottom=0.09, wspace=0.27, hspace=0.36)
        self.canvas = FigureCanvas(self.fig)
        self.canvas.setSizePolicy(QSizePolicy.Expanding, QSizePolicy.Expanding)
        ...
        self.notes.setStyleSheet("font-family:Consolas,Menlo,monospace; font-size:11px;")
    ...
    @staticmethod
    def _iq_for_output_torque(model: SystemModel, params: SystemParams, out_ftlbf: float) -> float:
        G = max(1e-9, float(params.gearbox.ratio()))
        eta = max(1e-9, float(params.gearbox.eff_total()))
        kt = max(1e-12, float(model.kt_effective_nm_per_arms()))
        t_motor_nm = ft_lbf_to_nm(abs(float(out_ftlbf))) / (G * eta)
        return t_motor_nm / kt

    def _rows_for(self, model: SystemModel, params: SystemParams, cable: CableParams):
        speeds = [0.05, 0.25, 0.50, 1.00, 1.50, 2.00]
        rows = []
        G = max(1e-9, float(params.gearbox.ratio()))
        eta = max(1e-9, float(params.gearbox.eff_total()))
        kt = max(1e-12, float(model.kt_effective_nm_per_arms()))
        target_tq = max(0.0, float(getattr(params.target, "out_torque_ftlbf", 1250.0)))
        iq_1000 = self._iq_for_output_torque(model, params, 1000.0)
        iq_target = self._iq_for_output_torque(model, params, target_tq)
        for out_rpm in speeds:
            motor_rpm = float(out_rpm) * G
            iq_max, id_best, im_best, f_e, *_ = model.max_iq_given_limits(motor_rpm, cable)
            iq_max = max(0.0, float(iq_max))
            id_best = float(id_best)
            im_best = float(im_best)
            tau_extra, _, _ = model.tau_extra_nm(rpm_to_rad_s(motor_rpm))
            t_em_nm = kt * iq_max
            t_use_nm = max(0.0, t_em_nm - max(0.0, float(tau_extra)))
            t_gb_in, _ = model._mag_forward_transmitted_to_gb_nm(t_use_nm)
            t_out_ftlbf = nm_to_ft_lbf(t_gb_in * G * eta)
            pdc = model.dc_input_power_w(motor_rpm, id_best, iq_max, cable)
            idc = model.dc_input_current_a(motor_rpm, id_best, iq_max, cable)
            p_margin = model.dc_power_margin_w(motor_rpm, id_best, iq_max, cable)
            emf_vll = float(params.motor.ke_vll_rms_per_krpm) * (motor_rpm / 1000.0)
            limit = model.infer_limit_reason(motor_rpm, id_best, iq_max, im_best, cable)
            rows.append({
                "out_rpm": out_rpm, "motor_rpm": motor_rpm, "f_e": f_e, "emf_vll": emf_vll,
                "iq_max": iq_max, "id_best": id_best, "motor_tq_lbin": (t_em_nm / LBIN_TO_NM),
                "out_tq_ftlbf": t_out_ftlbf, "idc": idc, "pdc": pdc, "p_margin": p_margin,
                "iq_1000": iq_1000, "iq_target": iq_target,
                "feas_1000": iq_max >= iq_1000 - 1e-9, "feas_target": iq_max >= iq_target - 1e-9,
                "limit": limit,
            })
        return rows
    ...
    def _render_table(self, rows):
        ...
        for r, row in enumerate(rows):
            vals = [
                self._fmt(row["out_rpm"], "{:.2f}"), self._fmt(row["motor_rpm"], "{:.0f}"),
                self._fmt(row["f_e"], "{:.1f}"), self._fmt(row["emf_vll"], "{:.0f}"),
                self._fmt(row["iq_max"], "{:.3f}"), self._fmt(row["id_best"], "{:.3f}"),
                self._fmt(row["motor_tq_lbin"], "{:.2f}"), self._fmt(row["out_tq_ftlbf"], "{:.0f}"),
                self._fmt(row["idc"], "{:.2f}"), self._fmt(row["pdc"], "{:.0f}"),
                self._fmt(row["p_margin"], "{:.0f}"), self._fmt(row["iq_1000"], "{:.3f}"),
                self._fmt(row["iq_target"], "{:.3f}"), "YES" if row["feas_1000"] else "NO",
                "YES" if row["feas_target"] else "NO", row["limit"],
            ]
            for c, val in enumerate(vals):
    ...
    def _supply_variant_rows(self, params: SystemParams, cable: CableParams, vdc: float, idc: float):
        p2 = copy.deepcopy(params)
        if not hasattr(p2, "supply"):
            p2.supply = SupplyParams()
        p2.vf.vdc_link_v = float(vdc)
        p2.supply.enabled = True
        p2.supply.enforce_dc_current_limit = True
        p2.supply.dc_current_limit_a = float(idc)
        return self._rows_for(SystemModel(p2), p2, cable)

    def render(self, model: SystemModel, params: SystemParams):
        cab = CableParams(**vars(params.cable))
        rows = self._rows_for(model, params, cab)
        self._render_table(rows)
        x = np.array([r["out_rpm"] for r in rows], dtype=float)
        iq = np.array([r["iq_max"] for r in rows], dtype=float)
        tq = np.array([r["out_tq_ftlbf"] for r in rows], dtype=float)
        idc = np.array([r["idc"] for r in rows], dtype=float)
        p_margin = np.array([r["p_margin"] for r in rows], dtype=float)
        rows_615 = self._supply_variant_rows(params, cab, 615.0, 1.3)
        rows_1000 = self._supply_variant_rows(params, cab, 1000.0, 4.0)
        iq_615 = np.array([r["iq_max"] for r in rows_615], dtype=float)
        iq_1000 = np.array([r["iq_max"] for r in rows_1000], dtype=float)
        tq_615 = np.array([r["out_tq_ftlbf"] for r in rows_615], dtype=float)
        tq_1000 = np.array([r["out_tq_ftlbf"] for r in rows_1000], dtype=float)
        for ax in (self.ax_iq, self.ax_tq, self.ax_idc, self.ax_margin):
            ax.clear(); ax.grid(True, alpha=0.28)
        self.ax_iq.set_title("Max motor-side Iq vs output rpm")
        self.ax_iq.plot(x, iq, marker="o", label="Active supply")
        self.ax_iq.plot(x, iq_615, marker="s", linestyle="--", label="615V / 1.3A")
        self.ax_iq.plot(x, iq_1000, marker="^", linestyle="--", label="1000V / 4A")
        self.ax_iq.axhline(rows[0]["iq_1000"], linestyle=":", label="Iq for 1000 ft-lbf")
        self.ax_iq.axhline(rows[0]["iq_target"], linestyle="-.", label="Iq for target")
        self.ax_iq.set_xlabel("Output rpm"); self.ax_iq.set_ylabel("Iq max [Arms]"); self.ax_iq.legend(fontsize=7)
        self.ax_tq.set_title("Output torque capability")
        self.ax_tq.plot(x, tq, marker="o", label="Active supply")
        self.ax_tq.plot(x, tq_615, marker="s", linestyle="--", label="615V / 1.3A")
        self.ax_tq.plot(x, tq_1000, marker="^", linestyle="--", label="1000V / 4A")
        self.ax_tq.axhline(1000.0, linestyle=":", label="1000 ft-lbf")
        self.ax_tq.axhline(float(getattr(params.target, "out_torque_ftlbf", 1250.0)), linestyle="-.", label="Target")
        self.ax_tq.set_xlabel("Output rpm"); self.ax_tq.set_ylabel("Output torque [ft-lbf]"); self.ax_tq.legend(fontsize=7)
        self.ax_idc.set_title("Estimated DC current at active max-Iq point")
        self.ax_idc.plot(x, idc, marker="o", label="Idc est")
        self.ax_idc.axhline(float(getattr(params.supply, "dc_current_limit_a", 0.0)), linestyle="--", label="Idc limit")
        self.ax_idc.set_xlabel("Output rpm"); self.ax_idc.set_ylabel("DC current [A]"); self.ax_idc.legend(fontsize=7)
        self.ax_margin.set_title("DC power margin at active max-Iq point")
        self.ax_margin.plot(x, p_margin, marker="o"); self.ax_margin.axhline(0.0, linestyle="--")
        self.ax_margin.set_xlabel("Output rpm"); self.ax_margin.set_ylabel("Pdc margin [W]")
        self.canvas.draw_idle()
        sp = getattr(params, "supply", SupplyParams())
        self.notes.setPlainText("\n".join([
            "How to read this tab:",
            "  Start with output rpm, then read maximum feasible motor-side Iq after all active gates.",
            "  If Iq max is above Iq@1000 or Iq@Target, that torque point is feasible at that speed.",
            "  The LUT should not command more than Iq max at that speed.",
            "",
            f"Active supply gate: enabled={bool(getattr(sp, 'enabled', False))}, enforced={bool(getattr(sp, 'enforce_dc_current_limit', False))}, Vdc={float(params.vf.vdc_link_v):.0f} V, Idc_limit={float(getattr(sp, 'dc_current_limit_a', 0.0)):.2f} A, usable Pdc={model.dc_available_power_w():.0f} W",
            f"Cable: Rphase={cab.effective_r_phase():.3f} ohm, Lphase={cab.effective_l_phase():.6f} H, Iphase_limit={cab.i_phase_limit():.3f} Arms",
        ]))
...
class DashboardPane(QWidget):
    ...
    the architecture blocks (surface → sine filter → CMC → cable → contact → motor → gearbox → loads).
    ...
    def __init__(self):
        ...
        add_block('Voltages (phase RMS)', [
            ('vbudget', 'Vcmd / Vreq(inv) / Vpost-filter / Vnode / Vmotor / drops'),
            ('vlimits', 'Limits (surface / downhole phase / downhole VLL)'),
        ])

        add_block('Losses + filter + CMC', [
    ...
    def update_from_status(self, full_text: str, p, res_sel, res_1, res_2, dominant=None):
        ...
        self._set('vbudget',
                  f"Vcmd={float(getattr(res, 'v_surface_cmd', 0.0)):.1f} (≈{ll_surface_cmd:.1f} Vll), "
                  f"Vreq(inv)={float(getattr(res, 'v_inverter_req', 0.0)):.1f} (≈{ll_inverter_req:.1f} Vll), "
                  f"Vpost-filter={float(getattr(res, 'v_post_filter_req', getattr(res, 'v_node_req', 0.0))):.1f}, "
                  f"Vnode={float(getattr(res, 'v_node_req', 0.0)):.1f}, Vmotor={float(getattr(res, 'v_motor_req', 0.0)):.1f} (≈{ll_motor_req:.1f} Vll), "
                  f"Vdrop(CMC)={float(getattr(res, 'v_cmc_drop', 0.0)):.2f}, Vdrop(cable)={float(getattr(res, 'v_cable_drop', 0.0)):.1f}")
        ...
        if (float(getattr(res, 'i_filter_cap_rms', 0.0)) > 1e-9) or (float(getattr(res, 'v_filter_drop', 0.0)) > 1e-9):
            self._set('filter',
                      f"I_load={float(getattr(res, 'i_mag_used_rms', 0.0)):.3f} Arms, I_inv≈{float(getattr(res, 'i_inverter_rms', 0.0)):.3f} Arms, I_shunt≈{float(getattr(res, 'i_filter_cap_rms', 0.0)):.3f} Arms, Vdrop(filter)≈{float(getattr(res, 'v_filter_drop', 0.0)):.2f} Vrms")
        else:
            self._set('filter', '—')
        ...
        if cmc is not None and bool(getattr(cmc, 'enabled', False)):
            self._set('cmc',
                      f"ON after sine filter | {getattr(cmc, 'part_number', 'CMC')} | Lcm={float(getattr(cmc, 'cm_l_mh', 0.0)):.1f} mH | Rdc={float(getattr(cmc, 'rdc_ohm_per_phase', 0.0)):.4f} Ω/ph | Vdrop≈{float(getattr(res, 'v_cmc_drop', 0.0)):.3f} Vrms | Irated={float(getattr(cmc, 'rated_current_arms', 0.0)):.2f} Arms")
        else:
            self._set('cmc', 'OFF / bypassed')
...
class NavigatorVfWindow(QMainWindow):
    ...
    def __init__(self):
        super().__init__()
        self.setWindowTitle("QuadScope v26k.11 - Post-Filter CMC Telemetry")
        ...
        self.mpl_motor_side = MotorSideTorquePane()
        self.tabs.addTab(self.mpl_motor_side, "Motor Side Torque")
        self.motor_side_pane = self.mpl_motor_side

        self.mpl_slim_if_lut = SlimIfLutPane()
        self.tabs.addTab(self.mpl_slim_if_lut, "SLIM I/F LUT")
        self.slim_if_lut_pane = self.mpl_slim_if_lut

        self.mpl_if_feas = IfFeasibilityPane()
        self.tabs.addTab(self.mpl_if_feas, "I/F Feasibility")
        self.if_feas_pane = self.mpl_if_feas

        self.mpl_concept_trade = ConceptTradePane()
        self.tabs.addTab(self.mpl_concept_trade, "Concept Trade")
        self.concept_trade_pane = self.mpl_concept_trade
        ...
        self.mpl_apples = ApplesPane()
        self.tabs.addTab(self.mpl_apples, "A2A")
        self.apples_pane = self.mpl_apples

        self.mpl_arch = ArchitecturePane()
        self.tabs.addTab(self.mpl_arch, "Architecture")
        self.arch_pane = self.mpl_arch

        self.mpl_cmc_lts = CmcTelemetryPane()
        self.tabs.addTab(self.mpl_cmc_lts, "CMC / LTS")
        self.cmc_lts_pane = self.mpl_cmc_lts
        ...
        self.sweep_tabs.addTab(self.sweep_sf, "Sine Filter")
        ...
        if hasattr(self, "c_preset"):
            self._apply_cable_preset(self.c_preset.currentText())
    ...
    def _build_controls(self):
        ...
        self.btn_export_json.setToolTip("Save the current UI setup to a JSON file so the full scenario can be restored later.")
        self.btn_import_json = QPushButton("Import JSON")
        self.btn_import_json.setToolTip("Load a previously exported JSON setup and restore the UI values.")
        ...
        status_row.addWidget(self.btn_export_json)
        status_row.addWidget(self.btn_import_json)
        ...
        self.t_cont_rpm_min = make_dspin(0.0, 5.0, 0.05, float(getattr(self.params.target, "cont_rpm_min", 0.5)), 3)
        ...
        self.t_tf_rpm = make_dspin(0.0, 1.0, 0.01, float(getattr(self.params.target, "tf_correction_rpm", 0.05)), 3)
        self.t_cont_tq_min = make_dspin(0.0, 2000.0, 10.0, float(getattr(self.params.target, "cont_torque_min_ftlbf", 250.0)), 1)
        self.t_cont_tq_max = make_dspin(0.0, 2000.0, 10.0, float(getattr(self.params.target, "cont_torque_max_ftlbf", 350.0)), 1)
        self.env_out_rpm_max = make_dspin(0.1, 5.0, 0.1, 1.5, 2)

        # Continuous RPM range row: Min / Max. The Max value is stored in existing out_rpm.
        cont_rpm_widget = QWidget()
        cont_rpm_layout = QHBoxLayout(cont_rpm_widget)
        cont_rpm_layout.setContentsMargins(0, 0, 0, 0)
        cont_rpm_layout.setSpacing(6)
        cont_rpm_layout.addWidget(QLabel("Min"))
        cont_rpm_layout.addWidget(self.t_cont_rpm_min)
        cont_rpm_layout.addWidget(QLabel("Max"))
        cont_rpm_layout.addWidget(self.t_out_rpm)
        G.addWidget(QLabel("Cont. RPM Range"), 0, 0)
        G.addWidget(cont_rpm_widget, 0, 1, 1, 2)

        add_slider_row(G, 1, "Peak Torque (ft-lbf)", self.t_out_tq)
        add_slider_row(G, 2, "TF correction RPM", self.t_tf_rpm)

        # Continuous torque range row: two spin boxes in one compact row.
        cont_range_widget = QWidget()
        cont_range_layout = QHBoxLayout(cont_range_widget)
        cont_range_layout.setContentsMargins(0, 0, 0, 0)
        cont_range_layout.setSpacing(6)
        cont_range_layout.addWidget(QLabel("Min"))
        cont_range_layout.addWidget(self.t_cont_tq_min)
        cont_range_layout.addWidget(QLabel("Max"))
        cont_range_layout.addWidget(self.t_cont_tq_max)
        G.addWidget(QLabel("Cont. Torque Range (ft-lbf)"), 3, 0)
        G.addWidget(cont_range_widget, 3, 1, 1, 2)

        add_slider_row(G, 4, "Envelope plot max RPM", self.env_out_rpm_max)

        self.chk_tq_override = QCheckBox("Use Peak Torque as active solve target")
        self.chk_tq_override.setToolTip("Concept Trade uses the continuous torque range separately. This checkbox only controls the main solve target behavior.")
        self.chk_tq_override.setChecked(bool(getattr(self.params.target, "torque_override_continuous", True)))
        G.addWidget(self.chk_tq_override, 5, 0, 1, 3)
        ...
        gb_vf = QGroupBox("SURFACE INVERTER / PWM STAGE (FED BY DC SUPPLY)")
        ...
        lbl_basis = QLabel("Inverter voltage-limit basis")
        self.v_entry_basis = QComboBox()
        self.v_entry_basis.addItems(["DC supply / DC link (Vdc)", "AC fundamental limit (Vrms)"])
        _vbasis = str(self.params.vf.voltage_entry_basis)
        self.v_entry_basis.setCurrentText("DC supply / DC link (Vdc)" if _vbasis.strip().upper().startswith("DC") else _vbasis)
        ...
        self.lbl_dc_bus_source = QLabel("Vdc is set in the upstream DC SUPPLY block. This inverter block only applies PWM modulation and utilization to convert that DC bus into the available AC fundamental voltage.")
        self.lbl_dc_bus_source.setWordWrap(True)
        self.lbl_dc_bus_source.setStyleSheet("color:#444;")
        V2.addWidget(self.lbl_dc_bus_source, 0, 0, 1, 3)
        ...
        # Upstream DC supply feeding the surface inverter
        gb_supply = QGroupBox("UPSTREAM DC SUPPLY FEEDING SURFACE INVERTER")
        Gsup = QGridLayout(gb_supply)
        self.supply_enable = QCheckBox("Enable DC supply model")
        self.supply_enable.setChecked(bool(getattr(self.params.supply, "enabled", True)))
        self.supply_enforce = QCheckBox("Enforce DC current/power as hard motoring limit")
        self.supply_enforce.setChecked(bool(getattr(self.params.supply, "enforce_dc_current_limit", True)))
        self.supply_preset = QComboBox()
        self.supply_preset.addItems(["615 V / 1.3 A", "1000 V / 4.0 A", "Custom"])
        self.supply_preset.setCurrentText(str(getattr(self.params.supply, "preset", "1000 V / 4.0 A")))
        self.vdc_link = make_dspin(0.0, 2000.0, 5.0, self.params.vf.vdc_link_v, 0)
        self.supply_idc = make_dspin(0.0, 100.0, 0.1, float(getattr(self.params.supply, "dc_current_limit_a", 4.0)), 2)
        self.supply_eff = make_dspin(0.05, 1.0, 0.01, float(getattr(self.params.supply, "inverter_efficiency", 0.95)), 3)
        self.supply_margin = make_dspin(0.10, 1.0, 0.01, float(getattr(self.params.supply, "dc_power_margin", 0.90)), 3)
        self.supply_aux = make_dspin(0.0, 5000.0, 10.0, float(getattr(self.params.supply, "aux_power_w", 0.0)), 1)
        self.supply_include_mech = QCheckBox("Include motor mechanical power in DC power estimate")
        self.supply_include_mech.setChecked(bool(getattr(self.params.supply, "include_mechanical_power", True)))
        self.lbl_supply_power = QLabel("—")
        self.lbl_supply_power.setWordWrap(True)
        Gsup.addWidget(self.supply_enable, 0, 0, 1, 3)
        Gsup.addWidget(self.supply_enforce, 1, 0, 1, 3)
        Gsup.addWidget(QLabel("Supply preset"), 2, 0)
        Gsup.addWidget(self.supply_preset, 2, 1, 1, 2)
        add_slider_row(Gsup, 3, "DC link voltage at inverter input (Vdc)", self.vdc_link,
                       "Upstream DC supply voltage feeding the surface inverter DC bus. This single Vdc is used for both AC voltage headroom and DC power/current gating.")
        add_slider_row(Gsup, 4, "DC current limit (A)", self.supply_idc,
                       "Upstream DC supply current limit. This is used as a hard motoring gate when enabled.")
        add_slider_row(Gsup, 5, "Inverter efficiency", self.supply_eff,
                       "Used only for DC input power estimate. It is not a second voltage setting.")
        add_slider_row(Gsup, 6, "Power margin", self.supply_margin,
                       "0.90 means only 90% of Vdc × Idc_limit is treated as usable.")
        add_slider_row(Gsup, 7, "Aux DC power (W)", self.supply_aux)
        Gsup.addWidget(self.supply_include_mech, 8, 0, 1, 3)
        Gsup.addWidget(QLabel("Available gated DC power"), 9, 0)
        Gsup.addWidget(self.lbl_supply_power, 9, 1, 1, 2)
        self.left_layout.addWidget(gb_supply)
        ...
        # Post-filter common-mode choke (optional block between sine filter and heptacable)
        gb_cmc = QGroupBox("POST-FILTER COMMON MODE CHOKE (CMC)")
        gl_cmc = QGridLayout(gb_cmc)

        self.cmc_enable = QCheckBox("Enable CMC after sine filter, before heptacable")
        self.cmc_enable.setChecked(bool(getattr(self.params.cmc, "enabled", True)))
        gl_cmc.addWidget(self.cmc_enable, 0, 0, 1, 2)

        gl_cmc.addWidget(QLabel("Part number"), 1, 0)
        self.cmc_part = QComboBox()
        self.cmc_part.addItems(["Würth 744839208072", "User defined"])
        self.cmc_part.setCurrentText(str(getattr(self.params.cmc, "part_number", "Würth 744839208072")))
        gl_cmc.addWidget(self.cmc_part, 1, 1)

        gl_cmc.addWidget(QLabel("Lcm (mH, common mode)"), 2, 0)
        self.cmc_lcm_mH = make_dspin(0.0, 1000.0, 1.0, float(getattr(self.params.cmc, "cm_l_mh", 208.0)), 1)
        gl_cmc.addWidget(self.cmc_lcm_mH, 2, 1)

        gl_cmc.addWidget(QLabel("Rdc (ohm/phase)"), 3, 0)
        self.cmc_rdc_ohm = make_dspin(0.0, 5.0, 0.001, float(getattr(self.params.cmc, "rdc_ohm_per_phase", 0.085)), 4)
        gl_cmc.addWidget(self.cmc_rdc_ohm, 3, 1)

        gl_cmc.addWidget(QLabel("Leakage Ldm (mH/phase)"), 4, 0)
        self.cmc_ldm_mH = make_dspin(0.0, 100.0, 0.01, float(getattr(self.params.cmc, "leakage_l_mh_per_phase", 0.0)), 3)
        gl_cmc.addWidget(self.cmc_ldm_mH, 4, 1)

        self.cmc_include_rdc = QCheckBox("Include Rdc in motor-drive voltage drop")
        self.cmc_include_rdc.setChecked(bool(getattr(self.params.cmc, "include_rdc_in_diff_model", True)))
        gl_cmc.addWidget(self.cmc_include_rdc, 5, 0, 1, 2)

        self.cmc_include_ldm = QCheckBox("Include leakage Ldm in motor-drive voltage drop")
        self.cmc_include_ldm.setChecked(bool(getattr(self.params.cmc, "include_leakage_in_diff_model", False)))
        gl_cmc.addWidget(self.cmc_include_ldm, 6, 0, 1, 2)

        gl_cmc.addWidget(QLabel("Irated (Arms)"), 7, 0)
        self.cmc_irated = make_dspin(0.0, 100.0, 0.1, float(getattr(self.params.cmc, "rated_current_arms", 7.2)), 2)
        gl_cmc.addWidget(self.cmc_irated, 7, 1)

        gl_cmc.addWidget(QLabel("Vrated (Vac)"), 8, 0)
        self.cmc_vrated = make_dspin(0.0, 5000.0, 10.0, float(getattr(self.params.cmc, "rated_voltage_vac", 760.0)), 1)
        gl_cmc.addWidget(self.cmc_vrated, 8, 1)

        self.cmc_enforce_i = QCheckBox("Enforce CMC current rating")
        self.cmc_enforce_i.setChecked(bool(getattr(self.params.cmc, "enforce_current_rating", True)))
        gl_cmc.addWidget(self.cmc_enforce_i, 9, 0, 1, 2)

        self.cmc_enforce_v = QCheckBox("Enforce CMC voltage rating")
        self.cmc_enforce_v.setChecked(bool(getattr(self.params.cmc, "enforce_voltage_rating", False)))
        gl_cmc.addWidget(self.cmc_enforce_v, 10, 0, 1, 2)

        note = QLabel("Placement: inverter → sine filter → CMC → heptacable. "
                      "The 208 mH value is common-mode inductance; it is not added as normal 3-phase differential motor inductance unless leakage Ldm is entered/enabled.")
        note.setWordWrap(True)
        note.setStyleSheet("color: #666;")
        gl_cmc.addWidget(note, 11, 0, 1, 2)

        self.left_layout.addWidget(gb_cmc)
        ...
        self.c_preset.addItems(list(HEPTACABLE_PRESETS.keys()))
        self.c_preset.setCurrentText("7-46A-XS-CB")

        self.c_preset_info = QLabel("—")
        self.c_preset_info.setWordWrap(True)
        self.c_preset_info.setStyleSheet("color:#444;")

        self.c_len = make_dspin(1.0, 10000.0, 1.0, self.params.cable.length_m, 0)
        self.c_rpm = make_dspin(0.01, 0.50, 0.001, self.params.cable.r_ohm_per_m, 4)
        ...
        self.c_ilim = make_dspin(0.1, 50.0, 0.01, self.params.cable.i_limit_arms, 2)
        ...
        C.addWidget(QLabel("Cable preset"), 0, 0)
        C.addWidget(self.c_preset, 0, 1, 1, 2)
        C.addWidget(self.c_preset_info, 1, 0, 1, 3)

        add_slider_row(C, 2, "Length (m)", self.c_len)
        add_slider_row(C, 3, "R (ohm/m)", self.c_rpm, "Preset uses helical-conductor resistance for the phase model")
        add_slider_row(C, 4, "L (H/m)", self.c_lpm, "Not listed on the cable datasheets; keep as your modeling assumption")
        add_slider_row(C, 5, "Temp factor on R", self.c_temp)

        C.addWidget(QLabel("Conductor count"), 6, 0)
        C.addWidget(self.c_wires, 6, 1, 1, 2)
        C.addWidget(QLabel("Current limit basis"), 7, 0)
        C.addWidget(self.c_ilim_basis, 7, 1, 1, 2)
        add_slider_row(C, 8, "I limit (Arms)", self.c_ilim, "Preset uses the helical-conductor current rating; per-conductor basis matches the datasheets")
        add_slider_row(C, 9, "2-wire L factor", self.c_lpar, "Inductance multiplier when using 2 wires/phase")
        ...
        # 5-segment custom profile (explicit Li + Ti per segment)
        ...
        C.addWidget(QLabel("Effective R_phase"), 10, 0)
        C.addWidget(self.lbl_cable_R, 10, 1, 1, 2)
        C.addWidget(QLabel("Effective L_phase"), 11, 0)
        C.addWidget(self.lbl_cable_L, 11, 1, 1, 2)
        C.addWidget(QLabel("Effective I_phase limit"), 12, 0)
        C.addWidget(self.lbl_cable_Iph, 12, 1, 1, 2)

        add_slider_row(C, 13, "Band: -ΔL (m)", self.c_len_band,
        ...
        footer = QLabel("GEMS 106068737 | ADARSH GOUDA | Version 23.0")
    ...
    def _connect_signals(self):
        widgets = [
            self.t_cont_rpm_min, self.t_out_rpm, self.t_out_tq, self.t_tf_rpm, self.t_cont_tq_min, self.t_cont_tq_max, self.env_out_rpm_max,
            ...
            self.supply_enable, self.supply_enforce, self.supply_preset, self.supply_idc, self.supply_eff, self.supply_margin, self.supply_aux, self.supply_include_mech,
            self.combo_strategy,
            ...
            self.sf_rd_ohm,
            self.c_preset, self.c_len, self.c_rpm, self.c_lpm, self.c_temp, self.c_wires, self.c_ilim_basis, self.c_ilim, self.c_lpar,
        ...
        self.btn_export_json.clicked.connect(self._export_settings_json)
        self.btn_import_json.clicked.connect(self._import_settings_json)
        ...
        if hasattr(self, "c_preset"):
            self.c_preset.currentIndexChanged.connect(self._on_cable_preset_changed)
        if hasattr(self, "supply_preset"):
            self.supply_preset.currentIndexChanged.connect(self._on_supply_preset_changed)

        self.tabs.currentChanged.connect(self._on_main_tab_changed)

    def _on_supply_preset_changed(self):
        txt = self.supply_preset.currentText() if hasattr(self, "supply_preset") else ""
        if txt.startswith("615"):
            self.vdc_link.blockSignals(True)
            self.supply_idc.blockSignals(True)
            self.vdc_link.setValue(615.0)
            self.supply_idc.setValue(1.3)
            self.vdc_link.blockSignals(False)
            self.supply_idc.blockSignals(False)
        elif txt.startswith("1000"):
            self.vdc_link.blockSignals(True)
            self.supply_idc.blockSignals(True)
            self.vdc_link.setValue(1000.0)
            self.supply_idc.setValue(4.0)
            self.vdc_link.blockSignals(False)
            self.supply_idc.blockSignals(False)
        if self.chk_auto.isChecked() and not getattr(self, "_bulk_updating", False):
            self.update_all()
    ...
    def _on_cable_preset_changed(self):
        if getattr(self, "_bulk_updating", False):
            return
        name = self.c_preset.currentText() if hasattr(self, "c_preset") else ""
        self._apply_cable_preset(name)
    ...
    def _apply_motor_preset(self, name: str):
        ...
        Mapping (per user clarification):
          ...
          - Windings_AASLB_VS_6in: Windings AASLB VS 6 in / 21 slot / 6750 rpm supplier data
        """
        n = (name or "").strip()
        if not n:
            return
        ...
        presets = {
            ...
            "Windings_AASLB_VS_6in": {
                # Windings ALT rotor topology preset.
                # Use the updated electrical constants from the Windings ALT report / parameter sheet:
                #   Ke = 122.187 Vll_peak/krpm (explicit peak line-line Back-EMF over speed)
                #   Rs_phase = 7.7898 ohm, R_line-line ~= 15.58 ohm
                #   Ld = 5.4877 mH, Lq = 6.6776 mH
                #   21 slots, 8 poles -> 4 pole pairs.
                #
                # Keep Kt/Ke UNLINKED for this preset.
                # Rationale:
                #   - The ALT table gives Kt = 1.43 Nm/A when derived from Ke.
                #   - The operating simulation gives shaft torque = 1.35 Nm at 1 Arms,
                #     so the useful shaft Kt for torque feasibility is 1.35 Nm/Arms
                #     = 11.94850682 lb-in/Arms.
                #   - Keeping Kt unlinked avoids overwriting the conservative shaft Kt
                #     when Ke/lambda is updated from the electromagnetic Back-EMF value.
                "pole_pairs": 4,
                "rs_ohm": 7.78976283678971,
                "ld_h": 5.48774523376536e-3,
                "lq_h": 6.67762575275575e-3,
                "link_kt_ke": False,
                "kt_lbin_arms": 11.94850682,
                "ke_vll_peak_krpm": 122.187220048912,
            },
            "User defined": {
        ...
        try:
            ...
            self.m_link.setChecked(bool(d.get("link_kt_ke", True)))
        ...
        if self.chk_auto.isChecked():
            ...
            # Preset edits changed multiple PMSM fields.  Make it obvious that the
            # plots need a recompute when Auto is disabled.
            try:
                self.lbl_status.setText("⚠ Preset changed — press Recompute")
                self.lbl_status.setStyleSheet("color:#92400e; font-weight:600;")
            except Exception:
                pass
    ...
    def _update_cable_preset_info(self, name: Optional[str] = None) -> None:
        if not hasattr(self, "c_preset_info"):
            return
        name = (name or (self.c_preset.currentText() if hasattr(self, "c_preset") else "User defined") or "User defined").strip()
        meta = HEPTACABLE_PRESETS.get(name, HEPTACABLE_PRESETS["User defined"])
        ...
        if name == "User defined":
            info = (
                "User defined heptacable. Manual electrical values stay in control. "
                "Length, wires/phase, and temperature profile remain scenario inputs."
            )
        else:
            info = (
                f"<b>{name}</b> | phase-model basis: {meta['model_basis']} | "
                f"Helical {meta['helical_awg']}: R={_fmt(meta['r_helical_ohm_per_m'] * 1000.0, '{:.1f}')} Ω/km, "
                f"I={_fmt(meta['current_helical_arms'], '{:.2f}')} Arms, "
                f"V={_fmt(meta['voltage_helical_vrms'], '{:.0f}')} Vrms, "
                f"C={_fmt(meta['cap_helical_pf_per_m'], '{:.0f}')} pF/m | "
                f"Center {meta['center_awg']}: R={_fmt(meta['r_center_ohm_per_m'] * 1000.0, '{:.1f}')} Ω/km, "
                f"V={_fmt(meta['voltage_center_vrms'], '{:.0f}')} Vrms | "
                f"Armor={_fmt(meta['armor_r_ohm_per_m'] * 1000.0, '{:.1f}')} Ω/km | "
                f"OD={_fmt(meta['od_mm'], '{:.3f}')} mm | Tmax={_fmt(meta['temp_max_C'], '{:.0f}')} °C. "
                f"{meta['notes']}"
            )
        self.c_preset_info.setText(info)

    def _apply_cable_preset(self, name: str) -> None:
        n = (name or "").strip()
        if not n:
            return
        meta = HEPTACABLE_PRESETS.get(n)
        if meta is None:
            return
        ...
        try:
            if n != "User defined":
                # Datasheets list per-conductor resistance/current. For the 3-phase heptacable power path,
                # the solver phase model uses the six helical conductors; scenario inputs such as length,
                # temperature profile, 1-wire vs 2-wire, and L/m stay under user control.
                self.c_rpm.setValue(float(meta['r_helical_ohm_per_m']))
                self.c_ilim_basis.setCurrentText("Per conductor")
                self.c_ilim.setValue(float(meta['current_helical_arms']))
                self.c_temp.setValue(1.00)
                self.c_temp_ref.setValue(20.0)
                self.c_temp_alpha.setValue(0.00393)
            self._update_cable_preset_info(n)
        finally:
            self._bulk_updating = False
    ...
    def _sync_params_from_ui(self):
        ...
        # target
        p.target.out_rpm = float(self.t_out_rpm.value())  # continuous rotation RPM max
        p.target.cont_rpm_min = float(self.t_cont_rpm_min.value()) if hasattr(self, "t_cont_rpm_min") else float(getattr(p.target, "cont_rpm_min", 0.5))
        p.target.out_torque_ftlbf = float(self.t_out_tq.value())  # peak / TF-correction torque
        p.target.tf_correction_rpm = float(self.t_tf_rpm.value()) if hasattr(self, "t_tf_rpm") else float(getattr(p.target, "tf_correction_rpm", 0.05))

        if p.target.cont_rpm_min > p.target.out_rpm:
            p.target.cont_rpm_min, p.target.out_rpm = p.target.out_rpm, p.target.cont_rpm_min

        if hasattr(self, "t_cont_tq_min") and hasattr(self, "t_cont_tq_max"):
            tq_min = float(self.t_cont_tq_min.value())
            tq_max = float(self.t_cont_tq_max.value())
            if tq_min > tq_max:
                tq_min, tq_max = tq_max, tq_min
            p.target.cont_torque_min_ftlbf = tq_min
            p.target.cont_torque_max_ftlbf = tq_max
        ...
        # DC supply current / power gate
        if not hasattr(p, "supply"):
            p.supply = SupplyParams()
        p.supply.enabled = bool(self.supply_enable.isChecked())
        p.supply.preset = self.supply_preset.currentText()
        p.supply.dc_current_limit_a = float(self.supply_idc.value())
        p.supply.enforce_dc_current_limit = bool(self.supply_enforce.isChecked())
        p.supply.inverter_efficiency = float(self.supply_eff.value())
        p.supply.dc_power_margin = float(self.supply_margin.value())
        p.supply.aux_power_w = float(self.supply_aux.value())
        p.supply.include_mechanical_power = bool(self.supply_include_mech.isChecked())
        ...
        # post-filter CMC
        if not hasattr(p, "cmc"):
            p.cmc = CommonModeChokeParams()
        p.cmc.enabled = bool(self.cmc_enable.isChecked())
        p.cmc.part_number = self.cmc_part.currentText()
        p.cmc.cm_l_mh = float(self.cmc_lcm_mH.value())
        p.cmc.rdc_ohm_per_phase = float(self.cmc_rdc_ohm.value())
        p.cmc.leakage_l_mh_per_phase = float(self.cmc_ldm_mH.value())
        p.cmc.include_rdc_in_diff_model = bool(self.cmc_include_rdc.isChecked())
        p.cmc.include_leakage_in_diff_model = bool(self.cmc_include_ldm.isChecked())
        p.cmc.rated_current_arms = float(self.cmc_irated.value())
        p.cmc.rated_voltage_vac = float(self.cmc_vrated.value())
        p.cmc.enforce_current_rating = bool(self.cmc_enforce_i.isChecked())
        p.cmc.enforce_voltage_rating = bool(self.cmc_enforce_v.isChecked())
    ...
    def update_all(self):
        ...
        if self.params.vf.voltage_entry_basis.strip().upper().startswith('DC'):
            self.lbl_vlimits.setText(
                f"Inverter AC limit from upstream DC supply:  VLL,rms,max = {vll_lim:.1f}  |  Vϕ,rms,max = {vph_lim:.1f}   "
                f"(supply Vdc={self.params.vf.vdc_link_v:.0f} V, {self.params.vf.modulation}, util={self.params.vf.v_util:.2f})"
            )
        else:
            self.lbl_vlimits.setText(
        ...
        try:
            sp = self.params.supply
            p_avail = self.model.dc_available_power_w() if hasattr(self, "model") else self.params.vf.vdc_link_v * sp.dc_current_limit_a * sp.dc_power_margin
            self.lbl_supply_power.setText(
                f"{p_avail:.0f} W usable upstream DC power = Vdc {self.params.vf.vdc_link_v:.0f} V × "
                f"Idc_limit {sp.dc_current_limit_a:.2f} A × margin {sp.dc_power_margin:.2f}; motoring gate={'ON' if (sp.enabled and sp.enforce_dc_current_limit) else 'OFF'}"
            )
        except Exception:
            self.lbl_supply_power.setText("—")

        self._update_cable_preset_info()
        ...
        # Motor-side torque tab (only render when visible)
        try:
            if hasattr(self, "motor_side_pane") and (self.tabs.currentWidget() == self.motor_side_pane):
                self._plot_motor_side_torque()
        except Exception:
            pass

        # SLIM I/F LUT tab (only render when visible)
        try:
            if hasattr(self, "slim_if_lut_pane") and (self.tabs.currentWidget() == self.slim_if_lut_pane):
                self._plot_slim_if_lut()
        except Exception:
            pass

        # I/F feasibility tab (only render when visible)
        try:
            if hasattr(self, "if_feas_pane") and (self.tabs.currentWidget() == self.if_feas_pane):
                self.if_feas_pane.render(self.model, self.params)
        except Exception:
            pass

        # Concept trade-space tab (only render when visible)
        try:
            if hasattr(self, "concept_trade_pane") and (self.tabs.currentWidget() == self.concept_trade_pane):
                self._plot_concept_trade_space(res_sel)
        except Exception as e:
            try:
                pane = self.concept_trade_pane
                pane.clear()
                pane.ax_overlay.text(
                    0.02, 0.95,
                    "Concept Trade render error:\n" + str(e),
                    transform=pane.ax_overlay.transAxes,
                    va="top",
                    ha="left",
                    fontsize=9,
                    color="crimson",
                    bbox=dict(boxstyle="round,pad=0.35", facecolor="white", alpha=0.90, edgecolor="crimson"),
                )
        ...
        # Apples-to-apples comparison tables (only render when tab is visible)
        try:
            if hasattr(self, "apples_pane") and (self.tabs.currentWidget() == self.apples_pane):
                self._plot_apples_to_apples()
        except Exception:
            pass
        ...
        try:
            if hasattr(self, "cmc_lts_pane") and (self.tabs.currentWidget() == self.cmc_lts_pane):
                self._plot_cmc_lts(res_sel)
        except Exception:
            pass
    ...
    def _plot_cmc_lts(self, res_sel=None):
        """Render the CMC/LTS coexistence pane."""
        if not hasattr(self, "cmc_lts_pane"):
            return
        try:
            self._sync_params_from_ui()
            self.model = SystemModel(self.params)
            ...
            pass
        try:
            self.cmc_lts_pane.render(self.model, self.params, res=res_sel)
        except Exception as e:
            try:
                pane = self.cmc_lts_pane
                for ax in (pane.ax1, pane.ax2, pane.ax3, pane.ax4):
                    ax.clear()
                pane.ax1.text(0.02, 0.95, "CMC/LTS render error:\n" + str(e),
                              transform=pane.ax1.transAxes, va="top", ha="left",
    ...
    def _plot_motor_side_torque(self):
        """Render the first-quadrant motor-side torque-speed tab using cached metrics.

        v26k.5 fix:
        - If the tab is opened before the cache exists, perform one full refresh.
        - If rendering fails, show the exception in the plot instead of silently
          leaving the tab blank.
        """
        if not hasattr(self, "motor_side_pane"):
            return

        # When the tab is opened before update_all() has produced envelope metrics,
        # rebuild once.  update_all() will populate _last_motor_metrics and then call
        # this method again only after the cache exists.
        if not hasattr(self, "_last_motor_metrics"):
            try:
                self.update_all()
            except Exception:
        ...
        try:
            m1, m2, res_1, res_2 = self._last_motor_metrics
            self.motor_side_pane.render(m1, m2, res_1, res_2, self.params)
        except Exception as e:
            try:
                pane = self.motor_side_pane
                for ax in (pane.ax_1w, pane.ax_2w, pane.ax_i, pane.ax_v, pane.ax_tbl):
                    ax.cla()
                pane.ax_1w.text(
                    0.02, 0.95,
                    "Motor Side Torque render error:\n" + str(e),
                    transform=pane.ax_1w.transAxes,
    ...
    def _plot_slim_if_lut(self):
        """Render the SLIM I/F LUT generator against the current QuadScope model."""
        if not hasattr(self, "slim_if_lut_pane"):
            return
        try:
            # Make sure parameters are synchronized when the tab is opened manually.
            self._sync_params_from_ui()
            self.model = SystemModel(self.params)
        except Exception:
            pass
        try:
            self.slim_if_lut_pane.render(self.model, self.params)
        except Exception as e:
            try:
                pane = self.slim_if_lut_pane
                pane.ax_lut.clear()
                pane.ax_current.clear()
                pane.ax_lut.text(
                    0.02, 0.95,
                    "SLIM I/F LUT render error:\n" + str(e),
                    transform=pane.ax_lut.transAxes,
                    va="top", ha="left", fontsize=9, color="crimson",
                    bbox=dict(boxstyle="round,pad=0.35", facecolor="white", alpha=0.90, edgecolor="crimson"),
                )
                pane.canvas.draw_idle()
    ...
    def _current_ui_motor_as_concept(self) -> Dict[str, float]:
        """Return the currently edited PMSM UI values as a Concept Trade motor.

        The previous Concept Trade implementation used only hard-coded comparison
        dictionaries.  That made the tab ignore manual edits to Rs/Ld/Lq/Kt/Ke in
        the PMSM UI.  This helper converts the live, synchronized MotorParams into
        the same basis used by the concept plots.
        """
        mp = self.params.motor
        try:
            preset = self.m_preset.currentText().strip() if hasattr(self, "m_preset") else "Active UI Motor"
        except Exception:
            preset = "Active UI Motor"

        if not preset or preset.lower().startswith("user"):
            name = "Active UI Motor"
        else:
            name = preset

        return {
            "name": name,
            "pole_pairs": int(mp.pole_pairs),
            "rs_ohm": float(mp.rs_ohm),
            "ld_mh": float(mp.ld_h) * 1000.0,
            "lq_mh": float(mp.lq_h) * 1000.0,
            "lambda_wb": float(mp.lambda_wb),
            "kt_lbin_arms": float(mp.kt_nm_per_arms) / LBIN_TO_NM,
            "ke_vll_peak_krpm": float(mp.ke_vll_rms_per_krpm) * math.sqrt(2.0),
            "color": "#DC2626",
            "is_ui_active": True,
        }

    def _concept_motor_dataset(self) -> List[Dict[str, float]]:
        """Motor concepts used for Concept Trade comparison.

        Basis:
          - Ke: V_LL,peak / krpm
          - Kt: lb-in / Arms
          - Rs, Ld, Lq: per-phase values

        v26k.5 fix:
          The live PMSM UI motor now participates in this dataset.  If the current
          preset maps to an existing concept (John1/John2/John3/Windings), the
          edited UI values replace that concept.  Otherwise, an "Active UI Motor"
          curve is added so manual motor edits are visible immediately.
        """
        motors = [
            {
                "name": "SLB_John1",
            ...
            },
            {
                ...
                "lq_mh": 6.678,
                "lambda_wb": 0.1684,
                "kt_lbin_arms": 11.9485,
                "ke_vll_peak_krpm": 122.19,
                "color": "#008756",
            },
        ]

        # Live UI motor overlay / replacement.
        try:
            ui = self._current_ui_motor_as_concept()
            raw_name = str(ui.get("name", "Active UI Motor"))

            # Map UI preset labels to the compact concept labels used in this tab.
            replacement_key = {
                "Annular Motor - John": "SLB_John1",
                "Annular Motor - John2": "SLB_John2",
                "Annular Motor - John3": "SLB_John3",
                "Windings_AASLB_VS_6in": "Windings_6in",
            }.get(raw_name)

            if replacement_key:
                for i, m in enumerate(motors):
                    if m.get("name") == replacement_key:
                        # Keep the established color/name for trend continuity, but use edited values.
                        ui["name"] = replacement_key
                        ui["color"] = m.get("color", ui.get("color", "#DC2626"))
                        motors[i] = ui
                        break
                else:
                    motors.append(ui)
            elif raw_name in {m.get("name") for m in motors}:
                for i, m in enumerate(motors):
                ...
                # Spear, Milling, and User-defined do not have a native Concept Trade slot.
                # Show them explicitly as the active UI motor.
                if raw_name in ("Spear motor", "Milling motor"):
                    ui["name"] = raw_name.replace(" motor", "") + "_UI"
                else:
                    ui["name"] = "Active_UI_Motor"
                motors.append(ui)
        except Exception:
            pass
    ...
    def _concept_effective_rs(self, rs_ohm: float) -> float:
        """Apply the same first-order winding temperature scaling used by the UI, if enabled."""
        try:
            ex = self.params.extra
            if not bool(getattr(ex, "rs_temp_enabled", False)):
                return float(rs_ohm)
            t_w = float(getattr(ex, "temp_C", 25.0)) + float(getattr(ex, "winding_rise_C", 0.0))
            t_ref = float(getattr(ex, "temp_ref_C", 25.0))
            alpha = float(getattr(ex, "rs_temp_coeff_per_C", 0.00393))
            return max(1e-12, float(rs_ohm) * (1.0 + alpha * (t_w - t_ref)))
        except Exception:
            return float(rs_ohm)

    def _concept_hot_kt_nm_per_arms(self, kt_lbin_arms: float) -> float:
        """Apply the same first-order Kt(T) scaling used by the UI, if enabled."""
        kt0 = float(kt_lbin_arms) * LBIN_TO_NM
        try:
            ex = self.params.extra
            if not bool(getattr(ex, "kt_temp_enabled", False)):
                return max(1e-12, kt0)
            dT = float(getattr(ex, "temp_C", 25.0)) - float(getattr(ex, "temp_ref_C", 25.0))
            scale = 1.0 + float(getattr(ex, "kt_temp_coeff_per_C", -0.0004)) * dT
            return max(1e-12, kt0 * scale)
        except Exception:
            return max(1e-12, kt0)

    def _concept_voltage_limit_vll(self) -> float:
        """Effective V_LL,RMS ceiling used by the Concept Trade screening plots."""
        vals = []
        try:
            vals.append(float(self.params.vf.v_ll_rms_limit()))
        except Exception:
            pass
        try:
            if bool(self.params.limits.enforce_downhole_vll_limit):
                vals.append(float(self.params.limits.downhole_vll_rms_limit))
        except Exception:
            pass
        try:
            if bool(self.params.limits.enforce_downhole_vphase_limit):
                vals.append(math.sqrt(3.0) * float(self.params.limits.downhole_v_phase_rms_limit))
        except Exception:
            pass
        vals = [v for v in vals if np.isfinite(v) and v > 0.0]
        return min(vals) if vals else 1.0

    def _concept_params_from_motor(self, motor: Dict[str, float], force_two_wires: bool = True) -> SystemParams:
        """Build a SystemParams copy for one concept motor.

        This is the key fix for the Concept Trade tab: use the same SystemModel
        voltage/current/cable/filter/downhole-limit machinery as the normal Motor
        Side Torque pane instead of a separate simplified equation.
        """
        p2 = copy.deepcopy(self.params)

        # Concept comparison is focused on 2-wires/phase unless explicitly changed later.
        if force_two_wires:
            p2.cable.wires_per_phase = 2

        p2.motor.pole_pairs = int(motor.get("pole_pairs", p2.motor.pole_pairs))
        pp = max(1, int(p2.motor.pole_pairs))
        ke_rms = float(motor["ke_vll_peak_krpm"]) / math.sqrt(2.0)

        # Set lambda from Ke so SystemModel.recompute_derived() preserves the desired Ke.
        krpm_to_rad_s = (1000.0 * 2.0 * math.pi) / 60.0
        lam_from_ke = (ke_rms / krpm_to_rad_s) * math.sqrt(2.0) / (pp * math.sqrt(3.0))

        p2.motor.rs_ohm = float(motor["rs_ohm"])
        p2.motor.ld_h = float(motor["ld_mh"]) / 1000.0
        p2.motor.lq_h = float(motor["lq_mh"]) / 1000.0
        p2.motor.lambda_wb = float(lam_from_ke)
        p2.motor.kt_nm_per_arms = float(motor["kt_lbin_arms"]) * LBIN_TO_NM
        p2.motor.ke_vll_rms_per_krpm = float(ke_rms)

        # Keep Kt unlinked from Ke so supplier-reported Kt and Ke can coexist.
        p2.motor.link_kt_ke = False
        p2.motor.motor_param_mode = "Ke"
        p2.motor.kt_basis = "lb-in/Arms"
        p2.motor.ke_basis = "Vll_peak/krpm"
        return p2

    def _concept_envelope(self, motor: Dict[str, float], motor_rpm: np.ndarray, i_limit: Optional[float] = None, vll_limit: Optional[float] = None):
        """Motor-side torque-speed envelope using the full QuadScope static solver.

        This intentionally mirrors the Motor Side Torque pane:
          - iq_max comes from SystemModel.max_iq_given_limits()
          - surface inverter, V/f plateau, sine filter, cable drop, current derating,
            downhole voltage limits, and optional field weakening are included
          - useful motor torque is raw EM torque minus tau_extra

        The previous v26f plot used a separate simplified equation. That made
        low-Ke John concepts look artificially flat over the full RPM range.
        """
        p2 = self._concept_params_from_motor(motor, force_two_wires=True)
        model = SystemModel(p2)
        cab = p2.cable
        fast = bool(self.chk_fast_sweeps.isChecked()) if hasattr(self, "chk_fast_sweeps") else True

        tor_lbin = np.zeros_like(motor_rpm, dtype=float)
        i_avail = np.zeros_like(motor_rpm, dtype=float)
        v_req = np.zeros_like(motor_rpm, dtype=float)

        kt_eff = float(model.kt_effective_nm_per_arms())

        for i, rpm in enumerate(np.asarray(motor_rpm, dtype=float)):
            rpm = float(abs(rpm))
            if fast:
                iq_max, id_best, im_best, *_ = model.max_iq_given_limits_fast(rpm, cab)
            else:
                iq_max, id_best, im_best, *_ = model.max_iq_given_limits(rpm, cab)

            tau_extra, _, _ = model.tau_extra_nm(rpm_to_rad_s(rpm))
            t_em = kt_eff * float(iq_max)
            t_use = max(0.0, float(t_em) - float(tau_extra))

            try:
                vph = model.motor_voltage_required_phase_rms(rpm, float(id_best), float(iq_max))
                vll = math.sqrt(3.0) * float(vph)
            except Exception:
                vll = np.nan

            tor_lbin[i] = t_use / LBIN_TO_NM
            i_avail[i] = float(im_best)
            v_req[i] = float(vll)

        return tor_lbin, i_avail, v_req


    def _output_ftlbf_to_motor_lbin_for_concept(self, out_ftlbf: float) -> float:
        """Convert CCRS output torque requirement to useful motor-side torque.

        Returned units: lb-in at the motor / magnetic-coupler side.

        The UI stays in output torque units because that is how the requirement is
        normally discussed. The Concept Trade plots stay motor-side because the
        comparison curves are motor-side torque-speed envelopes.
        """
        p = self.params
        G = max(1e-9, float(p.gearbox.ratio()))
        eta = max(1e-9, float(p.gearbox.eff_total()))

        # Torque required at gearbox input before magnetic-coupler inverse.
        t_gb_in_nm = ft_lbf_to_nm(abs(float(out_ftlbf))) / max(1e-9, G * eta)

        mc = getattr(p, "mag_coupler", None)
        if (mc is not None) and bool(getattr(mc, "enabled", False)):
            # Inverse of the simple magnetic-coupler model:
            # T_gb = slope * (T_motor - T_break)
            t_motor_nm = float(getattr(mc, "t_break_nm", 0.0)) + (
                t_gb_in_nm / max(1e-9, float(getattr(mc, "slope", 1.0)))
            )
        else:
        ...
        return float(t_motor_nm) / LBIN_TO_NM


    def _plot_concept_trade_space(self, res_sel: Optional[SolveResult] = None):
        """Render Concept Trade using explicit continuous and peak mission definitions.

        Key corrections in v26k.1:
          - Overlay shows Peak Torque line and Continuous Torque band.
          - Continuous margin is the worst margin across the continuous RPM range.
          - Continuous voltage headroom is the worst loaded headroom across the continuous RPM range.
          - Voltage/current/loss plots show both continuous and peak cases.
        """
        if not hasattr(self, "concept_trade_pane"):
            return
        ...
        p = self.params
        motors = self._concept_motor_dataset()
        G = max(1e-9, float(p.gearbox.ratio()))
        vll_limit = self._concept_voltage_limit_vll()

        cont_rpm_min_out = max(0.0, abs(float(getattr(p.target, "cont_rpm_min", 0.5))))
        cont_rpm_max_out = max(1e-6, abs(float(getattr(p.target, "out_rpm", 1.0))))
        if cont_rpm_min_out > cont_rpm_max_out:
            cont_rpm_min_out, cont_rpm_max_out = cont_rpm_max_out, cont_rpm_min_out

        tf_out_rpm = max(0.0, abs(float(getattr(p.target, "tf_correction_rpm", 0.05))))
        cont_tq_min_ftlbf = float(getattr(p.target, "cont_torque_min_ftlbf", 250.0))
        cont_tq_max_ftlbf = float(getattr(p.target, "cont_torque_max_ftlbf", 350.0))
        if cont_tq_min_ftlbf > cont_tq_max_ftlbf:
            cont_tq_min_ftlbf, cont_tq_max_ftlbf = cont_tq_max_ftlbf, cont_tq_min_ftlbf
        peak_tq_ftlbf = abs(float(getattr(p.target, "out_torque_ftlbf", 1000.0)))

        cont_rpm_min_m = cont_rpm_min_out * G
        cont_rpm_max_m = cont_rpm_max_out * G
        tf_motor_rpm = tf_out_rpm * G

        # Make sure the envelope covers both the continuous range and the peak point.
        try:
            rpm_max = max(float(self.env_out_rpm_max.value()) * G, cont_rpm_max_m * 1.20, tf_motor_rpm * 1.50, 1.0)
        except Exception:
            rpm_max = max(cont_rpm_max_m * 1.25, tf_motor_rpm * 1.50, 1.0)
        rpm_grid = np.linspace(0.0, rpm_max, 320)

        # Motor-side requirements
        cont_req_low_lbin = self._output_ftlbf_to_motor_lbin_for_concept(cont_tq_min_ftlbf)
        cont_req_high_lbin = self._output_ftlbf_to_motor_lbin_for_concept(cont_tq_max_ftlbf)
        peak_req_lbin = self._output_ftlbf_to_motor_lbin_for_concept(peak_tq_ftlbf)

        # Continuous evaluation grid across the full continuous RPM range.
        if cont_rpm_max_m <= cont_rpm_min_m + 1e-9:
            cont_eval_rpm = np.array([cont_rpm_max_m], dtype=float)
        else:
            cont_eval_rpm = np.linspace(cont_rpm_min_m, cont_rpm_max_m, 80)

        results = []
        for mtr in motors:
            t_lbin, i_arr, v_arr = self._concept_envelope(mtr, rpm_grid)
            p2 = self._concept_params_from_motor(mtr)
            model2 = SystemModel(p2)
            kt_hot = float(model2.kt_effective_nm_per_arms())
            rs_eff = float(model2.rs_effective_ohm())
            ke_rms = float(mtr["ke_vll_peak_krpm"]) / math.sqrt(2.0)
            lq_h = float(mtr["lq_mh"]) / 1000.0
            pp = max(1, int(p2.motor.pole_pairs))

            # Worst continuous torque margin across the speed range.
            t_avail_cont = np.interp(cont_eval_rpm, rpm_grid, t_lbin)
            cont_margins = (t_avail_cont - cont_req_high_lbin) / max(1e-9, cont_req_high_lbin)
            worst_idx = int(np.nanargmin(cont_margins))
            worst_cont_rpm = float(cont_eval_rpm[worst_idx])
            worst_cont_margin = float(cont_margins[worst_idx])
            worst_cont_avail_lbin = float(t_avail_cont[worst_idx])

            # Loaded voltage budget for continuous max torque across the range.
            iq_cont = (cont_req_high_lbin * LBIN_TO_NM) / max(1e-12, kt_hot)
            v_req_cont = []
            for rpm in cont_eval_rpm:
                we = 2.0 * math.pi * (float(rpm) * pp / 60.0)
                e_ll = ke_rms * float(rpm) / 1000.0
                ir_ll = math.sqrt(3.0) * iq_cont * rs_eff
                ix_ll = math.sqrt(3.0) * iq_cont * we * lq_h
                # Indicator magnitude, not exact phase-vector solution.
                v_req_cont.append(e_ll + ir_ll + ix_ll)
            v_req_cont = np.asarray(v_req_cont, dtype=float)
            worst_v_idx = int(np.nanargmax(v_req_cont))
            worst_v_rpm = float(cont_eval_rpm[worst_v_idx])
            worst_cont_vreq = float(v_req_cont[worst_v_idx])
            worst_cont_headroom = float(vll_limit) - worst_cont_vreq

            # Peak case at TF-correction RPM.
            peak_avail_lbin = float(np.interp(tf_motor_rpm, rpm_grid, t_lbin))
            peak_margin = (peak_avail_lbin - peak_req_lbin) / max(1e-9, peak_req_lbin)
            iq_peak = (peak_req_lbin * LBIN_TO_NM) / max(1e-12, kt_hot)
            we_peak = 2.0 * math.pi * (tf_motor_rpm * pp / 60.0)
            e_peak = ke_rms * tf_motor_rpm / 1000.0
            ir_peak = math.sqrt(3.0) * iq_peak * rs_eff
            ix_peak = math.sqrt(3.0) * iq_peak * we_peak * lq_h
            v_peak_ind = e_peak + ir_peak + ix_peak
            ...
            results.append({
                ...
                "cont_iq": iq_cont,
                "peak_iq": iq_peak,
                "cont_pcu": 3.0 * iq_cont * iq_cont * rs_eff,
                "peak_pcu": 3.0 * iq_peak * iq_peak * rs_eff,
                "cont_margin_worst": worst_cont_margin,
                "cont_margin_rpm": worst_cont_rpm,
                "cont_avail_worst_lbin": worst_cont_avail_lbin,
                "cont_vreq_worst": worst_cont_vreq,
                "cont_vrpm_worst": worst_v_rpm,
                "cont_headroom_worst": worst_cont_headroom,
                "peak_avail_lbin": peak_avail_lbin,
                "peak_margin": peak_margin,
                "peak_vreq": v_peak_ind,
                "budget_cont": (ke_rms * worst_v_rpm / 1000.0,
                                math.sqrt(3.0) * iq_cont * rs_eff,
                                math.sqrt(3.0) * iq_cont * (2.0 * math.pi * (worst_v_rpm * pp / 60.0)) * lq_h),
                "budget_peak": (e_peak, ir_peak, ix_peak),
            })

        # Common labels/colors
        names = [r["motor"]["name"].replace("_", "\n") for r in results]
        colors = [r["motor"]["color"] for r in results]
        x = np.arange(len(results))

        # 1) Overlay motor-side torque-speed with mission requirements
        ax = pane.ax_overlay
        for r in results:
            mtr = r["motor"]
            ax.plot(rpm_grid, r["t_lbin"], lw=2.0, label=mtr["name"], color=mtr["color"])
        ax.axhline(peak_req_lbin, linestyle="--", color="#111827", lw=1.25, label="Peak torque req.")
        ax.fill_between([cont_rpm_min_m, cont_rpm_max_m],
                        [cont_req_low_lbin, cont_req_low_lbin],
                        [cont_req_high_lbin, cont_req_high_lbin],
                        color="#16A34A", alpha=0.22, label="Cont. torque band")
        ax.axvspan(cont_rpm_min_m, cont_rpm_max_m, color="#BBF7D0", alpha=0.10)
        ax.axvline(tf_motor_rpm, linestyle=":", color="#92400E", lw=1.1, label="TF RPM")
        ax.set_title("Overlay: motor-side torque-speed with mission requirements")
        ax.set_xlabel("Motor RPM")
        ax.set_ylabel("Useful motor-side torque (lb-in)")
        ax.grid(True, alpha=0.25)
        ax.legend(fontsize=6, loc="best")

        # 2) Continuous rotation: worst margin vs worst loaded voltage headroom
        ax = pane.ax_cont
        ax.axhline(0.0, color="black", lw=0.8)
        ax.axvline(0.0, color="#9ca3af", lw=1.0, linestyle="--")
        for r in results:
            mtr = r["motor"]
            ax.scatter(r["cont_headroom_worst"], 100.0 * r["cont_margin_worst"],
                       s=95, color=mtr["color"], edgecolors="black", linewidths=0.6)
            ax.annotate(
                f'{mtr["name"]}\n@{r["cont_margin_rpm"]/G:.2f} rpm',
                (r["cont_headroom_worst"], 100.0 * r["cont_margin_worst"]),
                textcoords="offset points", xytext=(5, 5), fontsize=7,
            )
        ax.set_title("Continuous rotation: worst margin vs loaded headroom")
        ax.set_xlabel(
            f"Worst loaded headroom over {cont_rpm_min_out:.2f}-{cont_rpm_max_out:.2f} output rpm (VLL,RMS)"
        )
        ax.set_ylabel(f"Worst torque margin vs {cont_tq_max_ftlbf:.0f} ft-lbf cont. max (%)")
        ax.grid(True, alpha=0.25)

        # 3) Peak torque at TF-correction RPM
        ax = pane.ax_peak
        vals = np.array([r["peak_avail_lbin"] for r in results], dtype=float)
        margins = np.array([100.0 * r["peak_margin"] for r in results], dtype=float)
        bars = ax.bar(x, vals, color=colors, alpha=0.88)
        ax.axhline(peak_req_lbin, linestyle="--", color="#111827", lw=1.2, label="Peak torque req.")
        ax.set_xticks(x)
        ax.set_xticklabels(names, fontsize=7)
        ax.set_title("Peak torque at TF-correction RPM")
        ax.set_ylabel("Useful motor-side torque (lb-in)")
        ax.grid(True, axis="y", alpha=0.25)
        ymax_peak = max(1.0, float(np.nanmax(vals)), peak_req_lbin) * 1.25
        ax.set_ylim(0.0, ymax_peak)
        for bar, margin in zip(bars, margins):
            ax.text(bar.get_x() + bar.get_width() / 2.0,
                    bar.get_height() + 0.03 * ymax_peak,
                    f"{margin:+.0f}%",
                    ha="center", va="bottom", fontsize=7)
        ax.text(
            0.02, 0.94,
            f"Peak={peak_tq_ftlbf:.0f} ft-lbf output ≈ {peak_req_lbin:.1f} lb-in motor-side\n"
            f"TF correction={tf_out_rpm:.3f} output rpm ({tf_motor_rpm:.0f} motor rpm)",
            transform=ax.transAxes,
            fontsize=7, va="top", ha="left",
            bbox=dict(boxstyle="round,pad=0.25", facecolor="white", alpha=0.82, edgecolor="#d1d5db"),
        )
        ax.legend(fontsize=6, loc="lower right")

        # 4) Ke-Kt design map
        ax = pane.ax_kekt
        ax.axvspan(85.0, 95.0, color="#bfdbfe", alpha=0.45, label="Requested Ke 85–95")
        rs_vals = np.array([r["rs_eff"] for r in results], dtype=float)
        for r in results:
            mtr = r["motor"]
            bubble = 80.0 + 280.0 * (r["rs_eff"] / max(1e-9, float(np.nanmax(rs_vals))))
            ax.scatter(float(mtr["ke_vll_peak_krpm"]), float(mtr["kt_lbin_arms"]), s=bubble,
                       color=mtr["color"], alpha=0.82, edgecolors="black", linewidths=0.6)
            ax.annotate(mtr["name"], (float(mtr["ke_vll_peak_krpm"]), float(mtr["kt_lbin_arms"])),
                        textcoords="offset points", xytext=(5, 4), fontsize=7)
        ax.set_title("Design map: Ke vs Kt (bubble ≈ Rs)")
        ax.set_xlabel("Ke (VLL,peak/krpm)")
        ax.set_ylabel("Kt (lb-in/Arms)")
        ax.grid(True, alpha=0.25)
        ax.legend(fontsize=7, loc="best")

        # 5) Continuous-rotation voltage budget only.
        #
        # Do NOT mix peak / TF-correction voltage with continuous voltage in this plot.
        # Peak is evaluated at very low motor speed, so back-EMF is naturally tiny and
        # the plot becomes misleading. Voltage is mainly a continuous high-speed concern.
        ax = pane.ax_voltage
        e_cont = np.array([r["budget_cont"][0] for r in results], dtype=float)
        ir_cont = np.array([r["budget_cont"][1] for r in results], dtype=float)
        ix_cont = np.array([r["budget_cont"][2] for r in results], dtype=float)
        v_cont_total = e_cont + ir_cont + ix_cont
        v_cont_util = 100.0 * v_cont_total / max(1e-9, float(vll_limit))

        bars_e = ax.bar(x, e_cont, label="Back-EMF")
        ax.bar(x, ir_cont, bottom=e_cont, label="IR")
        ax.bar(x, ix_cont, bottom=e_cont + ir_cont, label="ωLqI")
        ax.axhline(vll_limit, linestyle="--", color="black", lw=1.0, label="V limit")

        ax.set_xticks(x)
        ax.set_xticklabels(names, fontsize=7)
        ax.set_title("Continuous voltage budget at worst-speed point")
        ax.set_ylabel("VLL,RMS magnitude")
        ax.grid(True, axis="y", alpha=0.25)
        ax.legend(fontsize=6, loc="best")

        ymax_v = max(float(vll_limit), float(np.nanmax(v_cont_total))) * 1.16
        ax.set_ylim(0.0, ymax_v)
        for xi, total, util, r in zip(x, v_cont_total, v_cont_util, results):
            ax.text(
                xi,
                total + 0.025 * ymax_v,
                f"{total:.0f} V\n{util:.0f}%",
        ...
        ax.text(
            0.02,
            0.96,
            "Peak / TF voltage is not shown here:\n"
            "TF correction occurs at low RPM, so voltage is not the limiting case.\n"
            "Peak case is assessed in the current/torque plots.",
            transform=ax.transAxes,
            fontsize=7,
            va="top",
            ha="left",
            bbox=dict(boxstyle="round,pad=0.25", facecolor="white", alpha=0.86, edgecolor="#d1d5db"),
        )

        # 6) Current and copper loss: continuous vs peak.
        # Restore current-limit line so the current plot is directly actionable.
        ax_iq = pane.ax_iq
        ax_pcu = pane.ax_pcu
        w = 0.36
        iq_cont = np.array([r["cont_iq"] for r in results], dtype=float)
        iq_peak = np.array([r["peak_iq"] for r in results], dtype=float)
        pcu_cont = np.array([r["cont_pcu"] for r in results], dtype=float)
        pcu_peak = np.array([r["peak_pcu"] for r in results], dtype=float)

        ax_iq.bar(x - w/2, iq_cont, width=w, color=colors, alpha=0.88, label="Cont.")
        ax_iq.bar(x + w/2, iq_peak, width=w, color=colors, alpha=0.45, label="Peak")
        try:
            i_limit_plot = float(p.cable.i_phase_limit())
            ax_iq.axhline(i_limit_plot, linestyle="--", color="#374151", linewidth=1.0, label="Cable I limit")
        except Exception:
            i_limit_plot = None

        ax_iq.set_title("Current required: continuous vs peak")
        ax_iq.set_ylabel("Arms")
        ax_iq.set_xticks(x)
        ax_iq.set_xticklabels([])
        ax_iq.grid(True, axis="y", alpha=0.25)
        ax_iq.legend(fontsize=6, loc="best")

        ymax_iq_candidates = [1.0, float(np.nanmax(np.r_[iq_cont, iq_peak])) * 1.25]
        if i_limit_plot is not None:
            ymax_iq_candidates.append(i_limit_plot * 1.18)
        ymax_iq = max(ymax_iq_candidates)
        ax_iq.set_ylim(0.0, ymax_iq)

        for xi, val in zip(x - w/2, iq_cont):
            ax_iq.text(xi, val + 0.03 * ymax_iq, f"{val:.2f}", ha="center", va="bottom", fontsize=6)
        for xi, val in zip(x + w/2, iq_peak):
            ax_iq.text(xi, val + 0.03 * ymax_iq, f"{val:.2f}", ha="center", va="bottom", fontsize=6)

        ax_pcu.bar(x - w/2, pcu_cont, width=w, color=colors, alpha=0.62, label="Cont.")
        ax_pcu.bar(x + w/2, pcu_peak, width=w, color=colors, alpha=0.32, label="Peak")
        ax_pcu.set_title("Copper loss estimate: continuous vs peak")
        ax_pcu.set_ylabel("W")
        ax_pcu.set_xticks(x)
        ax_pcu.set_xticklabels(names, fontsize=7)
        ax_pcu.grid(True, axis="y", alpha=0.25)
        ax_pcu.legend(fontsize=6, loc="best")
        ymax_pcu = max(1.0, float(np.nanmax(np.r_[pcu_cont, pcu_peak])) * 1.25)
        ax_pcu.set_ylim(0.0, ymax_pcu)

        for xi, val in zip(x - w/2, pcu_cont):
            ax_pcu.text(xi, val + 0.03 * ymax_pcu, f"{val:.0f}", ha="center", va="bottom", fontsize=6)
        for xi, val in zip(x + w/2, pcu_peak):
            ax_pcu.text(xi, val + 0.03 * ymax_pcu, f"{val:.0f}", ha="center", va="bottom", fontsize=6)

        try:
            pane.fig.suptitle(
                f"Concept Trade | Cont. {cont_rpm_min_out:.2f}-{cont_rpm_max_out:.2f} rpm / "
                f"{cont_tq_min_ftlbf:.0f}-{cont_tq_max_ftlbf:.0f} ft-lbf; "
                f"Peak {peak_tq_ftlbf:.0f} ft-lbf @ TF {tf_out_rpm:.3f} rpm",
                fontsize=10,
                y=0.985,
            )
            pane.fig.tight_layout(rect=[0.0, 0.0, 1.0, 0.96])
        except Exception:
            pass
    ...
    def _on_main_tab_changed(self, idx: int):
        ...
        # Render apples-to-apples pane on-demand.
        if hasattr(self, "apples_pane") and (w == self.apples_pane):
            try:
                self._plot_apples_to_apples()
            except Exception:
                pass
        ...
        # Render Motor-side torque pane on-demand.
        if hasattr(self, "motor_side_pane") and (w == self.motor_side_pane):
            try:
                # Use a full refresh so manual UI edits made while Auto was off are not stale.
                self.update_all()
            except Exception:
                try:
                    self._plot_motor_side_torque()
        ...
        # Render SLIM I/F LUT pane on-demand.
        if hasattr(self, "slim_if_lut_pane") and (w == self.slim_if_lut_pane):
            try:
                self.update_all()
            except Exception:
                try:
        ...
        # Render concept trade-space pane on-demand.
        if hasattr(self, "concept_trade_pane") and (w == self.concept_trade_pane):
            try:
                # Use a full refresh so the live edited PMSM values are synchronized before plotting.
                self.update_all()
            except Exception as e:
                try:
                    pane = self.concept_trade_pane
        ...
        # Render I/F feasibility pane on-demand.
        if hasattr(self, "if_feas_pane") and (w == self.if_feas_pane):
            try:
                self.if_feas_pane.render(self.model, self.params)
            except Exception:
                pass

        # Render CMC / LTS pane on-demand.
        if hasattr(self, "cmc_lts_pane") and (w == self.cmc_lts_pane):
            try:
                self.update_all()
            except Exception:
                try:
    ...
    def _state_widgets_for_defaults(self):
        ...
        ws = [
            ...
            # target
            self.t_cont_rpm_min, self.t_out_rpm, self.t_out_tq, self.t_tf_rpm, self.t_cont_tq_min, self.t_cont_tq_max, self.env_out_rpm_max, self.chk_tq_override,
            self.chk_tq_override,
            # vf/fw
            self.v_limit_type, self.v_limit_value, self.v_entry_basis, self.vdc_link, self.modulation, self.v_util,
            self.base_f, self.base_v, self.v_boost,
    ...
    def _iter_settings_widgets(self):
        """Return named value widgets that represent the current setup state.

        The list is attribute-driven so new spin boxes, combo boxes, and check boxes
        are automatically included in the exported JSON without maintaining a long
        hand-written schema. Buttons, labels, plots, and other non-value widgets are
        intentionally excluded.
        """
        items = []
        seen = set()
        value_types = (QDoubleSpinBox, QSpinBox, QComboBox, QCheckBox)
        for name, obj in sorted(vars(self).items()):
            if name.startswith("_"):
                continue
            if isinstance(obj, value_types):
                oid = id(obj)
                if oid not in seen:
                ...
                for idx, child in enumerate(obj):
                    if isinstance(child, value_types):
                        oid = id(child)
                        if oid not in seen:
                            items.append((f"{name}[{idx}]", child))
                            seen.add(oid)
        return items

    def _combo_data_to_jsonable(self, data):
        try:
            json.dumps(data)
            return data
        except TypeError:
    ...
    def _current_settings_payload(self) -> dict:
        """Collect all current UI settings into a portable JSON payload."""
        settings = {}
        for name, widget in self._iter_settings_widgets():
            if isinstance(widget, QDoubleSpinBox):
                ...
                settings[name] = {
                    ...
                    "data": self._combo_data_to_jsonable(widget.currentData()),
                }

        return {
            "schema": "QuadScope.UISettings.v1",
            "app": "QuadScope",
            "revision": "v26k.12",
            "exported_at": datetime.now().isoformat(timespec="seconds"),
            "window_title": self.windowTitle(),
            "active_main_tab": int(self.tabs.currentIndex()) if hasattr(self, "tabs") else 0,
            "active_sweep_tab": int(self.sweep_tabs.currentIndex()) if hasattr(self, "sweep_tabs") else 0,
            "settings": settings,
        }

    def _restore_combo_from_saved_state(self, combo: QComboBox, saved: dict) -> None:
        """Restore a combo box by itemData first, then text, then index."""
        data = saved.get("data", None)
        if data is not None:
        ...
        text = saved.get("text", None)
        if text is not None:
            idx = combo.findText(str(text))
            if idx >= 0:
                combo.setCurrentIndex(idx)
                return
            # setCurrentText is harmless for non-editable combos and supports future editable combos.
            combo.setCurrentText(str(text))
            return

        idx = int(saved.get("index", combo.currentIndex()))
        if 0 <= idx < combo.count():
            combo.setCurrentIndex(idx)

    def _apply_settings_payload(self, payload: dict) -> tuple[int, list[str]]:
        """Apply UI settings from JSON. Returns (applied_count, missing_keys)."""
        if not isinstance(payload, dict):
            raise ValueError("Settings file is not a JSON object.")

        settings = payload.get("settings", payload)
        if not isinstance(settings, dict):
            raise ValueError("Settings file does not contain a valid 'settings' object.")

        widget_map = dict(self._iter_settings_widgets())
        missing = []
        applied = 0

        self._bulk_updating = True
        try:
            for name, saved in settings.items():
                ...
                try:
                    if isinstance(widget, QDoubleSpinBox):
                        widget.setValue(float(saved.get("value", widget.value())))
                        applied += 1
                    elif isinstance(widget, QSpinBox):
                        widget.setValue(int(saved.get("value", widget.value())))
                        applied += 1
                    elif isinstance(widget, QCheckBox):
                        widget.setChecked(bool(saved.get("checked", widget.isChecked())))
                        applied += 1
                    elif isinstance(widget, QComboBox):
                        self._restore_combo_from_saved_state(widget, saved)
                        applied += 1
                finally:
        ...
        # Restore selected tabs after all controls exist and values are applied.
        try:
            idx = int(payload.get("active_main_tab", self.tabs.currentIndex()))
            if 0 <= idx < self.tabs.count():
                self.tabs.setCurrentIndex(idx)
        except Exception:
            pass
        try:
            idx = int(payload.get("active_sweep_tab", self.sweep_tabs.currentIndex()))
            if 0 <= idx < self.sweep_tabs.count():
                self.sweep_tabs.setCurrentIndex(idx)
        except Exception:
            pass

        # Some UI sections have dependent enable/disable behavior.
        self._apply_sweeps_tab_enabled()
        self._apply_voltage_basis_visibility()
        self._update_strategy_ui_enabled()
        self._update_cable_temp_ui_enabled()
    ...
    def _export_settings_json(self):
        ...
        try:
            # Keep params coherent before export, but export the actual widget state.
            self._sync_params_from_ui()
        except Exception:
            # Export should still work even if a transient parameter validation issue exists.
            pass

        default_name = f"QuadScope_settings_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
        path, _ = QFileDialog.getSaveFileName(
            self,
            "Export QuadScope Settings JSON",
            default_name,
        ...
        try:
            ...
            self.lbl_status.setText("✅ Settings exported")
            self.lbl_status.setStyleSheet("color:#065f46; font-weight:600;")
            QMessageBox.information(self, "Settings exported", f"Saved current UI settings to:\n{path}")
        except Exception as e:
            QMessageBox.critical(self, "Export failed", f"Could not export settings JSON:\n{e}")

    def _import_settings_json(self):
        """Load a JSON file previously produced by Export JSON and restore UI state."""
        path, _ = QFileDialog.getOpenFileName(
            self,
            "Import QuadScope Settings JSON",
            "",
            "JSON files (*.json);;All files (*)",
        )
        if not path:
        ...
        try:
            with open(path, "r", encoding="utf-8") as f:
                payload = json.load(f)
            applied, missing = self._apply_settings_payload(payload)
            self.lbl_status.setText("✅ Settings imported")
            self.lbl_status.setStyleSheet("color:#065f46; font-weight:600;")
            msg = f"Imported {applied} UI settings from:\n{path}"
            if missing:
                msg += f"\n\nIgnored {len(missing)} setting(s) not present in this build."
            QMessageBox.information(self, "Settings imported", msg)
        except Exception as e:
            QMessageBox.critical(self, "Import failed", f"Could not import settings JSON:\n{e}")
    ...
    def _update_status(self, res_sel: SolveResult, res_1: SolveResult, res_2: SolveResult):
        ...
        lines.append(
            ...
            f"Vdrop(filter)={res.v_filter_drop:.1f}, Vpost-filter={res.v_post_filter_req:.1f}, "
            f"Vdrop(CMC)={res.v_cmc_drop:.2f}, Vnode={res.v_node_req:.1f}, "
        ...
        if cmc is not None and bool(getattr(cmc, 'enabled', False)):
            lines.append(
                f"Post-filter CMC: ON ({getattr(cmc, 'part_number', 'CMC')}), "
                f"Lcm={float(getattr(cmc, 'cm_l_mh', 0.0)):.1f} mH, "
                f"Rdc={float(getattr(cmc, 'rdc_ohm_per_phase', 0.0)):.4f} ohm/phase, "
                f"Vdrop(CMC)≈{res.v_cmc_drop:.3f} Vrms/ph, "
                f"Irated={float(getattr(cmc, 'rated_current_arms', 0.0)):.2f} Arms"
            )
        else:
            lines.append("Post-filter CMC: OFF / bypassed")
    ...
    def _plot_apples_to_apples(self):

        """Render apples-to-apples comparison tables.

        This view compares cable presets and 1w/2w wiring at the SAME fixed
        output requirement. Unlike the Envelope/Tables tabs, these values are
        not taken at each cable's own max-torque point.
        """
        if not hasattr(self, "apples_pane"):
            return

        pane = self.apples_pane
        for ax in (pane.ax1, pane.ax2, pane.ax3, pane.ax4):
            ax.cla()
        ...
        preset_names = [k for k in HEPTACABLE_PRESETS.keys() if k != "User defined"]
        ...
        def _make_table(ax, title: str, subtitle: str, col_labels: list[str], rows: list[list[str]],
                        col_widths: list[float], header_bg: str = "#f3f4f6",
                        row_bg1: str = "white", row_bg2: str = "#fafafa",
                        fontsize: float = 7.8, header_fontsize: float = 8.6,
                        left_align_cols: tuple[int, ...] = (0,),
                        cell_bg_overrides=None):
            ax.set_axis_off()
            ax.text(0.01, 0.98, title, va="top", ha="left", fontsize=11, fontweight="bold")
            if subtitle:
                ax.text(0.01, 0.91, subtitle, va="top", ha="left", fontsize=9.0, color="0.25")
            ...
            for (r, c), cell in tbl.get_celld().items():
                ...
                if r == 0:
                    ...
                    cell.set_facecolor(row_bg1 if ((r - 1) % 2 == 0) else row_bg2)
                    cell.get_text().set_fontsize(fontsize)
                if c in left_align_cols:
                    cell.get_text().set_ha("left")
                if cell_bg_overrides and ((r, c) in cell_bg_overrides):
        ...
        def _cable_from_preset(name: str, wires: int) -> CableParams:
            cab = copy.deepcopy(self.params.cable)
            cab.wires_per_phase = int(wires)
            meta = HEPTACABLE_PRESETS.get(name, None)
            if meta is not None and name != "User defined":
                if meta.get("r_helical_ohm_per_m") is not None:
                    cab.r_ohm_per_m = float(meta["r_helical_ohm_per_m"])
                if meta.get("current_helical_arms") is not None:
                    cab.i_limit_arms = float(meta["current_helical_arms"])
                    cab.i_limit_basis = "Per conductor"
                cab.temp_factor_r = 1.0
                cab.temp_ref_C = 20.0
                cab.temp_alpha_per_C = 0.00393
            return cab

        def _fixed_req_metrics(out_rpm: float, out_tq_ftlbf: float, cab: CableParams) -> dict:
            m = self.model
            G = float(self.params.gearbox.ratio())
            eta = float(self.params.gearbox.eff_total())

            motor_rpm = float(abs(out_rpm)) * G
            t_out_req_nm = ft_lbf_to_nm(float(abs(out_tq_ftlbf)))
            t_gb_in_req_nm = t_out_req_nm / max(1e-12, (G * eta))
            ok_cpl, t_motor_load_nm, mag_slip = m._mag_inverse_required_motor_torque_nm(t_gb_in_req_nm)

            kt_eff = float(m.kt_effective_nm_per_arms())
            omega_m = rpm_to_rad_s(motor_rpm)
            tau_extra, tau_core, tau_visc = m.tau_extra_nm(omega_m)
            motor_torque_total = float(t_motor_load_nm) + float(tau_extra)
            iq_req = motor_torque_total / max(1e-12, kt_eff)

            iq_max, id_best_at_max, im_best_at_max, f_e, v_cmd, v_surface_limit, v_dh_phase = m.max_iq_given_limits(motor_rpm, cab)
            feasible, id_used, im_used, _, _, _, _ = m.find_feasible_id_for_iq(motor_rpm, iq_req, cab)

            if feasible:
                id_rep = float(id_used)
                iq_rep = float(iq_req)
                im_rep = float(im_used)
            else:
                id_rep = float(id_best_at_max)
                iq_rep = float(iq_max)
                im_rep = float(im_best_at_max)

            v_motor = float(m.motor_voltage_required_phase_rms(motor_rpm, id_rep, iq_rep))
            v_drop = float(m.cable_drop_phase_rms(motor_rpm, im_rep, cab))
            v_node = v_motor + v_drop
            i_inv = float(m.inverter_current_mag_rms(motor_rpm, v_node, im_rep))
            v_inv = float(m.inverter_voltage_required_phase_rms(motor_rpm, v_node, im_rep))
            p_cable = float(m.cable_loss_w(im_rep, cab))

            t_em_max = float(kt_eff) * float(iq_max)
            t_use_max = max(0.0, t_em_max - float(tau_extra))
            t_gb_in_cap, _ = m._mag_forward_transmitted_to_gb_nm(float(t_use_max))
            t_out_cap_nm = float(t_gb_in_cap) * G * eta
            torque_margin_ftlbf = nm_to_ft_lbf(t_out_cap_nm - t_out_req_nm)
            ...
            if float(iq_req) > float(iq_max) + 1e-9:
                flags.append("I")
            if float(v_inv) > float(v_cmd) + 1e-9:
                flags.append("Vsurf")
            if (v_dh_phase is not None) and (float(v_motor) > float(v_dh_phase) + 1e-9):
                flags.append("Vdh")
            if not flags and not feasible:
                flags.append("Other")

            p_out_w = ft_lbf_to_nm(float(abs(out_tq_ftlbf))) * abs(rpm_to_rad_s(float(abs(out_rpm))))
            loss_pct = (100.0 * p_cable / p_out_w) if p_out_w > 1e-9 else float('nan')

            return {
                "feasible": bool(feasible and ok_cpl),
                "margin_ftlbf": float(torque_margin_ftlbf),
                "iq_req": float(iq_req),
                "iq_max": float(iq_max),
                "i_mag": float(im_rep),
                "i_inv": float(i_inv),
                "vll_motor": float(math.sqrt(3.0) * v_motor),
                "vll_inv_req": float(math.sqrt(3.0) * v_inv),
                "vll_cmd": float(math.sqrt(3.0) * v_cmd),
                "p_cable": float(p_cable),
                "loss_pct": float(loss_pct),
                "bind": "+".join(flags) if flags else "—",
                "tau_extra": float(tau_extra),
                "mag_slip": bool(mag_slip),
            }
        ...
        for preset in preset_names:
            for wires in (1, 2):
                row_label = f"{preset} | {wires}w"
                labels.append(row_label)
                cab = _cable_from_preset(preset, wires)
                metric_map[row_label] = [_fixed_req_metrics(float(r), target_tq, cab) for r in rpm_pts]

        col_labels = ["Cable / wiring"] + [f"{r:g}" for r in rpm_pts]
        col_widths = [0.26] + [max(0.05, (1.0 - 0.26) / max(1, len(rpm_pts)))] * len(rpm_pts)

        rows_feas = []
        over1 = {}
        for ridx, lab in enumerate(labels, start=1):
            vals = metric_map[lab]
            row = [lab]
            for j, met in enumerate(vals):
                row.append(("PASS" if met["feasible"] else "FAIL") + f"\n({met['margin_ftlbf']:.0f})")
                over1[(ridx, 1 + j)] = green if met["feasible"] else red
            rows_feas.append(row)
        subtitle1 = (
            f"Fixed requirement comparison at the SAME output demand: {target_tq:.0f} ft-lbf. "
            f"Cells show PASS/FAIL with torque margin in ft-lbf relative to required torque. "
            f"Unlike Envelope/Tables, this is not each cable's own max-torque point."
        )
        _make_table(pane.ax1, "A2A feasibility vs RPM", _wrap(subtitle1, 92), col_labels, rows_feas, col_widths,
                    left_align_cols=(0,), cell_bg_overrides=over1, fontsize=7.4, header_fontsize=8.5)
        ...
        for ridx, lab in enumerate(labels, start=1):
            ...
            for j, met in enumerate(vals):
                lp = met["loss_pct"]
                if np.isfinite(lp):
                    row.append(f"{met['p_cable']:.0f}\n({lp:.0f}%)")
                    over2[(ridx, 1 + j)] = green if lp < 15.0 else (amber if lp < 30.0 else red)
                else:
                    row.append(f"{met['p_cable']:.0f}\n(—)")
                    over2[(ridx, 1 + j)] = grey
            rows_loss.append(row)
        subtitle2 = (
            "Cable copper loss at the same required output torque. Cells show W and, in parentheses, cable-loss as % of mechanical output power."
        )
        _make_table(pane.ax2, "A2A cable loss vs RPM", _wrap(subtitle2, 92), col_labels, rows_loss, col_widths,
                    left_align_cols=(0,), cell_bg_overrides=over2, fontsize=7.4, header_fontsize=8.5)
        ...
        for ridx, lab in enumerate(labels, start=1):
            ...
            row = [lab]
            for j, met in enumerate(vals):
                row.append(f"{met['i_mag']:.2f}\n({met['iq_req']:.2f})")
                util = met['iq_req'] / max(1e-12, met['iq_max'])
                over3[(ridx, 1 + j)] = green if util < 0.85 else (amber if util < 1.0 else red)
            rows_cur.append(row)
        subtitle3 = (
            "Current required for the same load. Cells show |I| used (phase Arms) and, in parentheses, Iq required. "
            "Color indicates Iq utilization vs Iq_max."
        )
        _make_table(pane.ax3, "A2A current demand vs RPM", _wrap(subtitle3, 92), col_labels, rows_cur, col_widths,
                    left_align_cols=(0,), cell_bg_overrides=over3, fontsize=7.4, header_fontsize=8.5)
        ...
        for ridx, lab in enumerate(labels, start=1):
            ...
            row = [lab]
            for j, met in enumerate(vals):
                row.append(f"{met['vll_inv_req']:.0f}/{met['vll_cmd']:.0f}\n{met['bind']}")
                b = str(met['bind'])
                if 'Vdh' in b:
                    over4[(ridx, 1 + j)] = red
                elif 'Vsurf' in b:
                    over4[(ridx, 1 + j)] = amber
                elif 'I' in b:
                    over4[(ridx, 1 + j)] = blue
                elif met['feasible']:
                    over4[(ridx, 1 + j)] = green
                else:
                    over4[(ridx, 1 + j)] = grey
            rows_v.append(row)
        subtitle4 = (
            "Surface voltage demand at the same load. Cells show VLL_inv_req / VLL_cmd in Vrms, with the binding cause underneath."
        )
        _make_table(pane.ax4, "A2A voltage demand + limiter", _wrap(subtitle4, 92), col_labels, rows_v, col_widths,
                    left_align_cols=(0,), cell_bg_overrides=over4, fontsize=7.2, header_fontsize=8.5)
    ...
    def _plot_architecture(self) -> None:
        ...
        # --- diagram blocks (left) ---
        cmc_on = bool(getattr(getattr(p, 'cmc', None), 'enabled', False))
        blocks = [
            ...
            ("Post-filter\nCMC",
             f"{getattr(p.cmc, 'part_number', 'CMC')}\nLcm={float(getattr(p.cmc, 'cm_l_mh', 0.0)):.0f} mH | Rdc={float(getattr(p.cmc, 'rdc_ohm_per_phase', 0.0)):.3f} Ω",
             cmc_on),
        ...
        ycur = _sec(axs, ycur, "Blocks", [
            f"{'✓' if bool(p.sine_filter.enabled) else '–'} Sine filter — Lf={p.sine_filter.lf_h * 1e3:.2f} mH, Cf={p.sine_filter.cf_f * 1e6:.1f} µF",
            f"{'✓' if cmc_on else '–'} Post-filter CMC — Irated={float(getattr(p.cmc, 'rated_current_arms', 0.0)):.1f} Arms, Vrated={float(getattr(p.cmc, 'rated_voltage_vac', 0.0)):.0f} Vac",
    ...
    def _plot_sweeps(self):
        ...
        self._plot_sweep_sine_filter_trade()
    ...
    def _plot_sweep_sine_filter_trade(self):
        """Sweep sine-filter capacitor Cf and show the trade-off between filtering burden and torque capability."""
        p0 = self.params
        fast = bool(self.chk_fast_sweeps.isChecked())

        ax1, ax2, ax3, ax4 = self.sweep_sf.ax1, self.sweep_sf.ax2, self.sweep_sf.ax3, self.sweep_sf.ax4
        for ax in (ax1, ax2, ax3, ax4):
            ax.clear()
        ...
        current_cf_uf = max(0.0, float(sf0.cf_f) * 1e6)
        if current_cf_uf > 1e-9:
            cf_min = max(0.1, 0.20 * current_cf_uf)
            cf_max = max(5.0, 4.00 * current_cf_uf)
        else:
            cf_min, cf_max = 0.1, 5.0
        cfs_uf = np.linspace(cf_min, cf_max, 41)

        # Use the high end of the continuous operating band as the focus point if available.
        try:
            focus_out_rpm = float(getattr(p0.target, 'out_rpm', 1.0))
        except Exception:
            focus_out_rpm = 1.0
        focus_out_rpm = max(0.5, min(1.5, focus_out_rpm if math.isfinite(focus_out_rpm) else 1.0))

        tq_05, tq_10 = [], []
        i_loads, i_shunts, i_invs, iq_maxs = [], [], [], []
        v_cmds, v_invreqs, v_filterdrops, v_motors, v_cabledrops = [], [], [], [], []
        fres_list, xc10k_list = [], []

        def max_iq(model, motor_rpm: float):
            if fast:
                return model.max_iq_given_limits_fast(motor_rpm, cab)
            return model.max_iq_given_limits(motor_rpm, cab)

        for cf_uf in cfs_uf:
            p = copy.deepcopy(p0)
            p.sine_filter.enabled = True
            p.sine_filter.cf_f = float(cf_uf) * 1e-6
            model = SystemModel(p)

            # Torque capability at 0.5 rpm and 1.0 rpm output.
            for out_rpm, store in ((0.5, tq_05), (1.0, tq_10)):
                motor_rpm = float(out_rpm) * G
                iq_max, id_best, im_best, *_ = max_iq(model, motor_rpm)
                kt_eff = model.kt_effective_nm_per_arms()
                tau_extra, _, _ = model.tau_extra_nm(rpm_to_rad_s(motor_rpm))
                t_motor_useful = max(0.0, float(kt_eff) * float(iq_max) - float(tau_extra))
                t_out_nm = t_motor_useful * G * eta
                store.append(nm_to_ft_lbf(t_out_nm))

            # Burden breakdown at the focus operating point.
            motor_rpm = float(focus_out_rpm) * G
            iq_max, id_best, im_best, f_e, v_cmd, v_surface_limit, v_dh = max_iq(model, motor_rpm)
            v_motor = model.motor_voltage_required_phase_rms(motor_rpm, id_best, iq_max)
            v_drop_cable = model.cable_drop_phase_rms(motor_rpm, im_best, cab)
            v_node = float(v_motor) + float(v_drop_cable)
            v_post_filter = model.post_filter_node_phase_rms(motor_rpm, v_node, float(im_best))
            _, _, i_shunt = model.sine_filter_shunt_current_components(motor_rpm, v_post_filter)
            i_inv = model.inverter_current_mag_rms(motor_rpm, v_node, float(im_best))
            v_inv_req = model.inverter_voltage_required_phase_rms(motor_rpm, v_node, float(im_best))
            v_filter_drop = max(0.0, float(v_inv_req) - float(v_post_filter))

            i_loads.append(float(im_best))
            i_shunts.append(float(i_shunt))
            i_invs.append(float(i_inv))
            iq_maxs.append(float(iq_max))

            v_cmds.append(float(v_cmd))
            v_invreqs.append(float(v_inv_req))
            v_filterdrops.append(float(v_filter_drop))
            v_motors.append(float(v_motor))
            v_cabledrops.append(float(v_drop_cable))

            c_eq = model.sine_filter_c_wye_per_phase_f()
            lf_h = max(0.0, float(p.sine_filter.lf_h))
            if lf_h > 0.0 and c_eq > 0.0:
                fres = 1.0 / (2.0 * math.pi * math.sqrt(lf_h * c_eq))
                xc10k = 1.0 / (2.0 * math.pi * 10000.0 * c_eq)
            else:
                fres = float('nan')
                xc10k = float('nan')
            fres_list.append(fres)
            xc10k_list.append(xc10k)

        tq_05 = np.array(tq_05)
        tq_10 = np.array(tq_10)
        i_loads = np.array(i_loads)
        i_shunts = np.array(i_shunts)
        i_invs = np.array(i_invs)
        iq_maxs = np.array(iq_maxs)
        v_cmds = np.array(v_cmds)
        v_invreqs = np.array(v_invreqs)
        v_filterdrops = np.array(v_filterdrops)
        v_motors = np.array(v_motors)
        v_cabledrops = np.array(v_cabledrops)
        fres_list = np.array(fres_list)
        xc10k_list = np.array(xc10k_list)

        i_lim = cab.i_phase_limit()

        ax1.plot(cfs_uf, tq_05, linewidth=2, label='Max torque @ 0.5 rpm')
        ax1.plot(cfs_uf, tq_10, linewidth=2, linestyle='--', label='Max torque @ 1.0 rpm')
        ax1.axhline(250.0, linestyle=':', linewidth=2, label='250 cont')
        ax1.axhline(1000.0, linestyle='-.', linewidth=2, label='1000 peak')
        ax1.axvline(current_cf_uf, linestyle='--', linewidth=1.5, label='Current Cf')
        ax1.set_title('Sine filter Cf sensitivity: torque capability')
        ax1.set_xlabel(f'Cf entered in UI (µF, current connection = {str(sf0.cap_connection).upper()})')
        ax1.set_ylabel('ft-lbf')
        ax1.grid(True, alpha=0.3)
        ax1.legend(fontsize=8, loc='best')

        ax2.plot(cfs_uf, iq_maxs, linewidth=2, label='Iq_max')
        ax2.plot(cfs_uf, i_loads, linewidth=2, linestyle='--', label='I_load magnitude')
        ax2.plot(cfs_uf, i_shunts, linewidth=2, linestyle='-.', label='I_shunt (capacitor)')
        ax2.plot(cfs_uf, i_invs, linewidth=2, linestyle=':', label='I_inverter total')
        ax2.axhline(i_lim, linestyle='--', linewidth=1.5, label='Phase current limit')
        ax2.axvline(current_cf_uf, linestyle='--', linewidth=1.5)
        ax2.set_title(f'Sine filter burden @ {focus_out_rpm:.2f} rpm output')
        ax2.set_xlabel(f'Cf entered in UI (µF, current connection = {str(sf0.cap_connection).upper()})')
        ax2.set_ylabel('Arms')
        ax2.grid(True, alpha=0.3)
        ax2.legend(fontsize=8, loc='best')

        ax3.plot(cfs_uf, v_cmds, linewidth=2, label='Vcmd available')
        ax3.plot(cfs_uf, v_invreqs, linewidth=2, linestyle='--', label='Vreq at inverter')
        ax3.plot(cfs_uf, v_motors, linewidth=2, linestyle='-.', label='Vmotor required')
        ax3.plot(cfs_uf, v_filterdrops, linewidth=2, linestyle=':', label='Vdrop filter')
        ax3.plot(cfs_uf, v_cabledrops, linewidth=2, label='Vdrop cable')
        ax3.axvline(current_cf_uf, linestyle='--', linewidth=1.5)
        ax3.set_title(f'Voltage burden @ {focus_out_rpm:.2f} rpm output')
        ax3.set_xlabel(f'Cf entered in UI (µF, current connection = {str(sf0.cap_connection).upper()})')
        ax3.set_ylabel('Vrms (phase)')
        ax3.grid(True, alpha=0.3)
        ax3.legend(fontsize=8, loc='best')

        ax4.plot(cfs_uf, fres_list, linewidth=2, label='LC resonance f0')
        ax4.axhline(5000.0, linestyle=':', linewidth=1.5, label='LTS DL ~5 kHz')
        ax4.axhline(10000.0, linestyle='--', linewidth=1.5, label='LTS UL ~10 kHz')
        ax4.axhline(15000.0, linestyle='-.', linewidth=1.5, label='LTS band top ~15 kHz')
        ax4.axvline(current_cf_uf, linestyle='--', linewidth=1.5)
        ax4.set_title('HF proxy: resonance and capacitor reactance')
        ax4.set_xlabel(f'Cf entered in UI (µF, current connection = {str(sf0.cap_connection).upper()})')
        ax4.set_ylabel('Hz')
        ax4.grid(True, alpha=0.3)
        ax4b = ax4.twinx()
        ax4b.plot(cfs_uf, xc10k_list, linewidth=2, linestyle='--', label='|Xc| @ 10 kHz')
        ax4b.set_ylabel('Ohm')
        lines1, labels1 = ax4.get_legend_handles_labels()
        lines2, labels2 = ax4b.get_legend_handles_labels()
        ax4.legend(lines1 + lines2, labels1 + labels2, fontsize=8, loc='best')

        note = 'WYE: Ceq≈Cf' if str(sf0.cap_connection).upper().startswith('WYE') else 'DELTA: Ceq per phase ≈ 3×Cf'
        ax4.text(0.02, 0.02, note, transform=ax4.transAxes, fontsize=8,
                 va='bottom', ha='left', bbox=dict(boxstyle='round', alpha=0.15))

        self.sweep_sf.fig.tight_layout(pad=2.0)
        self.sweep_sf.canvas.draw_idle()
